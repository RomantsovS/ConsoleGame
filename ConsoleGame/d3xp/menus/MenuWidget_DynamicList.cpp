...
void idMenuWidget_DynamicList::Update() {
	...
	for (size_t optionIndex = 0; optionIndex < GetNumVisibleOptions(); ++optionIndex) {
		...
		if (optionIndex >= children.size()) {
			std::shared_ptr<idSWFSpriteInstance> item = GetSprite()->GetScriptObject()->GetNestedSprite(va("item%d", optionIndex).c_str());
			if (item) {
...
========================
*/
bool idMenuWidget_DynamicList::PrepareListElement(std::shared_ptr<idMenuWidget> widget, const size_t childIndex) {
	...
	std::shared_ptr<idMenuWidget_Button> const button = std::dynamic_pointer_cast<idMenuWidget_Button>(widget);
	if (button) {
...
========================
*/
void idMenuWidget_DynamicList::SetListData(std::vector<std::vector<std::string>>& list) {
	listItemInfo.clear();
