...
========================
*/
void idMenuWidget_Button::Update() noexcept {
  ...
  std::shared_ptr<idSWFScriptObject> spriteObject =
      GetSprite()->GetScriptObject();

  if (btnLabel.empty()) {
    if (values.size() > 0) {
      for (size_t val = 0; val < values.size(); ++val) {
        std::shared_ptr<idSWFScriptObject> textObject =
    ...
    std::shared_ptr<idSWFScriptObject> textObject =
        spriteObject->GetNestedObj("label0", "txtVal");
    if (textObject) {
      std::shared_ptr<idSWFTextInstance> text = textObject->GetText();
  ...
  spriteObject->Set(
      ...
      idSWFScriptVar(std::make_shared<WrapWidgetSWFEvent>(
          shared_from_this(), widgetEvent_t::WIDGET_EVENT_PRESS, 0)));
  spriteObject->Set(
      "onRelease",
      idSWFScriptVar(std::make_shared<WrapWidgetSWFEvent>(
...
void idMenuWidget_LobbyButton::Update() noexcept {
  ...
  std::shared_ptr<idSWFScriptObject> spriteObject =
      GetSprite()->GetScriptObject();
  std::shared_ptr<idSWFTextInstance> txtName =
      spriteObject->GetNestedText("itemName", "txtVal");
  // std::shared_ptr<idSWFSpriteInstance> talkIcon =
  // spriteObject->GetNestedSprite("chaticon");
