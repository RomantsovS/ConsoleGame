  ...
  for (size_t optionIndex = 0; optionIndex < GetNumVisibleOptions();
    ...
    if (optionIndex < GetChildren().size()) {
      ...
      const int controlIndex =
          GetNumVisibleOptions() -
          std::min(GetNumVisibleOptions(), GetTotalNumberOfOptions()) +
          optionIndex;
    ...
    if (!shown) {
      // hide the item
      std::shared_ptr<idSWFSpriteInstance> const sprite =
          GetSprite()->GetScriptObject()->GetSprite(
              va("item%d", optionIndex - GetTotalNumberOfOptions()));
      if (sprite) {
...
========================
*/
bool idMenuWidget_List::HandleAction(idWidgetAction& action,
...
Pure functional encapsulation of how to calculate a new index and offset based
on how the user chose to move through the list.
========================
*/
void idMenuWidget_List::CalculatePositionFromIndexDelta(
    int& outIndex, int& outOffset, const int currentIndex,
    const int currentOffset, const int windowSize, const int maxSize,
    const int indexDelta, const bool allowWrapping,
    const bool wrapAround) const {
  ...
  // the intended behavior is that outOffset and outIndex are always within
  // maxSize of each other, as they are meant to model a window of items that
...
void idMenuWidget_List::Scroll(const int scrollAmount, const bool wrapAround) {
  ...
  CalculatePositionFromIndexDelta(newIndex, newOffset, GetViewIndex(),
                                  GetViewOffset(), GetNumVisibleOptions(),
                                  GetTotalNumberOfOptions(), scrollAmount,
                                  IsWrappingAllowed(), wrapAround);
...
void idMenuWidget_List::ScrollOffset(const int scrollAmount) {
  ...
  CalculatePositionFromOffsetDelta(newIndex, newOffset, GetViewIndex(),
                                   GetViewOffset(), GetNumVisibleOptions(),
                                   GetTotalNumberOfOptions(), scrollAmount);
  if (newOffset != GetViewOffset()) {
    SetViewOffset(newOffset);
