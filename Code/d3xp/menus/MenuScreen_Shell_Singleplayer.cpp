...
========================
*/
void idMenuScreen_Shell_Singleplayer::Initialize(
  ...
  while (options->GetChildren().size() < NUM_SINGLEPLAYER_OPTIONS) {
    std::shared_ptr<idMenuWidget_Button> buttonWidget =
        std::make_shared<idMenuWidget_Button>();
    buttonWidget->AddEventAction(widgetEvent_t::WIDGET_EVENT_PRESS)
        .Set(widgetAction_t::WIDGET_ACTION_PRESS_FOCUSED,
             options->GetChildren().size());
  ...
  options->AddEventAction(widgetEvent_t::WIDGET_EVENT_SCROLL_DOWN)
...
void idMenuScreen_Shell_Singleplayer::ShowScreen() {
  ...
  session->GetSaveGameManager().GetEnumeratedSavegames(); canContinue =
  (saveGameInfo.Num() > 0); if (canContinue) {
          ...
          int index = 0;
          idMenuWidget_Button* buttonWidget =
  dynamic_cast<idMenuWidget_Button*>(&options->GetChildByIndex(index)); if
  (buttonWidget != NULL) {
          ...
          buttonWidget =
  dynamic_cast<idMenuWidget_Button*>(&options->GetChildByIndex(index)); if
  (buttonWidget != NULL) { buttonWidget->SetDescription("#str_02209");
          }
          index++;
          buttonWidget =
  dynamic_cast<idMenuWidget_Button*>(&options->GetChildByIndex(index)); if
  (buttonWidget != NULL) { buttonWidget->SetDescription("#str_02213");
          }
          index++;
  ...
  if (options) {
    int index = 0;
    std::shared_ptr<idMenuWidget_Button> buttonWidget =
        std::dynamic_pointer_cast<idMenuWidget_Button>(
            options->GetChildByIndex(index));
    if (buttonWidget) {
      // buttonWidget->SetDescription("start a new game");
    }
    index++;
    buttonWidget = std::dynamic_pointer_cast<idMenuWidget_Button>(
        options->GetChildByIndex(index));
...
void idMenuScreen_Shell_Singleplayer::ContinueGame() noexcept {
  /*const saveGameDetailsList_t& saveGameInfo =
  session->GetSaveGameManager().GetEnumeratedSavegames(); saveGameDetailsList_t
  sortedSaves = saveGameInfo; sortedSaves.Sort(idSort_SavesByDate()); if
  (sortedSaves.Num() > 0) { if (sortedSaves[0].damaged) { class
  idSWFScriptFunction_ContinueDamaged : public idSWFScriptFunction_RefCounted {
                  public:
  ...
  const idSWFParmList& parms) {
                  ...
                  callbacks.Append(new (TAG_SWF)
  idSWFScriptFunction_ContinueDamaged()); idStaticList< idStrId, 4 > optionText;
                  optionText.Append(idStrId("#str_04339"));	// OK
                  common->Dialog().AddDynamicDialog(GDM_CORRUPT_CONTINUE,
  callbacks, optionText, false, "");
          }
          else {
...
========================
*/
bool idMenuScreen_Shell_Singleplayer::HandleAction(idWidgetAction& action,
  ...
  if (spMenuData->ActiveScreen() !=
      static_cast<int>(shellAreas_t::SHELL_AREA_CAMPAIGN)) {
    return false;
  }
  ...
  switch (actionType) {
    ...
    case widgetAction_t::WIDGET_ACTION_PRESS_FOCUSED: {
      ...
      const saveGameDetailsList_t& saveGameInfo =
      session->GetSaveGameManager().GetEnumeratedSavegames(); canContinue =
      (saveGameInfo.Num() > 0); if (canContinue) { if (selectionIndex == 0) {
                      ContinueGame();

              }
              else if (selectionIndex == 1) {
                      class idSWFScriptFunction_NewGame : public
      idSWFScriptFunction_RefCounted { public:
                              idSWFScriptFunction_NewGame(idMenuHandler*
      _menuData, bool _accept) { menuData = _menuData; accept = _accept;
                              }
                              idSWFScriptVar Call(idSWFScriptObject* thisObject,
      const idSWFParmList& parms) {
                                      common->Dialog().ClearDialog(GDM_DELETE_AUTOSAVE);
                                      if (accept) {
                                              menuData->SetNextScreen(SHELL_AREA_NEW_GAME,
      MENU_TRANSITION_SIMPLE);
                      ...
                      common->Dialog().AddDialog(GDM_DELETE_AUTOSAVE,
      DIALOG_ACCEPT_CANCEL, new idSWFScriptFunction_NewGame(menuData, true), new
      idSWFScriptFunction_NewGame(menuData, false), true);
              }
              else if (selectionIndex == 2) {
                      menuData->SetNextScreen(SHELL_AREA_LOAD,
      MENU_TRANSITION_SIMPLE);
              }
      }
      else {*/
