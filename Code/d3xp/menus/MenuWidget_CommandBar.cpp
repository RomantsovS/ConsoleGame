...
static const std::string BUTTON_NAMES[] = {"joy1", "joy2",  "joy3",
                                           "joy4", "joy10", "tab"};
...
void idMenuWidget_CommandBar::Update() noexcept {
  ...
  for (size_t i = 0; i < buttonOrder.size(); ++i) {
    ...
    std::shared_ptr<idSWFSpriteInstance> const buttonSprite =
        GetSprite()->GetScriptObject()->GetSprite(buttonName);
    if (!buttonSprite) {
      continue;
    }
    std::shared_ptr<idSWFTextInstance> const buttonText =
        buttonSprite->GetScriptObject()->GetText("txt_info");
    if (!buttonText) {
      continue;
    }
    ...
    if (buttons[i].action.GetType() != widgetAction_t::WIDGET_ACTION_NONE) {
      std::shared_ptr<idSWFScriptObject> shortcutKeys =
          GetSWFObject()->GetGlobal("shortcutKeys").GetObjectScript();
      if (shortcutKeys) {
        buttonSprite->GetScriptObject()->Set(
            "onPress",
            idSWFScriptVar(std::make_shared<WrapWidgetSWFEvent>(
                shared_from_this(), widgetEvent_t::WIDGET_EVENT_COMMAND, i)));

        // bind the main action - need to use all caps here because shortcuts
        // are stored that way
        shortcutName = buttonName;
        // shortcutName.ToUpper();
        shortcutKeys->Set(shortcutName, buttonSprite->GetScriptObject());

        // Some other keys have additional bindings. Remember that the button
        // here is actually the virtual button, and the physical button could be
        // swapped based on the UseCircleForAccept business on JPN PS3.
        switch (i) {
          case BUTTON_JOY1: {
