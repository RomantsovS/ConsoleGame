...
========================
*/
void idMenuScreen_Shell_Root::Initialize(
  ...
  /*
  options->AddEventAction(WIDGET_EVENT_SCROLL_DOWN).Set(new (TAG_SWF)
  idWidgetActionHandler(options, WIDGET_ACTION_EVENT_SCROLL_DOWN_START_REPEATER,
  WIDGET_EVENT_SCROLL_DOWN));
  options->AddEventAction(WIDGET_EVENT_SCROLL_UP).Set(new (TAG_SWF)
  idWidgetActionHandler(options, WIDGET_ACTION_EVENT_SCROLL_UP_START_REPEATER,
  WIDGET_EVENT_SCROLL_UP));
  options->AddEventAction(WIDGET_EVENT_SCROLL_DOWN_RELEASE).Set(new (TAG_SWF)
  idWidgetActionHandler(options, WIDGET_ACTION_EVENT_STOP_REPEATER,
  WIDGET_EVENT_SCROLL_DOWN_RELEASE));
  options->AddEventAction(WIDGET_EVENT_SCROLL_UP_RELEASE).Set(new (TAG_SWF)
  idWidgetActionHandler(options, WIDGET_ACTION_EVENT_STOP_REPEATER,
  WIDGET_EVENT_SCROLL_UP_RELEASE));
  options->AddEventAction(WIDGET_EVENT_SCROLL_DOWN_LSTICK).Set(new (TAG_SWF)
  idWidgetActionHandler(options, WIDGET_ACTION_EVENT_SCROLL_DOWN_START_REPEATER,
  WIDGET_EVENT_SCROLL_DOWN_LSTICK));
  options->AddEventAction(WIDGET_EVENT_SCROLL_UP_LSTICK).Set(new (TAG_SWF)
  idWidgetActionHandler(options, WIDGET_ACTION_EVENT_SCROLL_UP_START_REPEATER,
  WIDGET_EVENT_SCROLL_UP_LSTICK));
  options->AddEventAction(WIDGET_EVENT_SCROLL_DOWN_LSTICK_RELEASE).Set(new
  (TAG_SWF) idWidgetActionHandler(options, WIDGET_ACTION_EVENT_STOP_REPEATER,
  WIDGET_EVENT_SCROLL_DOWN_LSTICK_RELEASE));
  options->AddEventAction(WIDGET_EVENT_SCROLL_UP_LSTICK_RELEASE).Set(new
  (TAG_SWF) idWidgetActionHandler(options, WIDGET_ACTION_EVENT_STOP_REPEATER,
  WIDGET_EVENT_SCROLL_UP_LSTICK_RELEASE));
  */

  AddEventAction(widgetEvent_t::WIDGET_EVENT_SCROLL_RIGHT)
      .Set(std::make_shared<idWidgetActionHandler>(
          shared_from_this(),
          actionHandler_t::WIDGET_ACTION_EVENT_SCROLL_RIGHT_START_REPEATER,
          widgetEvent_t::WIDGET_EVENT_SCROLL_RIGHT));
  AddEventAction(widgetEvent_t::WIDGET_EVENT_SCROLL_RIGHT_RELEASE)
      .Set(std::make_shared<idWidgetActionHandler>(
          shared_from_this(),
          actionHandler_t::WIDGET_ACTION_EVENT_STOP_REPEATER,
          widgetEvent_t::WIDGET_EVENT_SCROLL_RIGHT_RELEASE));
  AddEventAction(widgetEvent_t::WIDGET_EVENT_SCROLL_LEFT)
      .Set(std::make_shared<idWidgetActionHandler>(
          shared_from_this(),
          actionHandler_t::WIDGET_ACTION_EVENT_SCROLL_LEFT_START_REPEATER,
          widgetEvent_t::WIDGET_EVENT_SCROLL_LEFT));
  AddEventAction(widgetEvent_t::WIDGET_EVENT_SCROLL_LEFT_RELEASE)
      .Set(std::make_shared<idWidgetActionHandler>(
          shared_from_this(),
          actionHandler_t::WIDGET_ACTION_EVENT_STOP_REPEATER,
          widgetEvent_t::WIDGET_EVENT_SCROLL_LEFT_RELEASE));
  AddEventAction(widgetEvent_t::WIDGET_EVENT_PRESS)
      .Set(widgetAction_t::WIDGET_ACTION_PRESS_FOCUSED, 0);
}
...
void idMenuScreen_Shell_Root::ShowScreen() {
  ...
  if (auto spMenuData = menuData.lock()) {
    std::shared_ptr<idMenuHandler_Shell> shell =
...
========================
*/
void idMenuScreen_Shell_Root::HideScreen() { idMenuScreen::HideScreen(); }
...
========================
*/
void idMenuScreen_Shell_Root::HandleExitGameBtn() { common->QuitRequest(); }
...
bool idMenuScreen_Shell_Root::HandleAction(idWidgetAction& action,
  ...
  if (spMenuData->ActiveScreen() !=
      static_cast<int>(shellAreas_t::SHELL_AREA_ROOT)) {
    return false;
  }
  ...
  switch (actionType) {
    ...
    case widgetAction_t::WIDGET_ACTION_PRESS_FOCUSED: {
      if (true) {
        ...
        const std::shared_ptr<idMenuWidget_MenuButton> buttonWidget =
            std::dynamic_pointer_cast<idMenuWidget_MenuButton>(
                menuBar->GetChildByIndex(menuBar->GetFocusIndex()));
        if (!buttonWidget) {
          return true;
        }

        idWidgetEvent pressEvent(widgetEvent_t::WIDGET_EVENT_PRESS, 0, nullptr,
                                 idSWFParmList());
