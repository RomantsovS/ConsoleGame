...
enum class widgetAction_t {
  ...
  WIDGET_ACTION_SCROLL_VERTICAL,  // scroll something. takes one param = amount
                                  // to scroll (can be negative)
  WIDGET_ACTION_SCROLL_VERTICAL_VARIABLE,
  WIDGET_ACTION_SCROLL_PAGE,
  WIDGET_ACTION_SCROLL_HORIZONTAL,  // scroll something. takes one param =
                                    // amount to scroll (can be negative)
...
class idWidgetEvent {
 public:
  idWidgetEvent()
      : type(widgetEvent_t::WIDGET_EVENT_PRESS), arg(0), thisObject(nullptr) {
#ifdef DEBUG_PRINT_Ctor_Dtor
    common->DPrintf("%s ctor\n", "idWidgetEvent");
#endif  // DEBUG_PRINT_Ctor_Dtor
  }

  idWidgetEvent(const widgetEvent_t type_, const int arg_,
                idSWFScriptObject* thisObject_, const idSWFParmList& parms_)
      : type(type_), arg(arg_), thisObject(thisObject_), parms(parms_) {
#ifdef DEBUG_PRINT_Ctor_Dtor
...
class idWidgetAction {
 public:
  ...
  bool operator==(const idWidgetAction& otherAction) const {
    ...
    // everything else is equal, so check all parms. NOTE: this assumes we are
    // only sending integral types.
    for (size_t i = 0; i < GetParms().size(); ++i) {
      if (GetParms()[i]->GetType() != otherAction.GetParms()[i]->GetType() ||
          GetParms()[i]->ToInteger() !=
              otherAction.GetParms()[i]->ToInteger()) {
        return false;
      }
    }
  ...
  std::shared_ptr<idSWFScriptFunction> GetScriptFunction() noexcept {
...
We're using a model/view architecture, so this is the combination of both model
and view.  The other part of the view is the SWF itself.
================================================
*/
class idMenuWidget : public std::enable_shared_from_this<idMenuWidget> {
  ...
  */
  class WrapWidgetSWFEvent : public idSWFScriptFunction {
   public:
    WrapWidgetSWFEvent(std::shared_ptr<idMenuWidget> widget,
                       const widgetEvent_t event, const int eventArg)
        : targetWidget(widget), targetEvent(event), targetEventArg(eventArg) {
#ifdef DEBUG_PRINT_Ctor_Dtor
      common->DPrintf("%s ctor\n", "WrapWidgetSWFEvent");
#endif  // DEBUG_PRINT_Ctor_Dtor
    }
    ...
    idSWFScriptVar Call(idSWFScriptObject* thisObject,
      ...
      targetWidget.lock()->ReceiveEvent(
          idWidgetEvent(targetEvent, targetEventArg, thisObject, parms));
  ...
  // typically this is where the allocations for a widget will occur: sub
  // widgets, etc. Note that SWF sprite objects may not be accessible at this
  // point.
  virtual void Initialize(std::shared_ptr<idMenuHandler> data) noexcept {
    menuData = data;
  }
  ...
  std::shared_ptr<idSWFSpriteInstance> GetSprite() noexcept {
    return boundSprite;
  }
  ...
  void SetSpritePath(const std::vector<std::string>& spritePath_,
                     Args... args) {
    // idLib::Printf(__PRETTY_FUNCTION__);
  ...
  // Executes an event in the context of this widget.  Only rarely should this
  // be called directly.  Instead calls should go through ReceiveEvent which
  // will propagate the event through the standard focus order.
  virtual bool ExecuteEvent(const idWidgetEvent& event);

  // returns the list of actions for a given event, or NULL if no actions are
  // registered for that event.  Events should not be directly added to the
  // returned list.  Instead use AddEventAction for adding new events.
  std::vector<idWidgetAction>* GetEventActions(
  ...
  //------------------------
  /*void
  SetDataSource(idMenuDataSource* dataSource, const int fieldIndex);
  idMenuDataSource* GetDataSource() { return dataSource; }
  void
  SetDataSourceFieldIndex(const int dataSourceFieldIndex_) {
  dataSourceFieldIndex = dataSourceFieldIndex_; } int
  GetDataSourceFieldIndex() const { return dataSourceFieldIndex; }
  */
  std::shared_ptr<idMenuWidget> GetFocus() noexcept {
    return (focusIndex >= 0 && focusIndex < children.size())
               ? children[focusIndex]
               : nullptr;
  }
  ...
  std::shared_ptr<idMenuWidget> GetChildByIndex(
  ...
  const std::shared_ptr<idMenuWidget> GetParent() const noexcept {
    return parent.lock();
  }
  void SetParent(std::shared_ptr<idMenuWidget> parent_) noexcept {
    parent = parent_;
  }
...
This might be better named "ComboButton", as it contains behavior for several
controls along with standard button behavior.
================================================
*/
...
class idMenuWidget_MenuButton : public idMenuWidget_Button {
 public:
  idMenuWidget_MenuButton() : xPos(0) {}
...
class idMenuWidget_List : public idMenuWidget {
 public:
  ...
  // virtual void				ObserveEvent(const idMenuWidget&
  // widget, const idWidgetEvent& event);
  virtual void Scroll(const int scrollIndexAmount,
                      const bool wrapAround = false);
  virtual void ScrollOffset(const int scrollIndexAmount);
  virtual size_t GetTotalNumberOfOptions() const noexcept {
    return GetChildren().size();
  }
  // virtual bool				PrepareListElement(idMenuWidget&
  // widget, const int childIndex) { return true; }
  ...
  void SetNumVisibleOptions(const size_t numVisibleOptions_) noexcept {
  ...
  void CalculatePositionFromIndexDelta(int& outIndex, int& outOffset,
                                       const int currentIndex,
                                       const int currentOffset,
                                       const int windowSize, const int maxSize,
                                       const int indexDelta,
                                       const bool allowWrapping,
                                       const bool wrapAround = false) const;
  void CalculatePositionFromOffsetDelta(int& outIndex, int& outOffset,
                                        const int currentIndex,
                                        const int currentOffset,
                                        const int windowSize, const int maxSize,
                                        const int offsetDelta) const;
...
class idMenuWidget_CommandBar : public idMenuWidget {
 public:
  ...
  idMenuWidget_CommandBar() { buttons.resize(MAX_BUTTONS); }

  void Update() noexcept override;
  bool ExecuteEvent(const idWidgetEvent& event) override;

  buttonInfo_t* GetButton(const button_t button) noexcept {
...
class idMenuWidget_LobbyList : public idMenuWidget_List {
 public:
  idMenuWidget_LobbyList() : numEntries(0) {}
...
class idMenuWidget_DynamicList : public idMenuWidget_List {
 public:
  idMenuWidget_DynamicList() : controlList(false), ignoreColor(false) {}
  ...
  virtual bool PrepareListElement(std::shared_ptr<idMenuWidget>,
...
class idMenuWidget_MenuBar : public idMenuWidget_DynamicList {
 public:
  idMenuWidget_MenuBar()
      : totalWidth(0.0f), buttonPos(0.0f), rightSpacer(0.0f) {}

  void Update() noexcept override;
  void Initialize(std::shared_ptr<idMenuHandler> data) noexcept override;
  virtual void SetButtonSpacing(float rSpace) noexcept { rightSpacer = rSpace; }
  bool PrepareListElement(std::shared_ptr<idMenuWidget>,
                          const size_t childIndex) override;
...
class idWidgetActionHandler : public idSWFScriptFunction {
 public:
  idWidgetActionHandler(std::shared_ptr<idMenuWidget> widget,
                        actionHandler_t actionEventType, widgetEvent_t _event)
      : targetWidget(widget), type(actionEventType), targetEvent(_event) {
#ifdef DEBUG_PRINT_Ctor_Dtor
    common->DPrintf("%s ctor\n", "idWidgetActionHandler");
#endif  // DEBUG_PRINT_Ctor_Dtor
  }
  ...
  idSWFScriptVar Call(idSWFScriptObject* thisObject,
    ...
    switch (type) {
      case actionHandler_t::WIDGET_ACTION_EVENT_SCROLL_DOWN_START_REPEATER: {
        action.Set(
            widgetAction_t::WIDGET_ACTION_START_REPEATER,
            static_cast<int>(widgetAction_t::WIDGET_ACTION_SCROLL_VERTICAL), 1);
        handled = true;
        break;
      }
      case actionHandler_t::WIDGET_ACTION_EVENT_SCROLL_UP_START_REPEATER: {
        action.Set(
            widgetAction_t::WIDGET_ACTION_START_REPEATER,
            static_cast<int>(widgetAction_t::WIDGET_ACTION_SCROLL_VERTICAL),
            -1);
        handled = true;
        break;
      }
      case actionHandler_t::
          WIDGET_ACTION_EVENT_SCROLL_DOWN_START_REPEATER_VARIABLE: {
        action.Set(widgetAction_t::WIDGET_ACTION_START_REPEATER,
                   static_cast<int>(
                       widgetAction_t::WIDGET_ACTION_SCROLL_VERTICAL_VARIABLE),
                   1);
        handled = true;
        break;
      }
      ...
      case actionHandler_t::
          WIDGET_ACTION_EVENT_SCROLL_PAGE_DOWN_START_REPEATER: {
        action.Set(widgetAction_t::WIDGET_ACTION_START_REPEATER,
                   static_cast<int>(widgetAction_t::WIDGET_ACTION_SCROLL_PAGE),
                   1);
        handled = true;
        break;
      }
      ...
      case actionHandler_t::WIDGET_ACTION_EVENT_SCROLL_LEFT_START_REPEATER: {
        action.Set(
            widgetAction_t::WIDGET_ACTION_START_REPEATER,
            static_cast<int>(widgetAction_t::WIDGET_ACTION_SCROLL_HORIZONTAL),
            -1);
        handled = true;
        break;
      }
    ...
    if (handled) {
      if (auto tw = targetWidget.lock())
        tw->HandleAction(
            action, idWidgetEvent(targetEvent, 0, thisObject, parms), tw.get());
      else
