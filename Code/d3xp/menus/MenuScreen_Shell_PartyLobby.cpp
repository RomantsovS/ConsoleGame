...
void idMenuScreen_Shell_PartyLobby::Initialize(
  ...
  /*while (lobby->GetChildren().size() < 8) {
          std::shared_ptr<idMenuWidget_LobbyButton> buttonWidget =
  std::make_shared<idMenuWidget_LobbyButton>();
          buttonWidget->AddEventAction(widgetEvent_t::WIDGET_EVENT_PRESS).Set(WIDGET_ACTION_SELECT_GAMERTAG,
  lobby->GetChildren().Num());
          buttonWidget->AddEventAction(widgetEvent_t::WIDGET_EVENT_COMMAND).Set(WIDGET_ACTION_MUTE_PLAYER,
  lobby->GetChildren().Num()); buttonWidget->Initialize(data);
...
void idMenuScreen_Shell_PartyLobby::UpdateOptions() {
  ...
  if (/*(session->GetPartyLobbyBase().IsHost() && (!isHost || forceUpdate)) &&*/
      options) {
    menuOptions.clear();
    ...
    auto buttonWidget = std::dynamic_pointer_cast<idMenuWidget_Button>(
        options->GetChildByIndex(index));
    if (buttonWidget) {
...
void idMenuScreen_Shell_PartyLobby::ShowScreen() {
  ...
  if (BindSprite(root.get())) {
    ...
    NULL) { waitTime->SetVisible(false);
    }*/
  }

  if (session->GetPartyLobbyBase().IsHost()) {
    /*idMatchParameters matchParameters =
    session->GetPartyLobbyBase().GetMatchParms(); if (net_inviteOnly.GetBool())
    ...
    matchParameters.numSlots =
    session->GetTitleStorageInt("MAX_PLAYERS_ALLOWED", 4);
...
========================
*/
void idMenuScreen_Shell_PartyLobby::HideScreen() { idMenuScreen::HideScreen(); }
...
========================
*/
bool idMenuScreen_Shell_PartyLobby::HandleAction(idWidgetAction& action,
    ...
    case widgetAction_t::WIDGET_ACTION_COMMAND: {
      ...
      switch (static_cast<partyLobbyCmds_t>(parms[0]->ToInteger())) {
        case partyLobbyCmds_t::PARTY_CMD_FIND: {
          idMatchParameters matchParameters =
              idMatchParameters(session->GetPartyLobbyBase().GetMatchParms());
          matchParameters.gameMap = GAME_MAP_RANDOM;

          // session->UpdatePartyParms(matchParameters);

          // Update flags for game lobby.
          matchParameters.matchFlags =
              DefaultPartyFlags | DefaultPublicGameFlags;
        ...
        case partyLobbyCmds_t::PARTY_CMD_CREATE: {
          ...
          // const bool isInviteOnly =
          // MatchTypeInviteOnly(matchParameters.matchFlags);

          matchParameters.matchFlags =
              DefaultPartyFlags | DefaultPrivateGameFlags;

          // int mode = idMath::ClampInt(-1, GAME_COUNT - 1,
          // si_mode.GetInteger());
...
void idMenuScreen_Shell_PartyLobby::UpdateLobby() {
  ...
  if (lobby) {
    std::shared_ptr<idMenuHandler_Shell> mgr =
    ...
    if (lobby->GetNumEntries() > 0 &&
        lobby->GetFocusIndex() >= lobby->GetNumEntries()) {
  ...
  if (session->GetState() == idSession::sessionState_t::PARTY_LOBBY) {
    ...
    /*std::shared_ptr<idSWFTextInstance> privacy =
    GetSprite()->GetScriptObject()->GetNestedText("matchInfo", "txtPrivacy"); if
    (privacy) { if (isPeer) { privacy->SetText("");
            }
            else {

                    idMatchParameters matchParameters =
    session->GetPartyLobbyBase().GetMatchParms(); int bitSet =
    (matchParameters.matchFlags & MATCH_INVITE_ONLY); bool privacySet = (bitSet
    != 0 ? true : false); if (privacySet) {
