...
void idMenuWidget_LobbyList::Update() noexcept {
  ...
  for (int i = 0; i < headings.size(); ++i) {
    std::shared_ptr<idSWFTextInstance> txtHeading =
        GetSprite()->GetScriptObject()->GetNestedText(
            va("heading%d", i).c_str());
    if (txtHeading) {
      txtHeading->SetText(headings[i]);
    }
  }

  for (int optionIndex = 0; optionIndex < GetNumVisibleOptions();
       ++optionIndex) {
    bool shown = false;
...
========================
*/
bool idMenuWidget_LobbyList::PrepareListElement(idMenuWidget* widget,
                                                const int childIndex) {
  idMenuWidget_LobbyButton* button =
      dynamic_cast<idMenuWidget_LobbyButton*>(widget);
  if (button == nullptr) {
    return false;
  }
