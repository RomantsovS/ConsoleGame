...
========================
*/
void idMenuScreen_Shell_GameLobby::Initialize(std::shared_ptr<idMenuHandler> data) noexcept {
...
========================
*/
void idMenuScreen_Shell_GameLobby::HideScreen() {
...
bool idMenuScreen_Shell_GameLobby::HandleAction(idWidgetAction& action, const idWidgetEvent& event, idMenuWidget* widget, bool forceHandled) {
	...
	if (spMenuData->ActiveScreen() != static_cast<int>(shellAreas_t::SHELL_AREA_GAME_LOBBY)) {
		return false;
	}
	...
	case widgetAction_t::WIDGET_ACTION_COMMAND: {
		...
		case gameLobbyCmd_t::GAME_CMD_START: {
			std::shared_ptr<idMenuHandler_Shell> handler = std::dynamic_pointer_cast<idMenuHandler_Shell>(spMenuData);
			if (handler) {
...
void idMenuScreen_Shell_GameLobby::UpdateLobby() {
	...
	if (spMenuData && spMenuData->ActiveScreen() != static_cast<int>(shellAreas_t::SHELL_AREA_GAME_LOBBY)) {
		return;
	}
	...
	if (session->GetState() == idSession::sessionState_t::GAME_LOBBY) {
		...
		std::shared_ptr<idSWFTextInstance> mapName = GetSprite()->GetScriptObject()->GetNestedText("matchInfo", "txtMapName");
		...
		if (mapName) {
			...
			std::string name = maps[idMath::ClampInt(0, maps.size() - 1, matchParameters.gameMap)].mapName;
