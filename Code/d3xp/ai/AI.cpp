...
=====================
*/
void idAI::Killed(idEntity* inflictor, idEntity* attacker, int damage,
...
bool AISimple::Collide(const trace_t& collision,
  ...
  if (other) {
    if (collision.c.entityNum == ENTITYNUM_WORLD ||
        other->IsType(AISimple::Type) || other->IsType(idStaticEntity::Type) ||
        other->IsType(idProjectile::Type)) {
      Vector2 vel = GetPhysics()->GetLinearVelocity();
      GetPhysics()->SetLinearVelocity(-vel);
...
void AISimple::Killed(idEntity* inflictor, idEntity* attacker, int damage,
                      const Vector2& dir) noexcept {
