...
class idGame {
 public:
  ...
  virtual void InitFromNewMap(const std::string& mapName,
  ...
  // virtual void ProcessReliableMessage(int clientNum, int type, const
  // idBitMsg& msg) = 0;

  virtual void SetInterpolation(const float fraction, const int serverGameMS,
                                const int ssStartTime, const int ssEndTime) = 0;
  ...
  virtual void ClientRunFrame(idUserCmdMgr& cmdMgr, bool lastPredictFrame,
                              gameReturn_t& ret) = 0;
...
class idGameEdit {
 public:
  ...
  virtual void ParseSpawnArgsToRenderEntity(gsl::not_null<const idDict*> args,
