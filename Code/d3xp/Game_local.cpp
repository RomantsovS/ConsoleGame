...
std::shared_ptr<idRenderWorld>
    gameRenderWorld;  // all drawing is done to this world
...
void idGameLocal::Init() {
  ...
  cmdSystem->AddCommand("addrandompoints", AddRandomPoints, CMD_FL_GAME,
                        "addes random point(s)");
  ...
  // weapons We want to run this once after the base doom config file has run so
  // we can have the correct xp binds
...
void idGameLocal::InitFromNewMap(const std::string& mapName,
...
void idGameLocal::MapShutdown() {
  ...
  collisionModelManager
      ->FreeMap();  // Fixes an issue where when maps were reloaded the
                    // materials wouldn't get their surfaceFlags re-set.  Now we
                    // free the map collision model forcing materials to be
                    // reparsed.
...
========================
*/
int idGameLocal::GetLocalClientNum() const noexcept { return 0; }
...
void idGameLocal::CacheDictionaryMedia(gsl::not_null<const idDict*> dict) {
  ...
  if (kv) {
    if (!kv->second.empty()) {
      ...
      if (!declManager->FindType(declType_t::DECL_MODELDEF, kv->second,
                                 false)) {
...
void idGameLocal::SpawnPlayer(int clientNum) {
  ...
  // precache the player
  args.Set("classname", gameLocal.world->spawnArgs.GetString(
                            "def_player", "player_doommarine"));

  // args.Set("model", "pixel");
  args.Set("color", std::to_string(static_cast<Screen::color_type>(
                        gameLocal.GetRandomColor())));

  std::shared_ptr<idEntity> ent;
  if (!SpawnEntityDef(args, &ent) || clientNum >= MAX_GENTITIES ||
      !entities[clientNum]) {
    // Error("Failed to spawn player as '%s'",
  ...
  if (ent && !ent->IsType(idPlayer::Type)) {
    Error(
        ...
        args.GetString("classname").c_str(), ent->GetClassname().c_str());
  }
}
...
idPlayer* idGameLocal::GetLocalPlayer() const {
  ...
  if (!entities.at(GetLocalClientNum()) ||
      !entities.at(GetLocalClientNum())->IsType(idPlayer::Type)) {
...
========================
*/
void idGameLocal::SetInterpolation(const float fraction, const int serverGameMS,
                                   const int ssStartTime, const int ssEndTime) {
...
void idGameLocal::RunFrame(idUserCmdMgr& cmdMgr, gameReturn_t& ret) {
  ...
  if (!common->IsMultiplayer() && g_stopTime.GetBool()) {
    ...
    static int game_add_point_delay =
...
====================
*/
void idGameLocal::RunSingleUserCmd(usercmd_t& cmd,
                                   gsl::not_null<idPlayer*> player) {
  player->HandleUserCmds(cmd);

  if (!common->IsMultiplayer() || common->IsServer()) {
    player->Think();

    // Keep track of the client time of the usercmd we just ran. We will send
    // this back to clients in a snapshot so they know when they can stop
    // predicting certain things.
    // usercmdLastClientMilliseconds[player.GetEntityNumber()] =
    // cmd.clientGameMilliseconds;
  } else {
    player->ClientThink(
        /*netInterpolationInfo.serverGameMs, netInterpolationInfo.pct, true*/);
  }
}
...
Runs a Think or ClientThink for each usercmd, but leaves a few cmds in the
buffer so that we have something to process while we wait for more from the
network.
====================
*/
void idGameLocal::RunAllUserCmdsForPlayer(idUserCmdMgr& cmdMgr,
  ...
  gsl::not_null<idPlayer*> player =
      dynamic_cast<idPlayer*>(entities.at(playerNumber).get());

  // Only run a single userCmd each game frame for local players, otherwise when
  // we are running < 60fps things like footstep sounds may get started right on
  // top of each other instead of spread out in time.
  if (player->IsLocallyControlled()) {
    if (cmdMgr.HasUserCmdForPlayer(playerNumber)) {
      ...
      cmdMgr.GetNumUnreadFrames(playerNumber));
      }*/
      RunSingleUserCmd(cmdMgr.GetWritableUserCmdForPlayer(playerNumber),
                       player);
    } else {
...
bool idGameLocal::Draw(int clientNum) {
  if (common->IsMultiplayer() &&
      session->GetState() == idSession::sessionState_t::INGAME) {
...
void idGameLocal::RunDebugInfo() {
  ...
  for (auto ent = spawnedEntities.Prev(); ent != NULL;
       ent = ent->spawnNode.Prev())
  ...
  if (g_showEntityInfo.GetBool()) {
    ...
    for (auto ent = spawnedEntities.Prev(); ent != NULL;
      ...
      if (ent->IsActive()) {
        ...
        auto str =
            string_format("%10s p[%5.2f %5.2f] v[%6.2f %6.2f] rest %d",
                          ent->GetName().c_str(), phys->GetOrigin().x,
  ...
  /*if (g_showActiveEntities.GetBool()) {
      for (ent = activeEntities.Next(); ent != NULL; ent =
  ent->activeNode.Next()) { idBounds	b = ent->GetPhysics()->GetBounds(); if
  (b.GetVolume() <= 0) { b[0][0] = b[0][1] = b[0][2] = -8; b[1][0] = b[1][1] =
  b[1][2] = 8;
          }
          if (ent->fl.isDormant) {
  ...
  if (g_showCollisionModels.GetBool()) {
    clip.DrawClipModels(player->GetPhysics()->GetOrigin(),
                        g_maxShowDistance.GetFloat(), nullptr);
  }
  ...
  SE_ENTER_OBSTACLE | SE_BLOCKED | SE_ENTER_LEDGE_AREA, path);
          }
      }
  }
...
void idGameLocal::PrintSpawnedEntities() {
  for (auto ent = spawnedEntities.Next(); ent != nullptr;
       ent = ent->spawnNode.Next()) {
    Printf("ent %s pos [%6.3f %6.3f] vel [%6.3f %6.3f] rest %d",
           ent->GetName().c_str(), ent->GetPhysics()->GetOrigin().x,
           ent->GetPhysics()->GetOrigin().y,
           ent->GetPhysics()->GetLinearVelocity().x,
           ent->GetPhysics()->GetLinearVelocity().y,
...
========================
*/
int idGameLocal::GetServerGameTimeMs() const { return serverTime; }
...
==============
*/
gameState_t idGameLocal::GameState() const noexcept { return gamestate; }
...
========================
*/
bool idGameLocal::InhibitControls() { return Shell_IsActive(); }
...
void idGameLocal::SpawnMapEntities() {
  ...
  tr.screen->setBackGroundPixel(
      {tr.screen->getBackgroundPixel().value,
...
void idGameLocal::AddRandomPoint() {
  ...
  if (gameLocal.GetRandomValue(0.0f, 1.0f) > 0) speed = -speed;

  args.Set("linearVelocity", (rand > 0) ? Vector2(speed, 0).ToString()
                                        : Vector2(0, speed).ToString());
...
================
*/
std::shared_ptr<idEntity> idGameLocal::SpawnEntityType(
    const idTypeInfo& classdef, const idDict* args,
  ...
  if (!classdef.IsType(idEntity::Type)) {
    Error("Attempted to spawn non-entity class '%s'",
          classdef.classname.c_str());
  }
...
bool idGameLocal::SpawnEntityDef(const idDict& args,
  ...
  if (!spawn.empty()) {
    ...
    if (!cls) {
      Warning("Could not spawn '%s'. Class '%s' not found%s.",
              classname.c_str(), spawn.c_str(), error.c_str());
      return false;
    }
...
================
*/
const std::shared_ptr<idDeclEntityDef> idGameLocal::FindEntityDef(
    const std::string& name, bool makeDefault) const {
  const std::shared_ptr<idDecl> decl =
      declManager->FindType(declType_t::DECL_ENTITYDEF, name, makeDefault);
...
================
*/
const idDict* idGameLocal::FindEntityDefDict(const std::string& name,
                                             bool makeDefault) const {
  const std::shared_ptr<idDeclEntityDef> decl =
      FindEntityDef(name, makeDefault);
...
void idGameLocal::RegisterEntity(std::shared_ptr<idEntity> ent,
                                 int forceSpawnId,
  ...
  if ((ent->entityNumber != ENTITYNUM_NONE) &&
    ...
    if (g_debugSpawn.GetBool())
      DPrintf("Unregister ent %d %s %s\n", ent->entityNumber,
              ent->GetClassname().c_str(), ent->name.c_str());

    entities[ent->entityNumber] = nullptr;
    spawnIds[ent->entityNumber] = -1;

    int freeListType =
        (ent->entityNumber >= ENTITYNUM_FIRST_NON_REPLICATED) ? 1 : 0;

    if (ent->entityNumber >= MAX_CLIENTS &&
        ent->entityNumber < firstFreeEntityIndex[freeListType]) {
...
================
*/
int idGameLocal::EntitiesWithinRadius(
    const Vector2 org, float radius,
    std::vector<std::shared_ptr<idEntity>>& entityList, int maxCount) const {
  std::shared_ptr<idEntity> ent;
  idBounds bo(org);
