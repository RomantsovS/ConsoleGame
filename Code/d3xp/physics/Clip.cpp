...
void idClipModel::Link(idClip& clp, idEntity* ent, int newId,
                       const Vector2& newOrigin, int renderModelHandle) {
  this->entity = ent;
  this->id = newId;
...
int idClipModel::AllocTraceModel(const idTraceModel& trm,
  ...
  if (persistantThroughSaves) {
    ...
    if (iter != traceModelHash.end() &&
        *traceModelCache[iter->second]->trm == trm) {
    ...
    if (iter != traceModelHash_Unsaved.end() &&
        *traceModelCache_Unsaved[iter->second]->trm == trm) {
      traceModelCache[iter->second]->refCount++;
    }
  }
...
void idClipModel::FreeTraceModel(int traceModelIndex) {
  ...
  if (traceModelIndex & TRACE_MODEL_SAVED) {
    if (realTraceModelIndex < 0 ||
        realTraceModelIndex >= traceModelCache.size() ||
        traceModelCache[realTraceModelIndex]->refCount <= 0) {
      gameLocal.Warning(
    ...
    if (realTraceModelIndex < 0 ||
        realTraceModelIndex >= traceModelCache_Unsaved.size() ||
        traceModelCache_Unsaved[realTraceModelIndex]->refCount <= 0) {
      gameLocal.Warning(
          "idClipModel::FreeTraceModel: tried to free uncached trace model");
      return;
    }
...
int idClipModel::GetTraceModelHashKey(const idTraceModel& trm) noexcept {
  ...
  return (trm.type << 8) ^ (trm.numVerts << 4) ^
...
bool idClip::Translation(trace_t& results, const Vector2& start,
                         const Vector2& end, const idClipModel* mdl,
  ...
  if (!passEntity || passEntity->entityNumber != ENTITYNUM_WORLD) {
    ...
    collisionModelManager->Translation(&results, start, end, trm, contentMask,
                                       0, vec2_origin);
    results.c.entityNum =
  ...
  if (!trm) {
    ...
    traceBounds.FromBoundsTranslation(trm->bounds, start,
                                      results.endpos - start);
...
bool idClip::Motion(trace_t& results, const Vector2& start, const Vector2& end,
                    const idClipModel* mdl, int contentMask,
...
int idClip::Contacts(contactInfo_t* contacts, const int maxContacts,
                     const Vector2& start, const Vector2& dir,
                     const float depth, const idClipModel* mdl, int contentMask,
                     const idEntity* passEntity) {
  ...
  if (!passEntity || passEntity->entityNumber != ENTITYNUM_WORLD) {
    // test world
    idClip::numContacts++;
    numContacts = collisionModelManager->Contacts(contacts, maxContacts, start,
                                                  dir, depth, trm, contentMask,
                                                  0, vec2_origin);
  } else {
    numContacts = 0;
  }
  ...
  for (i = 0; i < num; i++) {
    ...
    n = collisionModelManager->Contacts(
        ...
        trm, contentMask, touch->Handle(), touch->origin);
...
int idClip::ClipModelsTouchingBounds(const idBounds& bounds, int contentMask,
...
std::shared_ptr<clipSector_t> idClip::CreateClipSectors_r(
    const int depth, const idBounds& bounds, Vector2& maxSector) {
  int i;
  Vector2 size;
  idBounds front, back;
...
void idClip::ClipModelsTouchingBounds_r(const clipSector_t* node,
    ...
    if (auto entSp = cm->entity) {
      ...
      if (entSp == passEntity) {
        clipModelList[i] = nullptr;  // don't clip against the pass entity
              ...
              clipModelList[i] = nullptr;			// missiles
      don't clip with their owner } else if (auto ownerSp = cm->owner) { if
      (ownerSp == passEntity) { clipModelList[i] = nullptr;		// don't
      clip against own missiles
              }
              else if (ownerSp == passOwner) {
                      clipModelList[i] = nullptr;		// don't clip
      against other missiles from same owner
              }
      }*/
    }
  }
...
============
*/
void idClip::DrawClipModels(const Vector2& eye, const float radius,
                            const idEntity* passEntity) {
  int i, num;
  idBounds bounds;
  std::array<idClipModel*, MAX_GENTITIES> clipModelList;
  ...
  num = idClip::ClipModelsTouchingBounds(bounds, -1, clipModelList.data(),
                                         MAX_GENTITIES);
  ...
  for (i = 0; i < num; i++) {
    ...
    clipModel->GetOrigin(), clipModel->GetAxis(), eye, radius);
    }*/
  }
}
...
bool idClip::DrawClipSectors_r(const clipSector_t* node,
