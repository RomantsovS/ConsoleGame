...
class idClipModel : public std::enable_shared_from_this<idClipModel> {
 ...
 private:
  ...
  static int AllocTraceModel(const idTraceModel& trm,
                             bool persistantThroughSaves = true);
...
inline int idClipModel::GetContents() const { return contents; }
...
inline idEntity* idClipModel::GetEntity() const noexcept { return entity; }

inline idEntity* idClipModel::GetOwner() const noexcept { return owner; }
...
inline const Vector2& idClipModel::GetOrigin() const noexcept { return origin; }
...
inline void idClipModel::Enable() noexcept { enabled = true; }

inline void idClipModel::Disable() noexcept { enabled = false; }

inline void idClipModel::SetId(int newId) noexcept { id = newId; }

inline int idClipModel::GetId() const noexcept { return id; }
...
class idClip {
 ...
 public:
  ...
  int ClipModelsTouchingBounds(const idBounds& bounds, int contentMask,
                               idClipModel** clipModelList, int maxCount) const;
  ...
  void DrawClipModels(const Vector2& eye, const float radius,
                      const idEntity* passEntity);
 ...
 private:
  std::shared_ptr<clipSector_t> CreateClipSectors_r(const int depth,
                                                    const idBounds& bounds,
                                                    Vector2& maxSector);
  void ClipModelsTouchingBounds_r(const clipSector_t* node,
