...
int Physics_PlayerChain::AddBody(const std::shared_ptr<idAFBody>& body) {
  ...
  if (body->clipModel == nullptr) {
    gameLocal.Error(
        "Physics_PlayerChain::AddBody: body '%s' has no clip model.",
        body->name.c_str());
    return 0;
  }

  if (std::find(bodies.begin(), bodies.end(), body) != bodies.end()) {
    gameLocal.Error("Physics_PlayerChain::AddBody: body '%s' added twice.",
                    body->name.c_str());
  }

  if (GetBody(body->name)) {
    gameLocal.Error(
        "Physics_PlayerChain::AddBody: a body with the name '%s' already "
        "exists.",
        body->name.c_str());
  }
...
================
*/
int Physics_PlayerChain::GetBodyId(
    const std::shared_ptr<idAFBody>& body) const {
...
================
*/
std::shared_ptr<idAFBody> Physics_PlayerChain::GetBody(
...
bool Physics_PlayerChain::Evaluate(int timeStepMSec, int endTimeMSec) noexcept {
  ...
  if (IsOutsideWorld()) {
    gameLocal.Warning(
        "Physics_PlayerChain outside world bounds for entity '%s' type '%s' at "
        "(%s)",
...
================
*/
bool Physics_PlayerChain::CollisionImpulse(float timeStep, idAFBody* body,
...
================
*/
std::shared_ptr<idEntity> Physics_PlayerChain::SetupCollisionForBody(
    idAFBody* body) const noexcept {
  size_t i;
...
================
*/
bool Physics_PlayerChain::IsAtRest() const noexcept { return false; }
...
================
*/
void Physics_PlayerChain::Translate(const Vector2& translation,
                                    int id) noexcept {
...
================
*/
void Physics_PlayerChain::SetLinearVelocity(const Vector2& newLinearVelocity,
                                            int id) noexcept {
  if (id < 0 || id >= static_cast<int>(bodies.size())) {
    return;
  }
