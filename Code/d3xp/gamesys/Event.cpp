...
idEventDef::idEventDef(const std::string& command,
                       const std::string& formatspec, char returnType) {
  // idassert(command);
  ...
  for (int i = 0; i < eventnum; i++) {
    ...
    if (command == ev->name) {
      ...
      /*if (ev->returnType != returnType) {
              eventError = true;
              sprintf(eventErrorMsg, "idEvent '%s' defined twice with same name
      but differing return types ('%c'!='%c').", command, returnType,
      ev->returnType); return;
      }*/
...
================
*/
int idEventDef::NumEventCommands() noexcept { return numEventDefs; }
...
================
*/
std::shared_ptr<idEvent> idEvent::Alloc(gsl::not_null<const idEventDef*> evdef,
  ...
  for (auto i = 0; i < numargs; i++) {
    ...
    switch (format[i]) {
      ...
      case D_EVENT_VECTOR:
        if (arg->value) {
          *reinterpret_cast<Vector2*>(dataPtr) =
              *reinterpret_cast<const Vector2*>(arg->value);
        }
        break;
      ...
      /*case D_EVENT_ENTITY:
      case D_EVENT_ENTITY_NULL:
              *reinterpret_cast<idEntity>(dataPtr) =
      reinterpret_cast<idEntity*>(arg->value); break;
              */
      /*case D_EVENT_TRACE:
              if (arg->value) {
         ...
         *reinterpret_cast<const trace_t*>(arg->value);
                      ...
                      // save off the material as a string since the pointer
         won't be valid in save games.
                      // since we save off the entire trace_t structure, if the
         material is NULL here,
                      // it will be NULL when we process it, so we don't need to
         save off anything in that case. if (reinterpret_cast<const
         trace_t*>(arg->value)->c.material) { materialName =
         reinterpret_cast<const trace_t*>(arg->value)->c.material->GetName();
                              idStr::Copynz(reinterpret_cast<char*>(dataPtr +
         sizeof(bool) + sizeof(trace_t)), materialName, MAX_STRING_LEN);
                      }
              }
              else {
...
void idEvent::Free(bool setOwner) {
  ...
  if (setOwner) eventNode.SetOwner(shared_from_this());
...
================
*/
void idEvent::Schedule(idClass* obj, const idTypeInfo* type,
                       int time) noexcept {
  ...
  while (!EventQueue.IsListEmpty()) {
    ...
    for (auto i = 0; i < numargs; i++) {
      ...
      switch (formatspec[i]) {
        ...
        case D_EVENT_VECTOR:
          *reinterpret_cast<Vector2**>(&args[i]) =
              reinterpret_cast<Vector2*>(&data[offset]);
          break;
        ...
        case D_EVENT_ENTITY_NULL:
          ...
          // reinterpret_cast<idEntityPtr<idEntity>*>(&data[offset])->GetEntity();
          break;

        /*case D_EVENT_TRACE:
                tracePtr = reinterpret_cast<trace_t**>(&args[i]);
                if (*reinterpret_cast<bool*>(&data[offset])) {
                        *tracePtr = reinterpret_cast<trace_t*>(&data[offset +
           sizeof(bool)]);

                        if ((*tracePtr)->c.material != NULL) {
                                // look up the material name to get the material
           pointer materialName = reinterpret_cast<const char*>(&data[offset +
           sizeof(bool) + sizeof(trace_t)]);
                                (*tracePtr)->c.material =
