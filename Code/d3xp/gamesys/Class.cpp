...
idTypeInfo::idTypeInfo(const std::string& classname,
...
idTypeInfo::~idTypeInfo() { Shutdown(); }
...
================
*/
idClass::~idClass() { idEvent::CancelEvents(this); }
...
void idClass::Init() {
  ...
  num = 0;
  for (c = classHierarchy.GetNext(); c != nullptr;
       c = c->node.GetNext(), num++) {
    c->typeNum = num;
    c->lastChild += num;
  }
...
bool idClass::PostEventArgs(const idEventDef* ev, int time, int numargs, ...) {
  ...
  // If this is an entity with skipReplication, we want to process the event
  // normally even on clients.
  ...
  if (common->IsClient() && isReplicated &&
      (gameLocal.GameState() !=
       GAMESTATE_STARTUP) /* && !IsType(idThread::Type)*/) {
    return true;
  }
...
bool idClass::ProcessEventArgPtr(const idEventDef* ev, int* data) {
  ...
  IsType(idEntity::Type)) { const idEntity* ent =
  *reinterpret_cast<idEntity**>(data); gameLocal.Printf("%d: '%s' activated by
  '%s'\n", gameLocal.framenum, static_cast<idEntity*>(this)->GetName(), ent ?
  ent->GetName() : "NULL");
  }*/
  ...
  */
  ...
  switch (ev->GetFormatspecIndex()) {
    ...
    default:
      gameLocal.Warning("Invalid formatspec on event '%s'",
                        ev->GetName().c_str());
      break;
  }
