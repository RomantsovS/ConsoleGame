...
class idGameThread : public idSysThread {
 public:
  idGameThread() : userCmdMgr(nullptr), ret(), numGameFrames(), isClient() {}

  // the gameReturn_t is from the previous frame, the
  // new frame will be running in parallel on exit
  gameReturn_t RunGameAndDraw(int numGameFrames, idUserCmdMgr& userCmdMgr_,
...
class idCommonLocal : public idCommon {
 ...
 private:
  ...
  // This is ultimately controlled by net_maxBufferedSnapshots by running double
  // speed, but this is the hard max before seeing visual popping
 ...
 private:
  ...
  void InterpolateSnapshot(netTimes_t& prev, netTimes_t& next, float fraction,
                           bool predict);
