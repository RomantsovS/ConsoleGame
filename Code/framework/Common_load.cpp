...
===============
*/
void idCommonLocal::StartNewGame(const std::string& mapName, bool devmap,
                                 int gameMode) {
  if (!session->IsLocalUserRegistered()) {
    // For development make sure a controller is registered
    // Can't just register the local user because it will be removed because of
    // it's persistent state
...
Performs the initialization of a game based on session match parameters, used
for both single player and multiplayer, but not for renderDemos, which don't
create a game at all. Exits with mapSpawned = true
===============
*/
void idCommonLocal::ExecuteMapChange() {
  ...
  const idMatchParameters& matchParameters =
      session->GetActingGameStateLobbyBase().GetMatchParms();
  ...
  /*if (mapSpawnData.savegameFile) {
          if (!game->InitFromSaveGame(fullMapName, renderWorld, soundWorld,
  mapSpawnData.savegameFile, mapSpawnData.stringTableFile,
  mapSpawnData.savegameVersion)) {
                  // If the loadgame failed, end the session, which will force
  us to go back to the main menu session->QuitMatchToTitle();
          }
  }
  else
  {*/
  ...
  // if (!mapSpawnData.savegameFile)
  {
    // run a single frame to catch any resources that are referenced by events
    // posted in spawn
...
void idCommonLocal::InitCommands() {
  ...
  cmdSystem->AddCommand("restartMap", restartMap_f, CMD_FL_GAME,
                        "restarts the current map");
  cmdSystem->AddCommand("devmap", devmap_f, CMD_FL_GAME,
