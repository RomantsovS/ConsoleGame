...
idCVar net_snapRate("net_snapRate", "100", CVAR_SYSTEM | CVAR_INTEGER,
                    "How many milliseconds between sending snapshots");

idCVar net_debug_snapShotTime("net_debug_snapShotTime", "1",
                              CVAR_BOOL | CVAR_ARCHIVE, "");
...
// Factor at which we catch speed up interpolation if we fall behind our optimal
// interpolation window
//  -This is a static factor. We may experiment with a dynamic one that would be
//  faster the farther you are from the ideal window
idCVar net_interpolationCatchupRate(
    "net_interpolationCatchupRate", "1.3", CVAR_FLOAT,
    "Scale interpolationg rate when we fall behind");
idCVar net_interpolationFallbackRate(
    "net_interpolationFallbackRate", "0.95", CVAR_FLOAT,
    "Scale interpolationg rate when we fall behind");
idCVar net_interpolationBaseRate(
    "net_interpolationBaseRate", "1.0", CVAR_FLOAT,
    "Scale interpolationg rate when we fall behind");

// Extrapolation is now disabled
idCVar net_maxExtrapolationInMS(
    "net_maxExtrapolationInMS", "0", CVAR_INTEGER,
    "Max time in MS that extrapolation is allowed to occur.");
...
void idCommonLocal::SendSnapshots() {
  ...
  nextSnapshotSendTime =
      MSEC_ALIGN_TO_FRAME(currentTime + net_snapRate.GetInteger());
}
...
void idCommonLocal::NetReceiveSnapshot(class idSnapShot& ss) {
  ...
  //  If we are about to overwrite the oldest snap, then force a read, which
  //  will cause a pop on screen, but we have to do this.
  if (writeSnapshotIndex - readSnapshotIndex >= RECEIVE_SNAPSHOT_BUFFER_SIZE) {
    ...
    idassert(writeSnapshotIndex % RECEIVE_SNAPSHOT_BUFFER_SIZE ==
             readSnapshotIndex % RECEIVE_SNAPSHOT_BUFFER_SIZE);
    ProcessNextSnapshot();
  }
...
void idCommonLocal::ProcessSnapshot(idSnapShot& ss) {
  ...
  if (net_debug_snapShotTime.GetBool()) {
    idLib::Printf(
        "^2ProcessSnapshot. delta serverTime: %d  delta localTime: %d \n",
  ...
  // Set server game time here so that it accurately reflects the time when this
  // frame was saved out, in case any serialize function needs it.
...
void idCommonLocal::ProcessNextSnapshot() {
  ...
  ProcessSnapshot(
      receivedSnaps[readSnapshotIndex % RECEIVE_SNAPSHOT_BUFFER_SIZE]);
  readSnapshotIndex++;
}
...
totalBufferedTime - total amount of snapshot time (includng what we've already
past in current interpolate) totalRecvTime - total real time (sys_milliseconds)
all of totalBufferedTime was received over
========================
*/
int idCommonLocal::CalcSnapTimeBuffered(int& totalBufferedTime,
                                        int& totalRecvTime) {
  ...
  // receivedSnaps[readSnapshotIndex % RECEIVE_SNAPSHOT_BUFFER_SIZE] = next
  // buffered snapshot we haven't processed yet (might not exist)
  for (int i = readSnapshotIndex; i < writeSnapshotIndex; i++) {
    int buffTime = receivedSnaps[i % RECEIVE_SNAPSHOT_BUFFER_SIZE].GetTime();
    int recvTime =
        receivedSnaps[i % RECEIVE_SNAPSHOT_BUFFER_SIZE].GetRecvTime();
  ...
  totalRecvTime =
      static_cast<float>(initialBaseTicksPerSec) *
      static_cast<float>(totalRecvTime / 1000.0f);  // convert realMS to gameMS

  // remove time we've already interpolated over
  int timeLeft = totalBufferedTime - std::min(snapRate, snapCurrentTime);

  // idLib::Printf( "CalcSnapTimeBuffered. timeLeft: %d totalRecvTime: %d,
...
========================
*/
void idCommonLocal::InterpolateSnapshot(netTimes_t& prev, netTimes_t& next,
                                        float fraction, bool predict) {
  int serverTime = Lerp(prev.serverTime, next.serverTime, fraction);

  Game()->SetServerGameTimeMs(serverTime);  // Set the global server time to the
                                            // interpolated time of the server
  Game()->SetInterpolation(fraction, serverTime, prev.serverTime,
                           next.serverTime);
...
void idCommonLocal::RunNetworkSnapshotFrame() {
  ...
  /*for (int i = 0; i < reliableQueue.size(); i++) {
          game->ProcessReliableMessage(reliableQueue[i].client,
  reliableQueue[i].type, idBitMsg(reliableQueue[i].data,
  reliableQueue[i].dataSize)); Mem_Free(reliableQueue[i].data);
  }
  reliableQueue.clear();*/
  ...
  if (snapPrevious.serverTime >= 0) {
    ...
    do {
      // If we are extrapolating and have fresher snapshots, then use the
      // freshest one
      while ((snapCurrentTime >= snapRate || com_forceLatestSnap.GetBool()) &&
             readSnapshotIndex < writeSnapshotIndex) {
        snapCurrentTime -= snapRate;
        ProcessNextSnapshot();
      }

      // Clamp the current time so that it doesn't fall outside of our
      // extrapolation bounds
      // snapCurrentTime = idMath::ClampInt(0, snapRate +
      //	std::min(static_cast<int>(snapRate),
      // net_maxExtrapolationInMS.GetInteger()), snapCurrentTime);
      ...
      // if (!IsValid(fraction)) {
      //	idLib::Warning("Interpolation Fraction invalid: snapCurrentTime
      //%d / snapRate %d", 		(int)snapCurrentTime, (int)snapRate);
      // fraction = 0.0f;
      // }
      ...
      // snapTimeBuffered = CalcSnapTimeBuffered(totalBufferedTime,
      // totalRecvTime); effectiveSnapRate = static_cast<float>
      // (totalBufferedTime) / static_cast<float> (totalRecvTime);

      // if (net_minBufferedSnapPCT_Static.GetFloat() > 0.0f) {
      //	optimalPCTBuffer =
      // session->GetTitleStorageFloat("net_minBufferedSnapPCT_Static",
      // net_minBufferedSnapPCT_Static.GetFloat());
      // }

      //// Calculate optimal amount of buffered time we want
      // if (net_optimalDynamic.GetBool()) {
      //	optimalTimeBuffered = idMath::ClampInt(0,
      // net_maxBufferedSnapMS.GetInteger(), snapRate * optimalPCTBuffer);
      //	optimalTimeBufferedWindow = snapRate *
      // net_minBufferedSnapWinPCT_Static.GetFloat();
      // }
      // else {
      ...
      //	else {
      //		snapRateScale =
      // net_interpolationFallbackRate.GetFloat();
      //		// When we interpolate past our cushion of buffered
      // snapshot, we want to slow smoothly slow the
      //		// rate of interpolation. frac will go from 1.0 to 0.0
      //(if snapshots stop coming in). 		float startSlowdown =
      //(net_interpolationSlowdownStart.GetFloat() * optimalTimeBuffered);
      // if (startSlowdown > 0 && snapTimeBuffered < startSlowdown) {
      // float frac = idMath::ClampFloat(0.0f, 1.0f, snapTimeBuffered /
      // startSlowdown); 			if
      //(!IsValid(frac)) { 				frac = 0.0f;
      //			}
      ...
      //}
      // else if (snapTimeBuffered > optimalTimeBuffered +
      // optimalTimeBufferedWindow) {
      //	// Go faster
      //	snapRateScale = net_interpolationCatchupRate.GetFloat();

      //}

      // float delta_interpolate = (float)initialBaseTicksPerSec *
      // snapRateScale; if (net_effectiveSnapRateEnable.GetBool()) {

      //	float deltaFrameGameMS =
      // static_cast<float>(initialBaseTicksPerSec) *
      // static_cast<float>(deltaFrameTime / 1000.0f); 	delta_interpolate =
      //(deltaFrameGameMS * snapRateScale * effectiveSnapRate) +
      // snapCurrentResidual; 	if (!IsValid(delta_interpolate)) {
      //		delta_interpolate = 0.0f;
      //	}

      //	snapCurrentResidual = idMath::Frac(delta_interpolate); // fixme:
      // snapCurrentTime should just be a float, but would require changes in d4
      // too 	if (!IsValid(snapCurrentResidual)) {
      // snapCurrentResidual = 0.0f;
      //	}

      //	if (net_effectiveSnapRateDebug.GetBool()) {
      //		idLib::Printf("%d/%.2f snapRateScale: %.2f effectiveSR:
      //%.2f d.interp: %.2f snapTimeBuffered: %.2f res: %.2f\n", deltaFrameTime,
      // deltaFrameGameMS, snapRateScale, effectiveSnapRate, delta_interpolate,
      // snapTimeBuffered, snapCurrentResidual);
      //	}
      //}
      ...
      // snapCurrentTime += interpolate_interval;	// advance interpolation
      // time by the scaled interpolate_interval
      clientTimeResidual -=
          msec_interval;  // advance local client residual time (fixed step)
...
void idCommonLocal::ResetNetworkingState() {
  ...
  // Make sure our current snap state is cleared so state from last game doesn't
  // carry over into new game
  oldss.Clear();
