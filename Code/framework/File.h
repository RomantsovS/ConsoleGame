...
class idFile_Permanent : public idFile {
 ...
 public:
  idFile_Permanent(const std::filesystem::path& file_name,
...
class variable goes out of scope. Note that the pointer passed in to the
constructor can be for any type of File Stream that ultimately inherits from
idFile, and that this is not actually a SmartPointer, as it does not keep a
reference count.
================================================
*/
class idFileLocal {
 public:
  ...
  // Destructor that will destroy (close) the file when this wrapper class goes
  // out of scope.
  ~idFileLocal();
  ...
  operator idFile*() const noexcept { return file.get(); }

  // Member access operator for treating the wrapper as if it were the file,
  // itself.
  idFile* operator->() const noexcept { return file.get(); }
