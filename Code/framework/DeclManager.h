...
class idDecl {
 ...
 public:
  ...
  virtual std::string DefaultDefinition() const {
    return base.lock()->DefaultDefinition();
  }
...
class idDeclManager {
 public:
  ...
  virtual void RegisterDeclType(const std::string& typeName, declType_t type,
                                std::shared_ptr<idDecl> (*allocator)()) = 0;

  // Registers a new folder with decl files.
  virtual void RegisterDeclFolder(const std::string& folder,
  ...
  virtual const std::shared_ptr<idDecl> FindType(declType_t type,
                                                 std::string name,
  ...
  virtual const std::shared_ptr<idDecl> DeclByIndex(declType_t type, int index,
  ...
  virtual const std::shared_ptr<idMaterial> FindMaterial(
      const std::string& name, bool makeDefault = true) = 0;
};
