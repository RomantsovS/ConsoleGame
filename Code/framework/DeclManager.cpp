...
class idDeclLocal : public idDeclBase,
                    public std::enable_shared_from_this<idDeclLocal> {
 ...
 private:
  ...
  std::weak_ptr<idDeclFile>
      sourceFile;         // source file in which the decl was defined
...
class idDeclManagerLocal : public idDeclManager {
 ...
 public:
  idDeclManagerLocal() {
    linearLists.resize(static_cast<int>(declType_t::DECL_MAX_TYPES));
  }

  void Init() override;
  void Shutdown() override;
  void RegisterDeclType(const std::string& typeName, declType_t type,
                        std::shared_ptr<idDecl> (*allocator)()) override;
  void RegisterDeclFolder(const std::string& folder,
                          const std::string& extension,
                          declType_t defaultType) override;
  ...
  const std::shared_ptr<idDecl> FindType(declType_t type, std::string name,
                                         bool makeDefault = true) override;
  const std::shared_ptr<idDecl> DeclByIndex(declType_t type, int index,
                                            bool forceParse = true) override;

  void ListType(const idCmdArgs& args, declType_t type) override;
  void PrintType(const idCmdArgs& args, declType_t type) override;

  const std::shared_ptr<idMaterial> FindMaterial(
      const std::string& name, bool makeDefault = true) override;

 public:
  static void MakeNameCanonical(const std::string& name, std::string& result,
                                int maxLength) noexcept;
  std::shared_ptr<idDeclLocal> FindTypeWithoutParsing(declType_t type,
                                                      const std::string& name,
                                                      bool makeDefault = true);

  std::shared_ptr<idDeclType> GetDeclType(int type) const noexcept {
...
void idDeclManagerLocal::Init() {
  ...
  RegisterDeclType("material", declType_t::DECL_MATERIAL,
                   idDeclAllocator<idMaterial>);
  RegisterDeclType("entityDef", declType_t::DECL_ENTITYDEF,
                   idDeclAllocator<idDeclEntityDef>);
  RegisterDeclType("mapDef", declType_t::DECL_MAPDEF,
                   idDeclAllocator<idDeclEntityDef>);
  ...
  cmdSystem->AddCommand("listEntityDefs",
                        idListDecls_f<declType_t::DECL_ENTITYDEF>,
                        CMD_FL_SYSTEM, "lists entity defs");

  cmdSystem->AddCommand("printEntityDef",
...
===================
*/
void idDeclManagerLocal::RegisterDeclType(
    const std::string& typeName, declType_t type,
    std::shared_ptr<idDecl> (*allocator)()) {
  if (static_cast<size_t>(type) < declTypes.size() &&
      declTypes.at(static_cast<int>(type))) {
...
===================
*/
void idDeclManagerLocal::RegisterDeclFolder(const std::string& folder,
                                            const std::string& extension,
                                            declType_t defaultType) {
  size_t i, j;
  std::string fileName;
  ...
  for (i = 0; i < declFolders.size(); i++) {
    if (declFolders[i]->folder == folder &&
        declFolders[i]->extension == extension) {
      break;
    }
  }
  ...
  // scan for decl files
  std::shared_ptr<idFileList> fileList =
...
===================
*/
const std::string& idDeclManagerLocal::GetDeclNameFromType(
    declType_t type) const {
...
=================
*/
const std::shared_ptr<idDecl> idDeclManagerLocal::FindType(declType_t type,
                                                           std::string name,
                                                           bool makeDefault) {
  ...
  if (name.empty()) {
    name = "_emptyName";
    common->Warning("idDeclManager::FindType: empty %s name",
                    GetDeclType(static_cast<int>(type))->typeName.c_str());
  }

  std::shared_ptr<idDeclLocal> decl =
      FindTypeWithoutParsing(type, name, makeDefault);
  if (!decl) {
    return nullptr;
  }
...
===================
*/
const std::shared_ptr<idDecl> idDeclManagerLocal::DeclByIndex(declType_t type,
                                                              int index,
                                                              bool forceParse) {
  int typeIndex = (int)type;
...
void idDeclManagerLocal::ListType(const idCmdArgs& args, declType_t type) {
  ...
  common->Printf("%i of %i %s\n", printed, count,
                 declTypes[static_cast<int>(type)]->typeName.c_str());
}
...
void idDeclManagerLocal::PrintType(const idCmdArgs& args, declType_t type) {
  ...
  declTypes[type]->typeName.c_str(), args.Argv(1)); return;
  }

  // print information common to all decls
  common->Printf("%s %s:\n", declTypes[type]->typeName.c_str(),
  decl->name.c_str()); common->Printf("source: %s:%i\n",
...
const std::shared_ptr<idMaterial> idDeclManagerLocal::FindMaterial(
    const std::string& name, bool makeDefault) {
  return std::static_pointer_cast<idMaterial>(
      FindType(declType_t::DECL_MATERIAL, name, makeDefault));
}
...
===================
*/
void idDeclManagerLocal::MakeNameCanonical(const std::string& name,
                                           std::string& result,
                                           int maxLength) noexcept {
  int i, lastDot;
...
===================
*/
std::shared_ptr<idDeclLocal> idDeclManagerLocal::FindTypeWithoutParsing(
    declType_t type, const std::string& name, bool makeDefault) {
  int typeIndex = static_cast<int>(type);

  if (typeIndex < 0 || static_cast<size_t>(typeIndex) >= declTypes.size() ||
      !declTypes[typeIndex] ||
  ...
  auto iter = std::find_if(
      list.begin(), list.end(),
      [&canonicalName](std::shared_ptr<idDeclLocal> decl) noexcept {
        return decl->name == canonicalName;
      });
  if (iter != list.end()) {
...
=================
*/
const std::string& idDeclLocal::GetName() const noexcept { return name; }
...
=================
*/
int idDeclLocal::Index() const { return index; }
...
=================
*/
int idDeclLocal::GetLineNum() const noexcept { return sourceLine; }
...
=================
*/
int idDeclLocal::GetTextLength() const noexcept { return textLength; }
...
=================
*/
bool idDeclLocal::SetDefaultText() noexcept { return false; }
...
=================
*/
std::string idDeclLocal::DefaultDefinition() const { return "{ }"; }
