...
#define NAMEKEY(code, strId) \
  { K_##code, #code, strId }
#define NAMEKEY2(code) \
  { K_##code, #code, #code }

#define ALIASKEY(alias, code) \
  { K_##code, alias, "" }
...
keyNum_t idKeyInput::StringToKeyNum(const std::string& str) {
  ...
  auto iter =
      std::find_if(keynames.begin(), keynames.end(),
                   [&str](auto& kn) noexcept { return kn.name == str; });

  if (iter != keynames.end()) return iter->keynum;
...
const std::string idKeyInput::KeyNumToString(keyNum_t keynum) {
  ...
  auto iter = std::find_if(
      ...
      [&keynum](auto& kn) noexcept { return kn.keynum == keynum; });

  if (iter != keynames.end()) return iter->name;
...
void Key_ListBinds_f(const idCmdArgs& args) {
  for (int i = 0; i < static_cast<int>(keyNum_t::K_LAST_KEY); i++) {
    if (!keys[i].binding.empty()) {
      common->Printf("%s \"%s\"\n",
                     idKeyInput::KeyNumToString((keyNum_t)i).c_str(),
...
===================
*/
void idKeyInput::Shutdown() noexcept { keys.clear(); }
