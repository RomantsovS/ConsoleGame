...
struct searchpath_t {
  searchpath_t(const std::filesystem::path& _path,
               const std::filesystem::path& _gamedir)
      : path(_path), gamedir(_gamedir) {
...
class idFileSystemLocal : public idFileSystem {
 public:
  idFileSystemLocal() {}
  ~idFileSystemLocal() { isFileSystemExists = false; }
  ...
  std::shared_ptr<idFileList> ListFiles(
      const std::string& relativePath, const std::string& extension,
      bool sort = false, bool fullRelativePath = false,
      const std::string& gamedir = "") override;
  void FreeFileList(std::shared_ptr<idFileList> fileList) noexcept override;
  virtual std::filesystem::path BuildOSPath(
      const std::filesystem::path& base, const std::filesystem::path& game,
      const std::filesystem::path& relativePath);
  virtual void CreateOSPath(const std::filesystem::path& OSPath);
  std::unique_ptr<char[]> ReadFile(const std::string& relativePath, int& len,
                                   ID_TIME_T* timestamp,
                                   bool returnBuffer = false) override;
  virtual std::shared_ptr<idFile> OpenFileReadFlags(
      const std::string& relativePath, int searchFlags,
      bool allowCopyFiles = true, const std::string& gamedir = nullptr);
  std::shared_ptr<idFile> OpenFileRead(
      const std::string& relativePath, bool allowCopyFiles = true,
      const std::string& gamedir = "") override;
  std::shared_ptr<idFile> OpenFileReadMemory(
      const std::string& relativePath, bool allowCopyFiles = true,
      const std::string& gamedir = "") override;
  std::shared_ptr<idFile> OpenFileWrite(
      const std::string& relativePath,
      const std::string& basePath = "fs_basepath") override;
 ...
 private:
  void ReplaceSeparators(std::string& path,
                         char sep = PATHSEPARATOR_CHAR) noexcept;
  int ListOSFiles(const std::filesystem::path& directory,
                  const std::string& extension, std::vector<std::string>& list);
  std::shared_ptr<idFile> OpenOSFile(const std::filesystem::path& fileName,
                                     fsMode_t mode);

  void GetExtensionList(const std::string& extension,
                        std::vector<std::string>& extensionList) const;
  int GetFileList(const std::filesystem::path& relativePath,
                  const std::vector<std::string>& extensions,
                  std::vector<std::string>& list, bool fullRelativePath,
...
idCVar idFileSystemLocal::fs_debug("fs_debug", "1", CVAR_SYSTEM | CVAR_INTEGER,
                                   "", 0, 2);
...
================
*/
std::shared_ptr<idFile> idFileSystemLocal::OpenOSFile(
    const std::filesystem::path& fileName, fsMode_t mode) {
...
====================
*/
void idFileSystemLocal::ReplaceSeparators(std::string& path,
                                          char sep) noexcept {
...
===================
*/
std::filesystem::path idFileSystemLocal::BuildOSPath(
    const std::filesystem::path& base, const std::filesystem::path& game,
    const std::filesystem::path& relativePath) {
...
===========
*/
std::shared_ptr<idFile> idFileSystemLocal::OpenFileReadFlags(
    const std::string& relativePath, int searchFlags, bool allowCopyFiles,
    const std::string& gamedir) {
  if (!IsInitialized()) {
  ...
  if (relativePath.empty()) {
    common->FatalError(
        "idFileSystemLocal::OpenFileRead: empty 'relativePath' parameter "
        "passed\n");
    return nullptr;
  }
  ...
  if (searchFlags & FSFLAG_SEARCH_DIRS) {
    for (int sp = searchPaths.size() - 1; sp >= 0; sp--) {
      ...
      auto netpath = BuildOSPath(searchPaths[sp].path, searchPaths[sp].gamedir,
                                 relativePath);
      std::shared_ptr<idFile_Permanent> file =
          std::dynamic_pointer_cast<idFile_Permanent>(
              OpenOSFile(netpath, FS_READ));
      if (!file->o) {
        continue;
      }
...
===========
*/
std::shared_ptr<idFile> idFileSystemLocal::OpenFileRead(
    const std::string& relativePath, bool allowCopyFiles,
    const std::string& gamedir) {
  return OpenFileReadFlags(relativePath, FSFLAG_SEARCH_DIRS, allowCopyFiles,
                           gamedir);
}
...
===========
*/
std::shared_ptr<idFile> idFileSystemLocal::OpenFileReadMemory(
    const std::string& relativePath, bool allowCopyFiles,
    const std::string& gamedir) {
  return OpenFileReadFlags(relativePath,
                           FSFLAG_SEARCH_DIRS | FSFLAG_RETURN_FILE_MEM,
...
============
*/
std::unique_ptr<char[]> idFileSystemLocal::ReadFile(
    const std::string& relativePath, int& len, ID_TIME_T* timestamp,
    bool returnBuffer) {
  if (!IsInitialized()) {
    common->FatalError("Filesystem call made without initialization\n");
    return 0;
...
===============
*/
void idFileSystemLocal::GetExtensionList(
...
===============
*/
int idFileSystemLocal::GetFileList(const std::filesystem::path& relativePath,
                                   const std::vector<std::string>& extensions,
                                   std::vector<std::string>& list,
  ...
  for (int sp = searchPaths.size() - 1; sp >= 0; sp--) {
    ...
    std::filesystem::path netpath = BuildOSPath(
        searchPaths[sp].path, searchPaths[sp].gamedir, relativePath);
    ...
    for (size_t i = 0; i < extensions.size(); i++) {
      ...
      if (extensions[i][0] == '/' && extensions[i][1] == 0) {
        sysFiles.erase(std::remove(sysFiles.begin(), sysFiles.end(), "."),
                       sysFiles.end());
        sysFiles.erase(std::remove(sysFiles.begin(), sysFiles.end(), ".."),
                       sysFiles.end());
      }
...
===============
*/
std::shared_ptr<idFileList> idFileSystemLocal::ListFiles(
    const std::string& relativePath, const std::string& extension, bool sort,
    bool fullRelativePath, const std::string& gamedir) {
  ...
  GetFileList(relativePath, extensionList, fileList->list, fullRelativePath,
              gamedir);
...
void idFileSystemLocal::FreeFileList(
    std::shared_ptr<idFileList> fileList) noexcept {
  fileList = nullptr;
}
...
===============
*/
int idFileSystemLocal::ListOSFiles(const std::filesystem::path& directory,
                                   const std::string& extension,
                                   std::vector<std::string>& list) {
  return Sys_ListFiles(directory, extension, list);
}

std::shared_ptr<idFile> idFileSystemLocal::OpenFileWrite(
    const std::string& relativePath, const std::string& basePath) {
  if (!IsInitialized()) {
    common->FatalError("Filesystem call made without initialization\n");
  }
  ...
  auto f =
      std::dynamic_pointer_cast<idFile_Permanent>(OpenOSFile(OSpath, FS_WRITE));
  if (!f->o) {
    f = nullptr;
    return nullptr;
  }
...
================
*/
void idFileSystemLocal::AddGameDirectory(const std::string& path,
                                         const std::string& dir) {
  // check if the search path already exists
  for (size_t i = 0; i < searchPaths.size(); i++) {
    if (searchPaths[i].path.compare(path) == 0 &&
        searchPaths[i].gamedir.compare(dir) == 0) {
      return;
    }
  }
