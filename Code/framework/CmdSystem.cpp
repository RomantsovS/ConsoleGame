...
idCVar net_allowCheats("net_allowCheats", "1", CVAR_BOOL | CVAR_NOCHEAT,
                       "Allow cheats in multiplayer");
...
class idCmdSystemLocal : public idCmdSystem {
 public:
  ...
  std::list<std::shared_ptr<commandDef_t>>& GetCommands() noexcept {
    return commands;
  }
...
void idCmdSystemLocal::ListByFlags(const idCmdArgs& args, cmdFlags_t flags) {
  // idStr match;
  std::vector<std::shared_ptr<commandDef_t>> cmd_list(
      cmdSystemLocal.GetCommands().size());
  ...
  std::copy_if(cmdSystemLocal.GetCommands().begin(),
               cmdSystemLocal.GetCommands().end(), cmd_list.begin(),
               [&](auto& cmd) noexcept { return cmd->flags & flags; });
...
void idCmdSystemLocal::Init() {
  ...
  /*AddCommand("listRendererCmds", RendererList_f, CMD_FL_SYSTEM, "lists
  renderer commands"); AddCommand("listSoundCmds", SoundList_f, CMD_FL_SYSTEM,
  "lists sound commands"); AddCommand("listGameCmds", GameList_f, CMD_FL_SYSTEM,
  "lists game commands"); AddCommand("listToolCmds", ToolList_f, CMD_FL_SYSTEM,
  "lists tool commands");*/
  AddCommand("exec", Exec_f, CMD_FL_SYSTEM,
             "executes a config file" /*, ArgCompletion_ConfigName*/);
  /*AddCommand("vstr", Vstr_f, CMD_FL_SYSTEM, "inserts the current value of a
  cvar as command text"); AddCommand("echo", Echo_f, CMD_FL_SYSTEM, "prints
  text"); AddCommand("parse", Parse_f, CMD_FL_SYSTEM, "prints tokenized
  string"); AddCommand("wait", Wait_f, CMD_FL_SYSTEM, "delays remaining buffered
  commands one or more frames");

  // link in all the commands declared with static idCommandLink variables or
  CONSOLE_COMMAND macros for (idCommandLink* link = CommandLinks(); link !=
  NULL; link = link->next) { AddCommand(link->cmdName_, link->function_,
  CMD_FL_SYSTEM, link->description_, link->argCompletion_);
  }*/
...
============
*/
void idCmdSystemLocal::AddCommand(
    const std::string& cmdName, cmdFunction_t function, int flags,
    const std::string& description /*, argCompletion_t argCompletion = NULL*/) {
  // fail if the command already exists
  auto iter =
      std::find_if(commands.begin(), commands.end(),
                   [&](auto cmd) noexcept { return cmd->name == cmdName; });
...
void idCmdSystemLocal::ExecuteTokenizedString(const idCmdArgs& args) {
  ...
  for (auto iter = commands.begin(); iter != commands.end(); ++iter) {
    ...
    if (idStr::caseInSensStringCompareCpp11(args.Argv(0), cmd->name)) {
      ...
      if ((cmd->flags & (CMD_FL_CHEAT | CMD_FL_TOOL)) &&
          !net_allowCheats.GetBool()) {
