...
void idCommonLocal::InitializeMPMapsModes() {
	...
	for (int i = 0; i < numMaps; i++) {
		const std::shared_ptr<idDeclEntityDef> mapDef = std::static_pointer_cast<idDeclEntityDef>(declManager->DeclByIndex(declType_t::DECL_MAPDEF, i));
...
void idCommonLocal::OnStartHosting(idMatchParameters& parms) {
	if ((parms.matchFlags & static_cast<int>(matchFlags_t::MATCH_REQUIRE_PARTY_LOBBY)) == 0) {
