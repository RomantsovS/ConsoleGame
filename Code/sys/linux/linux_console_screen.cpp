...
LinuxConsoleScreen::LinuxConsoleScreen(pos_type wd, pos_type ht,
                                       Pixel back) noexcept
    : Screen(wd, ht) {
  setBackGroundPixel(back);
}

void LinuxConsoleScreen::init() { buffer.resize(width * height); }

Screen& LinuxConsoleScreen::set(pos_type col, pos_type row,
