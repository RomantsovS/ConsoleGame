...
bool Sys_CompareNetAdrBase(const netadr_t a, const netadr_t b) {
  return std::make_tuple(a.type, a.address, a.port) ==
         std::make_tuple(b.type, b.address, b.port);
}
...
bool idUDP::InitForPort(int portNumber) {
  ...
  if (ec) {
    idLib::Printf("NET: Error socket opening: %d %s", ec.value(),
  ...
  if (net_interface.empty() || net_interface == "localhost")
    ep = ba::ip::udp::endpoint(ba::ip::address_v4::any(),
                               portNumber == PORT_ANY ? 0 : portNumber);
  else
    ep = ba::ip::udp::endpoint(boost::asio::ip::make_address(net_interface),
                               portNumber == PORT_ANY ? 0 : portNumber);
  ...
  ec.value(), NET_ErrorString(ec.value())); return false;
  }
  else {
...
bool idUDP::GetPacket(netadr_t& from, void* data, int& size, int maxSize) {
  while (1) {
    ...
    boost::system::error_code ec;
    size = socket->receive_from(boost::asio::buffer(data, maxSize),
...
void idUDP::SendPacket(const netadr_t to, const void* data, int size) {
  ...
  boost::system::error_code ec;
  size_t len =
      socket->send_to(boost::asio::buffer(data, size), destination, 0, ec);
