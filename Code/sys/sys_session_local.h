...
class idSessionLocal : public idSession {
 ...
 protected:
  ...
  enum class state_t {
    ...
    STATE_CREATE_AND_MOVE_TO_PARTY_LOBBY,  // We are creating a party lobby, and
                                           // will move to that state when done
    STATE_CREATE_AND_MOVE_TO_GAME_LOBBY,   // We are creating a game lobby, and
                                           // will move to that state when done
    STATE_CREATE_AND_MOVE_TO_GAME_STATE_LOBBY,  // We are creating a game state
                                                // lobby, and will move to that
                                                // state when done
 ...
 public:
  ...
  // Misc
  void EndMatch(bool premature =
                    false) override;  // Meant for host to end match gracefully,
                                      // go back to lobby, tally scores, etc
  void QuitMatchToTitle() noexcept override;  // Will forcefully quit the match
 ...
 protected:
  ...
  // The party and game lobby are the two platform lobbies that notify the
  // backends (Steam/PSN/LIVE of changes)
  ...
  // Game state lobby is the lobby used while in-game.  It is so the dedicated
  // server can host this lobby and have all platform clients join. It does NOT
  // notify the backends of changes, it's purely for the dedicated server to be
  // able to host the in-game lobby. Generally, you would call
  // GetActingGameStateLobby.  If we are not using game state lobby,
  // GetActingGameStateLobby will return GetGameLobby insread.
  ...
  // GetActivePlatformLobby will return either the game or party lobby, it won't
  // return the game state lobby This function is generally used for menus,
  // in-game code should refer to GetActingGameStateLobby
 ...
 protected:
  ...
  idLobby stubLobby;  // We use this when we request the active lobby when we
                      // are not in a lobby (i.e at press start)
  ...
  void GoodbyeFromHost(idLobby& lobby, int peerNum,
                       const lobbyAddress_t& remoteAddress, int msgType);
  ...
  void MoveToMainMenu() noexcept;  // End all session (async), and return to
                                   // IDLE state
...
idSessionLocalCallbacks
        The more the idLobby class needs to call back into this class, the more
likely we're doing something wrong, and there is a better way.
========================
*/
class idSessionLocalCallbacks : public idSessionCallbacks {
 public:
  ...
  idLobbyBackend* CreateLobbyBackend(
      const idMatchParameters& p, float skillLevel,
      idLobbyBackend::lobbyBackendType_t lobbyType) override;
  idLobbyBackend* FindLobbyBackend(
      const idMatchParameters& p, int numPartyUsers, float skillLevel,
      idLobbyBackend::lobbyBackendType_t lobbyType) override;
  idLobbyBackend* JoinFromConnectInfo(
      const lobbyConnectInfo_t& connectInfo,
      idLobbyBackend::lobbyBackendType_t lobbyType) override;
...
class idSessionLocalWin : public idSessionLocal {
 ...
 private:
  ...
  idLobbyBackend* CreateLobbyInternal(
...
class idLobbyToSessionCBLocal : public idLobbyToSessionCB {
 public:
  ...
  bool CanJoinLocalHost() const override {
    sessionLocalWin->EnsurePort();
    return sessionLocalWin->canJoinLocalHost;
  }
  idLobbyBackend* GetLobbyBackend(
      idLobbyBackend::lobbyBackendType_t type) const override {
    return sessionLocalWin->lobbyBackends[type].get();
  }
