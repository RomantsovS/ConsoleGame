...
int idLobby::GetLobbyUserIndexByID(lobbyUserID_t lobbyUserId, bool ignoreLobbyType) const {
	...
	idassert(lobbyUserId.GetLobbyType() == static_cast<std::byte>(lobbyType) || ignoreLobbyType);
...
lobbyUser_t idLobby::CreateLobbyUserFromLocalUser() {
	...
	lobbyUser.lobbyUserID = lobbyUserID_t(Sys_GetClockTicks(), static_cast<std::byte>(lobbyType));	// Generate the lobby using a combination of local user id, and lobby type
...
void idLobby::SendNewUsersToPeers(int skipPeer, int userStart, int numUsers) {
	...
	//	if (p == skipPeer || peers[p].GetConnectionState() != connectionState_t::CONNECTION_ESTABLISHED) {
