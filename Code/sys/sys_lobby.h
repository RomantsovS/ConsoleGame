...
class idLobby : public idLobbyBase {
 public:
  ...
  void Pump();
  void Shutdown(bool retainMigrationInfo = false,
                bool skipGoodbye = false);  // Goto idle state
  void HandlePacket(lobbyAddress_t& remoteAddress, idBitMsg& fragMsg,
  ...
  enum class reliablePlayerToPlayer_t {
    ...
    // Game messages would be reserved here in the same way that
    // RELIABLE_GAME_DATA is.
    // I'm worried about using up the 0xff values we have for reliable type, so
    // I'm not
  ...
  enum class reliableType_t {
    ...
    RELIABLE_CONNECT_AND_MOVE_TO_LOBBY,  // host to peer : connect to this
                                         // server
    ...
    // User join in progress msg's (join in progress for the party/game lobby,
    // not inside a match)
    RELIABLE_USER_CONNECT_REQUEST,  // peer to host: local user wants to join
                                    // session in progress
    RELIABLE_USER_CONNECT_DENIED,   // host to peer: user join session in
                                    // progress denied (not enough slots)

    // User leave in progress msg's (leave in progress for the party/game lobby,
    // not inside a match)
    RELIABLE_USER_DISCONNECT_REQUEST,  // peer to host: request host to remove
                                       // user from session

    RELIABLE_KICK_PLAYER,  // host to peer : kick a player

    RELIABLE_MATCHFINISHED,  // host to peer - Match is in post looking at score
                             // board
    RELIABLE_ENDMATCH,       // host to peer - End match, and go to game lobby
    RELIABLE_ENDMATCH_PREMATURE,  // host to peer - End match prematurely, and
                                  // go to game lobby (onl possible in
                                  // unrated/custom games)

    RELIABLE_SESSION_USER_MODIFIED,  // peer to host : user changed something
                                     // (emblem, name, etc)
    RELIABLE_UPDATE_SESSION_USER,    // host to peers : inform all peers of the
                                     // change

    RELIABLE_HEADSET_STATE,  // * to * : headset state change for user
    RELIABLE_VOICE_STATE,  // * to * : voice state changed for user pair (mute,
                           // unmute, etc)
    RELIABLE_PING,         // * to * : send host->peer, then reflected
    RELIABLE_PING_VALUES,  // host to peers : ping data from lobbyUser_t for
                           // everyone
    ...
    RELIABLE_MIGRATION_GAME_DATA,  // host to peers: game data to use incase of
                                   // a migration

    RELIABLE_START_MATCH_GAME_LOBBY_HOST,  // game lobby host to game state
                                           // lobby host: start the match, since
                                           // all players are in
    ...
    RELIABLE_PLAYER_TO_PLAYER_END =
        RELIABLE_PLAYER_TO_PLAYER_BEGIN +
        static_cast<int>(
            reliablePlayerToPlayer_t::NUM_RELIABLE_PLAYER_TO_PLAYER),
  ...
  static const std::string
      stateToString[static_cast<int>(lobbyState_t::NUM_STATES)];

  static const int CONNECT_REQUEST_FREQUENCY_IN_SECONDS =
      5;  // Frequency at which we resend a request to connect to a server (will
          // increase in frequency over time down to
          // MIN_CONNECT_FREQUENCY_IN_SECONDS)
  inline static const int MIN_CONNECT_FREQUENCY_IN_SECONDS =
      1;  // Min frequency of connection attempts
  ...
  enum class connectionState_t {
    ...
    CONNECTION_CONNECTING =
        1,  // Waiting for response from host for initial connection
  ...
  struct peer_t {
    ...
    void ResetConnectState() { needToSubmitPendingSnap = false; }
    ...
    bool IsActive() const {
      return connectionState != connectionState_t::CONNECTION_FREE;
    }
    bool IsConnected() const {
      return connectionState == connectionState_t::CONNECTION_ESTABLISHED;
    }
    ...
    bool inGame;  // true if this peer received the first snapshot, and is
                  // in-game

    lobbyAddress_t address;

    std::unique_ptr<idPacketProcessor>
        packetProc;  // Processes packets for this peer
    std::unique_ptr<idSnapshotProcessor>
        snapProc;  // Processes snapshots for this peer
  ...
  std::string GetLobbyName() {
    switch (lobbyType) {
      case lobbyType_t::TYPE_PARTY:
        return "TYPE_PARTY";
      case lobbyType_t::TYPE_GAME:
        return "TYPE_GAME";
      case lobbyType_t::TYPE_GAME_STATE:
  ...
  lobbyUser_t* GetLobbyUser(int index) {
    return (index >= 0 && index < GetNumLobbyUsers()) ? userList[index]
                                                      : nullptr;
  }
  ...
  // IsRunningAsHostOrPeer means we are either an active host, and can accept
  // connections from peers, or we are a peer with an active connection to a
  // host
  ...
  int FindPeer(const lobbyAddress_t& remoteAddress,
               idPacketProcessor::sessionId_t sessionID,
               bool ignoreSessionID = false);
  int FindFreePeer() const;
  int AddPeer(const lobbyAddress_t& remoteAddress,
              idPacketProcessor::sessionId_t sessionID);
  void DisconnectPeerFromSession(int p);
  void SetPeerConnectionState(int p, connectionState_t newState,
                              bool skipGoodbye = false);

  void SendGoodbye(const lobbyAddress_t& remoteAddress, bool wasFull = false);
  void QueueReliableMessage(int peerNum, reliableType_t type) {
    QueueReliableMessage(peerNum, type, nullptr, 0);
  }
  void QueueReliableMessage(int p, reliableType_t type, const std::byte* data,
                            int dataLen);

  void SendConnectionLess(const lobbyAddress_t& remoteAddress, std::byte type) {
  ...
  void HandleGoodbyeFromPeer(int peerNum, lobbyAddress_t& remoteAddress,
                             int msgType);
  void HandleConnectionAttemptFailed();
  bool ConnectToNextSearchResult();
  int HandleInitialPeerConnection(idBitMsg& msg,
                                  const lobbyAddress_t& peerAddress,
                                  int peerNum);
  void InitStateLobbyHost();

  void SendMembersToLobby(lobbyType_t destLobbyType,
  ...
  void SendPeerMembersToLobby(int peerIndex, lobbyType_t destLobbyType,
                              const lobbyConnectInfo_t& connectInfo,
                              bool waitForOtherMembers);
  ...
  void ProcessOutgoingMsg(int p, const void* data, int size, bool isOOB,
                          int userData);
  ...
  void DecodeSessionID(idPacketProcessor::sessionId_t sessionID,
                       uint32_t& key) const;
  idPacketProcessor::sessionId_t GenerateSessionID() const;
  bool SessionIDCanBeUsedForInBand(
      idPacketProcessor::sessionId_t sessionID) const;
  idPacketProcessor::sessionId_t IncrementSessionID(
      idPacketProcessor::sessionId_t sessionID) const;
  ...
  int GetLobbyUserIndexByID(lobbyUserID_t lobbyUserId,
                            bool ignoreLobbyType = false) const;

  // Helper function to create a lobby user from a local user
  lobbyUser_t CreateLobbyUserFromLocalUser();

  // This function is designed to initialize the session users of type lobbyType
  // (TYPE_GAME or TYPE_PARTY) to the current list of local users that are being
  // tracked by the sign-in manager
  ...
  int host;  // which peer is the host of this type of session (-1 if we are the
             // host)
  ...
  std::array<std::byte, SNAP_OBJ_JOB_MEMORY>
      objMemory;  // Shared across all snapshot jobs
};
...
class idSessionCallbacks {
 public:
  ...
  virtual void SendRawPacket(const lobbyAddress_t& to, const void* data,
                             int size, bool useDirectPort) = 0;

  virtual void GoodbyeFromHost(idLobby& lobby, int peerNum,
                               const lobbyAddress_t& remoteAddress,
                               int msgType) = 0;

  virtual idSession::sessionState_t GetState() const = 0;

  // Called once the lobby received its first full snap (used to advance from
  // LOADING to INGAME state)
  virtual void ReceivedFullSnap() = 0;

  virtual idLobbyBackend* CreateLobbyBackend(
      const idMatchParameters& p, float skillLevel,
      idLobbyBackend::lobbyBackendType_t lobbyType) = 0;
  virtual idLobbyBackend* FindLobbyBackend(
      const idMatchParameters& p, int numPartyUsers, float skillLevel,
      idLobbyBackend::lobbyBackendType_t lobbyType) = 0;
  virtual idLobbyBackend* JoinFromConnectInfo(
      const lobbyConnectInfo_t& connectInfo,
      idLobbyBackend::lobbyBackendType_t lobbyType) = 0;
