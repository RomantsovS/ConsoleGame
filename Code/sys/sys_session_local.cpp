...
idCVar net_port(
    "net_port", "27015", CVAR_INTEGER,
    "host port number");  // Port to host when using dedicated servers, port to
                          // broadcast on when looking for a dedicated server to
                          // connect to
idCVar net_headlessServer(
    "net_headlessServer", "0", CVAR_BOOL,
    "toggle to automatically host a game and allow peer[0] to control menus");

const std::string
    idSessionLocal::stateToString[static_cast<int>(state_t::NUM_STATES)] = {
        ...
        idassert_ENUM_STRING(state_t::STATE_CREATE_AND_MOVE_TO_GAME_STATE_LOBBY,
                             10),
...
========================
*/
idSessionLocal::idSessionLocal() { InitBaseState(); }
...
idCVar net_connectTimeoutInSeconds("net_connectTimeoutInSeconds", "15",
                                   CVAR_INTEGER,
                                   "timeout (in seconds) while connecting");
...
void idSessionLocal::CreateMatch(const idMatchParameters& p) {
  ...
  if ((p.matchFlags &
       static_cast<int>(matchFlags_t::MATCH_PARTY_INVITE_PLACEHOLDER)) &&
      !GetPartyLobby().IsLobbyActive()) {
...
void idSessionLocal::FindOrCreateMatch(const idMatchParameters& p) {
  ...
  /*if ((p.matchFlags & MATCH_PARTY_INVITE_PLACEHOLDER) &&
  !GetPartyLobby().IsLobbyActive()) { NET_VERBOSE_PRINT("NET: FindOrCreateMatch
  MATCH_PARTY_INVITE_PLACEHOLDER\n"); CreatePartyLobby(p); connectType =
  CONNECT_FIND_OR_CREATE; return;
  }*/
...
void idSessionLocal::StartMatch() noexcept {
  ...
  if (net_headlessServer.GetBool()) {
    StartLoading();  // This is so we can force start matches on headless
                     // servers to test performance using bots
    return;
  }
...
idSessionLocal::sessionState_t idSessionLocal::GetBackState() {
  ...
  const bool isInGame =
      currentState == sessionState_t::INGAME ||
      currentState ==
          sessionState_t::LOADING;  // Counting loading as ingame as far as what
                                    // back state to go to
  ...
  if (!isInPartyLobby && isInGameLobby) {
    return sessionState_t::PARTY_LOBBY;  // If in the game lobby, and we should
                                         // have a party lobby, and we are the
                                         // host, go back to party lobby
  }

  if (currentState != sessionState_t::IDLE) {
    return sessionState_t::IDLE;  // From here, go to idle if we aren't there
                                  // yet
  }
  ...
  switch (GetBackState()) {
    ...
    case sessionState_t::PARTY_LOBBY:
      if (GetPartyLobby().IsHost()) {
        ...
        //	// so it won't be affected by the fact that we're shutting down
        // the game lobby 	GetPartyLobby().NotifyPartyOfLeavingGameLobby();
        // }
        // else {
        ...
        // Always remove this flag.  SendGoodbye uses this to determine if we
        // should send a "leave with party" and we don't want this flag hanging
        // around, and causing false positives when it's called in the future.
        // Make them set this each time.
        // sessionOptions &= ~OPTION_LEAVE_WITH_PARTY;
      } else {
        // If we aren't the host of a party and we want to go back to one, we
        // need to create a party now
...
Called from State_Create_And_Move_To_Party_Lobby and
State_Create_And_Move_To_Game_Lobby and
State_Create_And_Move_To_Game_State_Lobby. This function will create the lobby,
then wait for it to either succeed or fail.
========================
*/
bool idSessionLocal::WaitOnLobbyCreate(idLobby& lobby) {
  idassert(localState == state_t::STATE_CREATE_AND_MOVE_TO_PARTY_LOBBY ||
           localState == state_t::STATE_CREATE_AND_MOVE_TO_GAME_LOBBY ||
           localState == state_t::STATE_CREATE_AND_MOVE_TO_GAME_STATE_LOBBY);
  // idassert(connectType == CONNECT_FIND_OR_CREATE || connectType ==
  // CONNECT_NONE);

  if (lobby.GetState() == idLobby::lobbyState_t::STATE_FAILED) {
    NET_VERBOSE_PRINT(
        "NET: idSessionLocal::WaitOnLobbyCreate lobby.GetState() == "
        "idLobby::STATE_FAILED (%s)\n",
        lobby.GetLobbyName().c_str());
    // If we failed to create a lobby, assume connection to backend service was
    // lost
    MoveToMainMenu();
    return false;
  }
...
bool idSessionLocal::DetectDisconnectFromService(bool cancelAndShowMsg) {
  ...
  if (DETECT_SERVICE_DISCONNECT_TIMEOUT_IN_SECONDS > 0) {
    if (Sys_Milliseconds() - connectTime >
        1000 * DETECT_SERVICE_DISCONNECT_TIMEOUT_IN_SECONDS) {
      NET_VERBOSE_PRINT(
          "NET: idSessionLocal::DetectDisconnectFromService timed out\n");
      if (cancelAndShowMsg) {
        MoveToMainMenu();
      }
...
void idSessionLocal::HandleConnectionFailed(idLobby& lobby, bool wasFull) {
  idassert(localState == state_t::STATE_CONNECT_AND_MOVE_TO_PARTY ||
           localState == state_t::STATE_CONNECT_AND_MOVE_TO_GAME ||
...
bool idSessionLocal::HandleConnectAndMoveToLobby(idLobby& lobby) {
  ...
  if (lobby.GetState() == idLobby::lobbyState_t::STATE_FAILED) {
    // If we get here, we were trying to connect to a lobby (from state
    // State_Connect_And_Move_To_Party/State_Connect_And_Move_To_Game)
  ...
  // If we are connecting to a game lobby, see if we need to keep waiting as
  // either a host or peer while we're confirming all party members made it
  if (lobby.lobbyType == idLobby::lobbyType_t::TYPE_GAME) {
    if (GetPartyLobby().IsHost()) {
      const int timeoutMs = net_connectTimeoutInSeconds.GetInteger() * 1000;

      if (timeoutMs != 0 &&
          Sys_Milliseconds() - lobby.helloStartTime > timeoutMs) {
      ...
      for (int i = 0; i < GetPartyLobby().GetNumLobbyUsers(); i++) {
        ...
        for (int j = 0; j < GetGameLobby().GetNumLobbyUsers(); j++) {
          ...
          if (GetGameLobby().IsSessionUserLocal(gameUser) ||
              gameUser->address.Compare(partyUser->address, true)) {
            numUsersIn++;
            foundUser = true;
            break;
          }
      ...
      if (numUsersIn != GetPartyLobby().GetNumLobbyUsers()) {
        return HandlePackets();  // All users not in, keep waiting until all
                                 // user make it, or we time out
      }

      NET_VERBOSE_PRINT(
          "NET: All party members made it into the game lobby.\n");

      // Let all the party members know everyone made it, and it's ok to stay at
      // this server
      for (int i = 0; i < GetPartyLobby().peers.size(); i++) {
        if (GetPartyLobby().peers[i].IsConnected()) {
          // GetPartyLobby().QueueReliableMessage(i,
          // idLobby::RELIABLE_PARTY_CONNECT_OK);
        }
      }
    } else {
  ...
  switch (lobby.lobbyType) {
    ...
    case idLobby::lobbyType_t::TYPE_GAME_STATE:
      // As a host of the game lobby, it's our duty to notify our members to
      // also join this game state lobby
...
========================
*/
bool idSessionLocal::State_Party_Lobby_Host() { return HandlePackets(); }
...
========================
*/
bool idSessionLocal::State_Game_Lobby_Host() { return HandlePackets(); }
...
bool idSessionLocal::State_Create_And_Move_To_Party_Lobby() {
  if (WaitOnLobbyCreate(GetPartyLobby())) {
    ...
    //	// If this party lobby was for a placeholder, continue on with either
    // finding or creating a game lobby 	if (connectType ==
    // CONNECT_FIND_OR_CREATE) {
    // FindOrCreateMatch(GetPartyLobby().parms); 		return true;
    //	}
...
bool idSessionLocal::State_Find_Or_Create_Match() {
  ...
  if (GetGameLobby().GetState() == idLobby::lobbyState_t::STATE_FAILED) {
    // Failed to find any games.  Create one instead (we're assuming this always
    // gets called from FindOrCreateMatch
...
========================
*/
bool idSessionLocal::State_InGame() { return HandlePackets(); }
...
bool idSessionLocal::State_Loading() {
  ...
  if (GetActingGameStateLobby().IsHost()) {
    ...
    for (int p = 0; p < GetActingGameStateLobby().peers.size(); p++) {
      ...
      if (!peer.IsConnected()) {
        continue;  // We don't care about peers that aren't connected as a game
                   // session
      }

      if (!peer.loaded) {
        everyoneLoaded = false;
        continue;  // Don't waste time sending resources to a peer who hasn't
                   // loaded the map yet
      }
      ...
      //	// if client is taking a LONG time to load up - give them the
      // boot: they're just holding up the lunch line. Useful for loose assets
      // playtesting. 	int time = Sys_Milliseconds(); 	int maxLoadTime =
      // net_maxLoadResourcesTimeInSeconds.GetInteger(); 	if (maxLoadTime
      // > 0 && peer.startResourceLoadTime + SEC2MS(maxLoadTime) < time) {
      //		NET_VERBOSERESOURCE_PRINT("NET: dropping client %i - %s
      // because they took too long to load resources.\n Check
      //'net_maxLoadResourcesTimeInSeconds' to adjust the time allowed.\n", p,
      // GetPeerName(p));
    ...
    // not sure how we got there, but we won't be receiving anything that could
    // get us out of this state anymore possible step towards fixing the join
    // stalling/disconnect problems
    if (GetActingGameStateLobby().peers.size() == 0) {
      NET_VERBOSE_PRINT(
          "NET: no peers in idSessionLocal::State_Loading - giving up\n");
      MoveToMainMenu();
    }
    ...
    if (!haveOneGoodPeer) {
      NET_VERBOSE_PRINT(
          "NET: no good peers in idSessionLocal::State_Loading - giving up\n");
      MoveToMainMenu();
    }
  ...
  SetState(
      state_t::STATE_INGAME);  // NOTE - Only the host is in-game at this point,
                               // all peers will start becoming in-game when
                               // they receive their first full snap
  return true;
}

idCVar net_verbose("net_verbose", "0", CVAR_BOOL,
...
EndMatch is meant for the host to cleanly end a match and return to the lobby
page
========================
*/
void idSessionLocal::EndMatch(bool premature /*=false*/) {
  if (idverify(GetActingGameStateLobby().IsHost())) {
    // Host quits back to game lobby, and will notify peers internally to do the
    // same
...
========================
*/
void idSessionLocal::QuitMatchToTitle() noexcept { MoveToMainMenu(); }
...
void idSessionLocal::EndMatchInternal(bool premature /*=false*/) {
  idassert(GetGameStateLobby().IsLobbyActive() ==
           net_useGameStateLobby.GetBool());

  for (int p = 0; p < GetActingGameStateLobby().peers.size(); p++) {
    // If we are the host, increment the session ID.  The client will use a
    // rolling check to catch it
    if (GetActingGameStateLobby().IsHost()) {
      if (GetActingGameStateLobby().peers[p].IsConnected()) {
        /*if (GetActingGameStateLobby().peers[p].packetProc != NULL) {
                GetActingGameStateLobby().peers[p].packetProc->VerifyEmptyReliableQueue(idLobby::RELIABLE_GAME_DATA,
        idLobby::RELIABLE_DUMMY_MSG);
        }
        GetActingGameStateLobby().peers[p].sessionID =
        GetActingGameStateLobby().IncrementSessionID(GetActingGameStateLobby().peers[p].sessionID);*/
      }
    }
  ...
  // gameLobbyWasCoalesced = false;		// Reset this back to false.  We
  // use this so the lobby code doesn't randomly choose a map when we coalesce
  numFullSnapsReceived = 0;
  ...
  if (GetActingGameStateLobby().IsLobbyActive() /* && (GetActingGameStateLobby().GetMatchParms().matchFlags & MATCH_REQUIRE_PARTY_LOBBY)*/) {
    ...
    if (GetActingGameStateLobby().IsHost()) {
      ...
      // so he can wait on all his game state peers to leave
      SetState(
          GetGameStateLobby().IsHost()
              ? state_t::STATE_GAME_STATE_LOBBY_HOST
              : state_t::STATE_GAME_LOBBY_HOST);  // We want the dedicated host
                                                  // to go back to
                                                  // STATE_GAME_STATE_LOBBY_HOST
    } else {
      SetState(state_t::STATE_GAME_LOBBY_PEER);
    }
  } else {
  ...
  //		GetActingGameStateLobby().QueueReliableMessage(p, premature ?
  // idLobby::RELIABLE_ENDMATCH_PREMATURE : idLobby::RELIABLE_ENDMATCH);
  //	}
  // }
  // else if (premature) {
  //	// Notify client that host left early and thats why we are back in the
  // lobby 	const bool stats =
  // MatchTypeHasStats(GetActingGameStateLobby().GetMatchParms().matchFlags) &&
  //(GetFlushedStats() == false); 	common->Dialog().AddDialog(stats ?
  // GDM_HOST_RETURNED_TO_LOBBY_STATS_DROPPED : GDM_HOST_RETURNED_TO_LOBBY,
  // DIALOG_ACCEPT, NULL, NULL, false, __FUNCTION__, __LINE__, true);
  // }

  // if (GetGameStateLobby().IsLobbyActive()) {
  //	if (GetGameStateLobby().IsHost()) {
  //		// As a game state host, keep the lobby around, so we can make
  // sure we know when everyone leaves (which means they got the reliable msg to
  // EndMatch) 		waitingOnGameStateMembersToLeaveTime =
  // Sys_Milliseconds();
  //	}
...
void idSessionLocal::ValidateLobbies() {
  if (localState == state_t::STATE_PRESS_START ||
      localState == state_t::STATE_IDLE) {
    // At press start or main menu, don't do anything
    return;
  }

  if (GetActivePlatformLobby() == nullptr) {
    // If we're in between lobbies, don't do anything yet (the state
    // transitioning code will handle error cases)
    return;
  }

  // Validate lobbies that should be alive and active
  if (/*ShouldHavePartyLobby() &&*/ GetState() >=
      idSession::sessionState_t::PARTY_LOBBY) {
    ValidateLobby(GetPartyLobby());
  }
  if (GetState() >= idSession::sessionState_t::GAME_LOBBY &&
      !net_headlessServer.GetBool()) {
...
void idSessionLocal::ValidateLobby(idLobby& lobby) {
  if (!lobby.lobbyBackend ||
      lobby.lobbyBackend->GetState() ==
          idLobbyBackend::lobbyBackendState_t::STATE_FAILED ||
      lobby.GetState() == idLobby::lobbyState_t::STATE_FAILED) {
    NET_VERBOSE_PRINT(
        "NET: ValidateLobby: FAILED (lobbyType = %i, state = %s)\n",
        lobby.lobbyType, stateToString[static_cast<int>(localState)].c_str());
    if (lobby.failedReason ==
            idLobby::failedReason_t::FAILED_MIGRATION_CONNECT_FAILED ||
        lobby.failedReason == idLobby::failedReason_t::FAILED_CONNECT_FAILED) {
      MoveToMainMenu();
      // common->Dialog().AddDialog(GDM_INVALID_INVITE, DIALOG_ACCEPT, NULL,
      // NULL, false);		// The game session no longer exists
    } else {
      // If the lobbyBackend goes bad under our feet for no known reason, assume
      // we lost connection to the back end service
      MoveToMainMenu();
      // common->Dialog().ClearDialogs(true);
      // common->Dialog().AddDialog(GDM_CONNECTION_LOST, DIALOG_ACCEPT, NULL,
      // NULL, false);		// Lost connection to XBox LIVE
    }
  }
}
...
void idSessionLocal::Pump() {
  ...
  if (lastPumpTime != -1 && elapsedPumpSeconds > 2) {
    idLib::Warning("idSessionLocal::Pump was not called for %i seconds",
                   elapsedPumpSeconds);
  }
  ...
  if (GetPartyLobby().lobbyBackend) {
    // Make sure game properties aren't set on the lobbyBackend if we aren't in
    // a game lobby. This is so we show up properly in search results in Play
    // with Friends option
  ...
  // Do some last minute checks, make sure everything about the current state
  // and lobbyBackend state is valid, otherwise, take action
  ValidateLobbies();
  ...
  //		// the player chose "Quick Match" and doesn't care about the
  // mode.
  ...
  //		gameLobbyWasCoalesced = true;		// Remember that this
  // round was coalesced.  We so this so main menu doesn't randomize the map,
  // which looks odd 		nextGameCoalesceTime = 0;
  //	}
  //}
}
...
bool idSessionLocal::HandleState() {
  ...
  if (GetPartyLobby().IsHost() &&
      localState == state_t::STATE_PARTY_LOBBY_PEER) {
    SetState(state_t::STATE_PARTY_LOBBY_HOST);
  } else if (GetPartyLobby().IsPeer() &&
             localState == state_t::STATE_PARTY_LOBBY_HOST) {
    SetState(state_t::STATE_PARTY_LOBBY_PEER);
  }
  ...
  if (GetGameLobby().IsHost() && localState == state_t::STATE_GAME_LOBBY_PEER) {
    SetState(state_t::STATE_GAME_LOBBY_HOST);
  } else if (GetGameLobby().IsPeer() &&
             localState == state_t::STATE_GAME_LOBBY_HOST) {
    SetState(state_t::STATE_GAME_LOBBY_PEER);
  }

  switch (localState) {
    case state_t::STATE_PRESS_START:
      return false;
    case state_t::STATE_IDLE:
      HandlePackets();
      return false;  // Call handle packets, since packets from old sessions
                     // could still be in flight, which need to be emptied
    case state_t::STATE_PARTY_LOBBY_HOST:
      return State_Party_Lobby_Host();
      // case state_t::STATE_PARTY_LOBBY_PEER:
      // return State_Party_Lobby_Peer();
    case state_t::STATE_GAME_LOBBY_HOST:
      return State_Game_Lobby_Host();
    case state_t::STATE_GAME_LOBBY_PEER:
      return State_Game_Lobby_Peer();
      /*case state_t::STATE_GAME_STATE_LOBBY_HOST:
      return State_Game_State_Lobby_Host(); case
      state_t::STATE_GAME_STATE_LOBBY_PEER:				return
      State_Game_State_Lobby_Peer();*/
    case state_t::STATE_LOADING:
      return State_Loading();
    case state_t::STATE_INGAME:
      return State_InGame();
    case state_t::STATE_CREATE_AND_MOVE_TO_PARTY_LOBBY:
      return State_Create_And_Move_To_Party_Lobby();
    case state_t::STATE_CREATE_AND_MOVE_TO_GAME_LOBBY:
      return State_Create_And_Move_To_Game_Lobby();
    // case state_t::STATE_CREATE_AND_MOVE_TO_GAME_STATE_LOBBY:	return
    // State_Create_And_Move_To_Game_State_Lobby();
    case state_t::STATE_FIND_OR_CREATE_MATCH:
      return State_Find_Or_Create_Match();
    // case state_t::STATE_CONNECT_AND_MOVE_TO_PARTY:			return
    // State_Connect_And_Move_To_Party();
    case state_t::STATE_CONNECT_AND_MOVE_TO_GAME:
      return State_Connect_And_Move_To_Game();
      /*case state_t::STATE_CONNECT_AND_MOVE_TO_GAME_STATE:		return
      State_Connect_And_Move_To_Game_State(); case state_t::STATE_BUSY:
      return State_Busy();*/
    default:
      idLib::Error("HandleState:  Unknown state in idSessionLocal: %s",
...
idSessionLocal::sessionState_t idSessionLocal::GetState() const {
  ...
  switch (localState) {
    case state_t::STATE_PRESS_START:
      return sessionState_t::PRESS_START;
    case state_t::STATE_IDLE:
      return sessionState_t::IDLE;
    case state_t::STATE_PARTY_LOBBY_HOST:
      return sessionState_t::PARTY_LOBBY;
    case state_t::STATE_PARTY_LOBBY_PEER:
      return sessionState_t::PARTY_LOBBY;
    case state_t::STATE_GAME_LOBBY_HOST:
      return sessionState_t::GAME_LOBBY;
    case state_t::STATE_GAME_LOBBY_PEER:
      return sessionState_t::GAME_LOBBY;
    case state_t::STATE_GAME_STATE_LOBBY_HOST:
      return sessionState_t::GAME_LOBBY;
    case state_t::STATE_GAME_STATE_LOBBY_PEER:
      return sessionState_t::GAME_LOBBY;
    case state_t::STATE_LOADING:
      return sessionState_t::LOADING;
    case state_t::STATE_INGAME:
      return sessionState_t::INGAME;
    case state_t::STATE_CREATE_AND_MOVE_TO_PARTY_LOBBY:
      return sessionState_t::CONNECTING;
    case state_t::STATE_CREATE_AND_MOVE_TO_GAME_LOBBY:
      return sessionState_t::CONNECTING;
    case state_t::STATE_CREATE_AND_MOVE_TO_GAME_STATE_LOBBY:
      return sessionState_t::CONNECTING;
    case state_t::STATE_FIND_OR_CREATE_MATCH:
      return sessionState_t::SEARCHING;
    case state_t::STATE_CONNECT_AND_MOVE_TO_PARTY:
      return sessionState_t::CONNECTING;
    case state_t::STATE_CONNECT_AND_MOVE_TO_GAME:
      return sessionState_t::CONNECTING;
    case state_t::STATE_CONNECT_AND_MOVE_TO_GAME_STATE:
      return sessionState_t::CONNECTING;
    case state_t::STATE_BUSY:
      return sessionState_t::BUSY;
    default: {
...
void idSessionLocal::UpdateSignInManager() {
  if (!localUserRegistered) {
    // If we don't have a master user at all, then we need to be at "Press
    // Start"
    MoveToPressStart();
    return;
  } else if (localState == state_t::STATE_PRESS_START) {
    // If we have a master user, and we are at press start, move to the menu
    // area
    SetState(state_t::STATE_IDLE);
  }
}
...
void idSessionLocal::LoadingFinished() noexcept {
  ...
  if (!GetActingGameStateLobby()
           .IsHost()) {  // Tell game host we're done loading
    std::array<std::byte, idPacketProcessor::MAX_PACKET_SIZE> buffer;
    idBitMsg msg(buffer.data(), sizeof(buffer));
    GetActingGameStateLobby().QueueReliableMessage(
        GetActingGameStateLobby().host,
        idLobby::reliableType_t::RELIABLE_LOADING_DONE, msg.GetReadData(),
        msg.GetSize());
  } else {
...
bool idSessionLocal::HandlePackets() {
  ...
  while (ReadRawPacket(remoteAddress, packetBuffer.data(), recvSize,
                       packetBuffer.size()) &&
         recvSize > 0) {
    idBitMsg fragMsg;
    ...
    idPacketProcessor::sessionId_t sessionID =
        idPacketProcessor::GetSessionID(fragMsg);
    ...
    idLobby::lobbyType_t lobbyType =
        static_cast<idLobby::lobbyType_t>(maskedType - 1);
...
idLobby* idSessionLocal::GetActivePlatformLobby() {
  ...
  if ((state == sessionState_t::GAME_LOBBY) ||
      (state == sessionState_t::BUSY) || (state == sessionState_t::INGAME) ||
      (state == sessionState_t::LOADING)) {
    return &GetGameLobby();
...
========================
*/
void idSessionLocal::SendRawPacket(const lobbyAddress_t& to, const void* data,
                                   int size, bool dedicated) {
  const int now = Sys_Milliseconds();

  // short path
  // NOTE: network queuing: will go to tick the queue whenever sendQueue isn't
  // empty, regardless of latency
...
========================
*/
bool idSessionLocal::ReadRawPacket(lobbyAddress_t& from, void* data, int& size,
                                   int maxSize) {
  for (int i = 0; i < 2; i++) {
...
========================
*/
void idSessionLocal::GoodbyeFromHost(idLobby& lobby, int peerNum,
                                     const lobbyAddress_t& remoteAddress,
                                     int msgType) {
  if (!idverify(static_cast<int>(localState) >
                static_cast<int>(state_t::STATE_IDLE))) {
    idLib::Printf(
        "NET: Got disconnected from host %s on session %s when we were not in "
        "a lobby or game.\n",
        remoteAddress.ToString().c_str(), lobby.GetLobbyName().c_str());
    MoveToMainMenu();
    return;  // Ignore if we are not past the main menu
  }

  // Goodbye from host.  See if we were connecting vs connected
  if ((localState == state_t::STATE_CONNECT_AND_MOVE_TO_PARTY ||
       localState == state_t::STATE_CONNECT_AND_MOVE_TO_GAME) &&
      lobby.peers[peerNum].GetConnectionState() ==
          idLobby::connectionState_t::CONNECTION_CONNECTING) {
    // We were denied a connection attempt
    idLib::Printf(
        "NET: Denied connection attempt from host %s on session %s. MsgType "
        "%i.\n",
        remoteAddress.ToString().c_str(), lobby.GetLobbyName().c_str(),
        msgType);
    // This will try to move to the next connection if one exists, otherwise
    // will create a match
    ...
    idLib::Printf("NET: Disconnected from host %s on session %s. MsgType %i.\n",
                  remoteAddress.ToString().c_str(),
                  lobby.GetLobbyName().c_str(), msgType);
...
void idSessionLocalWin::Initialize() noexcept {
  ...
  GetPartyLobby().Initialize(idLobby::lobbyType_t::TYPE_PARTY,
                             sessionCallbacks.get());
  GetGameLobby().Initialize(idLobby::lobbyType_t::TYPE_GAME,
                            sessionCallbacks.get());
  GetGameStateLobby().Initialize(idLobby::lobbyType_t::TYPE_GAME_STATE,
                                 sessionCallbacks.get());
}
...
void idSessionLocalWin::Shutdown() noexcept {
  ...
  while (localState != state_t::STATE_IDLE &&
         localState != state_t::STATE_PRESS_START) {
    Pump();
  }
}
...
void idSessionLocalWin::EnsurePort() {
  ...
  if (port.InitPort(net_port.GetInteger(), false)) {
    ...
    // Assume this is another instantiation on the same machine, and just init
    // using any available port
...
========================
*/
idLobbyBackend* idSessionLocalWin::CreateLobbyBackend(
...
========================
*/
idLobbyBackend* idSessionLocalWin::FindLobbyBackend(
...
========================
*/
idLobbyBackend* idSessionLocalWin::JoinFromConnectInfo(
...
void idSessionLocalWin::PumpLobbies() {
  idassert(lobbyBackends[idLobbyBackend::TYPE_PARTY] == nullptr ||
           lobbyBackends[idLobbyBackend::TYPE_PARTY]->GetLobbyType() ==
               idLobbyBackend::TYPE_PARTY);
  idassert(lobbyBackends[idLobbyBackend::TYPE_GAME] == nullptr ||
           lobbyBackends[idLobbyBackend::TYPE_GAME]->GetLobbyType() ==
               idLobbyBackend::TYPE_GAME);
  idassert(lobbyBackends[idLobbyBackend::TYPE_GAME_STATE] == nullptr ||
           lobbyBackends[idLobbyBackend::TYPE_GAME_STATE]->GetLobbyType() ==
               idLobbyBackend::TYPE_GAME_STATE);
...
========================
*/
idLobbyBackend* idSessionLocalWin::CreateLobbyInternal(
    idLobbyBackend::lobbyBackendType_t lobbyType) {
  EnsurePort();
  std::unique_ptr<idLobbyBackend> lobbyBackend =
      std::make_unique<idLobbyBackendDirect>();
...
========================
*/
bool lobbyAddress_t::Compare(const lobbyAddress_t& addr,
                             bool ignoreSessionCheck) const {
...
========================
*/
bool idNetSessionPort::ReadRawPacket(lobbyAddress_t& from, void* data,
...
========================
*/
void idNetSessionPort::SendRawPacket(const lobbyAddress_t to, const void* data,
                                     int size) {
...
========================
*/
bool idNetSessionPort::IsOpen() { return UDP.IsOpen(); }
...
========================
*/
void idNetSessionPort::Close() { UDP.Close(); }