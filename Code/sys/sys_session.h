...
enum class matchFlags_t {
  ...
  MATCH_PRIVATE =
      BIT(3),  // Match will NOT be searchable through FindOrCreateMatch
  ...
  MATCH_PARTY_INVITE_PLACEHOLDER =
      BIT(6),  // Party is never shown in the UI, it's simply used as a
               // placeholder for invites
  MATCH_JOIN_IN_PROGRESS = BIT(7),  // Join in progress supported for this match
};

inline bool MatchTypeIsOnline(uint8_t matchFlags) {
  return (matchFlags & static_cast<int>(matchFlags_t::MATCH_ONLINE)) ? true
                                                                     : false;
}

const int8_t GAME_MAP_RANDOM = -1;

const int DefaultPartyFlags =
    static_cast<int>(matchFlags_t::MATCH_JOIN_IN_PROGRESS) |
    static_cast<int>(matchFlags_t::MATCH_ONLINE);
const int DefaultPublicGameFlags =
    static_cast<int>(matchFlags_t::MATCH_JOIN_IN_PROGRESS) |
    static_cast<int>(matchFlags_t::MATCH_REQUIRE_PARTY_LOBBY) |
    static_cast<int>(matchFlags_t::MATCH_RANKED) |
    static_cast<int>(matchFlags_t::MATCH_STATS);
const int DefaultPrivateGameFlags =
    static_cast<int>(matchFlags_t::MATCH_JOIN_IN_PROGRESS) |
    static_cast<int>(matchFlags_t::MATCH_REQUIRE_PARTY_LOBBY) |
...
class idMatchParameters {
 public:
  idMatchParameters() : gameMap(GAME_MAP_RANDOM), matchFlags(0) {}

  int8_t gameMap;
  uint8_t matchFlags;

  std::string
      mapName;  // This is only used for SP (gameMap == GAME_MAP_SINGLEPLAYER)
};
...
struct lobbyUserID_t {
 public:
  ...
  explicit lobbyUserID_t(uint32_t localUser_, std::byte lobbyType_)
      : localUserHandle(localUser_), lobbyType(lobbyType_) {}

  bool operator==(const lobbyUserID_t& other) const {
    return localUserHandle == other.localUserHandle &&
           lobbyType == other.lobbyType;  // Lobby type must match
  }
  ...
  */
  ...
  bool IsValid() const {
    return localUserHandle > 0 && lobbyType != std::byte{0xFF};
  }
...
class idSession {
 public:
  ...
  virtual void EndMatch(
      bool premature = false) = 0;  // Meant for host to end match gracefully,
                                    // go back to lobby, tally scores, etc
  virtual void QuitMatchToTitle() = 0;  // Will forcefully quit the match and
  ...
  // able to host the in-game lobby.
  virtual idLobbyBase& GetActingGameStateLobbyBase() = 0;

  // GetActivePlatformLobbyBase will return either the game or party lobby, it
  // won't return the game state lobby This function is generally used for
  // menus, in-game code should refer to GetActingGameStateLobby
