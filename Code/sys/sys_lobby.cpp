...
========================
*/
void idLobby::Initialize(lobbyType_t sessionType_,
                         idSessionCallbacks* callbacks) {
  ...
  if (lobbyType == GetActingGameStateLobbyType()) {
    ...
    // lzwData = (lzwCompressionData_t*)Mem_Alloc(sizeof(lzwCompressionData_t),
    // TAG_NETWORKING);
  }
}
...
void idLobby::StartHosting(const idMatchParameters& parms_) {
  ...
  // Get the skill level of all the players that will eventually go into the
  // lobby
  StartCreating();
}
...
void idLobby::StartFinding(const idMatchParameters& parms_) {
  ...
  lobbyBackend = sessionCB->FindLobbyBackend(
      parms, sessionCB->GetPartyLobby().GetNumLobbyUsers(), 0,
...
void idLobby::Pump() {
  ...
  switch (state) {
    case lobbyState_t::STATE_IDLE:
      State_Idle();
      break;
    case lobbyState_t::STATE_CREATE_LOBBY_BACKEND:
      State_Create_Lobby_Backend();
      break;
    case lobbyState_t::STATE_SEARCHING:
      State_Searching();
      break;
    case lobbyState_t::STATE_OBTAINING_ADDRESS:
      State_Obtaining_Address();
      break;
    case lobbyState_t::STATE_CONNECT_HELLO_WAIT:
      State_Connect_Hello_Wait();
      break;
    case lobbyState_t::STATE_FINALIZE_CONNECT:
      State_Finalize_Connect();
      break;
    case lobbyState_t::STATE_FAILED:
      break;
    default:
      idLib::Error("idLobby::Pump:  Unknown state: %s",
...
void idLobby::Shutdown(bool retainMigrationInfo, bool skipGoodbye) {
  ...
  if (!lobbyBackend) {
    ...
    // If we don't have this lobbyBackend type, we better be properly shutdown
    // for this lobby
    idassert(GetNumLobbyUsers() == 0);
    idassert(host == -1);
    ...
    // !respondToArbitrate); idassert(snapDeltaAckQueue.Num() == 0);
  ...
  //		SetPeerConnectionState(p, CONNECTION_FREE, skipGoodbye);
  //// This will send goodbye's
  //	}
  // }

  //// Remove any users that weren't handled in ResetPeers
  //// (this will happen as a client, because we won't get the reliable msg from
  /// the server since we are severing the connection)
...
========================
*/
void idLobby::HandlePacket(lobbyAddress_t& remoteAddress, idBitMsg& fragMsg,
  ...
  if (peerNum >= 0) {
    if (!peers[peerNum].IsActive()) {
      idLib::Printf(
          "NET: Received in-band packet from peer %s with no active "
          "connection.\n",
          remoteAddress.ToString());
      return;
    }
    type = peers[peerNum].packetProc->ProcessIncoming(
        Sys_Milliseconds(), peers[peerNum].sessionID, fragMsg, msg, userData,
        peerNum);
  } else {
    if (!idPacketProcessor::ProcessConnectionlessIncoming(fragMsg, msg,
                                                          userData)) {
  ...
  if (type == idPacketProcessor::RETURN_TYPE_NONE) {
    // This packet is not necessarily invalid, it could be a start or middle of
    // a fragmented packet that's not fully constructed.
    return;
  }

  // Handle server query requests.  We do this before the STATE_IDLE check. This
  // is so we respond. We may want to change this to just ignore the request if
  // we are idle, and change the timeout time on the requesters part to just
  // timeout faster.
  /*if (type == idPacketProcessor::RETURN_TYPE_OOB) {
          if (userData == OOB_MATCH_QUERY || userData == OOB_SYSTEMLINK_QUERY) {
                  sessionCB->HandleServerQueryRequest(remoteAddress, msg,
  userData); return;
          }
  ...
  if (type == idPacketProcessor::RETURN_TYPE_OOB) {
    if (userData == OOB_HELLO) {
      ...
      return;
    } else if (userData == OOB_GOODBYE || userData == OOB_GOODBYE_W_PARTY ||
               userData == OOB_GOODBYE_FULL) {
    ...
    for (int r = 0; r < numReliable; r++) {
      ...
      // (this can happen when our party/game host is the same, he quits the
      // game lobby, and sends a reliable msg for us to leave the game)
      peerNum = FindPeer(remoteAddress, sessionID);

      if (peerNum == -1) {
        idLib::Printf(
            "NET: Dropped peer while processing reliable msg's: %i, %s\n",
    ...
    if (peerNum == -1 || !peers[peerNum].IsConnected()) {
      // If the peer still has no connection after HandleReliableMsg, then
      // something is wrong. (We could have been in CONNECTION_CONNECTING state
      // for this session type, but the first message we should receive from the
      // server is the ack, otherwise, something went wrong somewhere)
      idLib::Printf(
          "NET: In-band message from host with no active connection: %i, %s\n",
          peerNum, remoteAddress.ToString().c_str());
      return;
    }
    ...
    if (msg.GetRemainingData() > 0 && loaded) {
      ...
      if (peerNum == host) {
        ...
        // If we are the peer, we assume we only receive snapshot data on the
        // in-band channel
        const std::byte* deltaData = msg.GetReadData() + msg.GetReadCount();
        int deltaLength = msg.GetRemainingData();

        if (peers[peerNum].snapProc->ReceiveSnapshotDelta(
                deltaData, deltaLength, sequence, baseseq, localSnap,
                fullSnap)) {
          NET_VERBOSESNAPSHOT_PRINT_LEVEL(
              2, va("NET: Got %s snapshot %d delta'd against %d. SS Time: %d\n",
                    (!fullSnap ? "partial" : "full"), sequence, baseseq,
                    localSnap.GetTime())
                     .c_str());
          ...
          //	// When we aren't in the game, we need to send this as reliable
          // msg's, since usercmds won't be taking care of it for us
          //	QueueReliableMessage(host, RELIABLE_SNAPSHOT_ACK, ackbuffer,
          // sizeof(ackbuffer));
          //}
        }
...
const std::string
    idLobby::stateToString[static_cast<int>(lobbyState_t::NUM_STATES)] = {
...
void idLobby::State_Idle() {
  // If lobbyBackend is in a failed state, shutdown, go to a failed state
  // ourself, and return
  if (lobbyBackend && lobbyBackend->GetState() ==
                          idLobbyBackend::lobbyBackendState_t::STATE_FAILED) {
...
void idLobby::State_Create_Lobby_Backend() {
  ...
  if (lobbyBackend->GetState() ==
      idLobbyBackend::lobbyBackendState_t::STATE_CREATING) {
    return;  // Busy but valid
  }

  if (lobbyBackend->GetState() !=
      idLobbyBackend::lobbyBackendState_t::STATE_READY) {
    SetState(lobbyState_t::STATE_FAILED);
    return;
  }
...
void idLobby::State_Searching() {
  ...
  if (lobbyBackend->GetState() ==
      idLobbyBackend::lobbyBackendState_t::STATE_SEARCHING) {
    return;  // Busy but valid
  }
  ...
  if (lobbyBackend->GetState() !=
      idLobbyBackend::lobbyBackendState_t::STATE_READY) {
    SetState(
        lobbyState_t::STATE_FAILED);  // Any other lobbyBackend state is invalid
    return;
  }
  ...
  const int maxSearchResultsToTry =
  session->GetTitleStorageInt("net_maxSearchResultsToTry",
...
void idLobby::State_Obtaining_Address() {
  if (lobbyBackend->GetState() ==
      idLobbyBackend::lobbyBackendState_t::STATE_OBTAINING_ADDRESS) {
  ...
  if (lobbyBackend->GetState() !=
    ...
    NET_VERBOSE_PRINT(
        "idLobby::State_Obtaining_Address: the lobby backend failed.");
    SetState(lobbyState_t::STATE_FAILED);
    return;
  }
  ...
  // Send first connect attempt now (we'll send more periodically if we fail to
  // receive an ack) (we do this after changing state, since the function
  // expects we're in the right state)
...
void idLobby::State_Connect_Hello_Wait() {
  if (lobbyBackend->GetState() !=
    ...
    NET_VERBOSE_PRINT(
        "NET: Lobby is no longer ready while waiting for lobbyType %s hello.\n",
        GetLobbyName().c_str());
    HandleConnectionAttemptFailed();
    return;
  }
  ...
  if (connectionAttempts < MAX_CONNECT_ATTEMPTS) {
    idassert(connectionAttempts >=
             1);  // Should have at least the initial connection attempt

    // See if we need to send another hello request
    // (keep getting more frequent to increase chance due to possible packet
    // loss, but clamp to MIN_CONNECT_FREQUENCY seconds)
    // TODO: We could eventually make timing out a function of actual number of
    // attempts rather than just plain time.
    int resendTime =
        std::max(MIN_CONNECT_FREQUENCY_IN_SECONDS,
...
void idLobby::SetState(lobbyState_t newState) {
  idassert(static_cast<int>(newState) <
           static_cast<int>(lobbyState_t::NUM_STATES));
  idassert(static_cast<int>(state) <
           static_cast<int>(lobbyState_t::NUM_STATES));

  if (state == newState) {
    NET_VERBOSE_PRINT("NET: idLobby::SetState: State SAME %s for session %s\n",
                      stateToString[static_cast<int>(newState)],
                      GetLobbyName().c_str());
    return;
  }
  ...
  NET_VERBOSE_PRINT(
      "NET: idLobby::SetState: State changing from %s to %s for session %s\n",
      stateToString[static_cast<int>(state)].c_str(),
...
void idLobby::StartCreating() {
  ...
  lobbyBackend = sessionCB->CreateLobbyBackend(
      parms, 0, (idLobbyBackend::lobbyBackendType_t)lobbyType);
...
========================
*/
int idLobby::FindPeer(const lobbyAddress_t& remoteAddress,
                      idPacketProcessor::sessionId_t sessionID,
                      bool ignoreSessionID) {
  bool connectionless =
      (sessionID == idPacketProcessor::SESSION_ID_CONNECTIONLESS_PARTY ||
  ...
  if (connectionless && !ignoreSessionID) {
    return -1;  // This was meant to be connectionless. FindPeer is meant for
  ...
  for (int p = 0; p < peers.size(); p++) {
    ...
    if (peers[p].address.Compare(remoteAddress)) {
      ...
      // Using a rolling check, so that we account for possible packet loss, and
      // out of order issues
      if (IsPeer()) {
        idPacketProcessor::sessionId_t searchStart = peers[p].sessionID;

        //	// Since we only roll the code between matches, we should only
        // need to look ahead a couple increments.
        //	// Worse case, if the stars line up, the client doesn't see the
        // new sessionId, and times out, and gets booted.
        //	// This should be impossible though, since the timings won't be
        // possible considering how long it takes to end the match,
        ...
        //			idLib::Printf("NET: Rolling session ID check
        // found new ID: %i\n", searchStart); 			if
        // (peers[p].packetProc != NULL) {
        //				peers[p].packetProc->VerifyEmptyReliableQueue(RELIABLE_GAME_DATA,
        // RELIABLE_DUMMY_MSG);
        //			}
...
========================
*/
int idLobby::AddPeer(const lobbyAddress_t& remoteAddress,
                     idPacketProcessor::sessionId_t sessionID) {
  // First, make sure we don't already have this peer
  int p = FindPeer(remoteAddress, sessionID);
  idassert(p == -1);  // When using session ID's, we SHOULDN'T find this
                      // remoteAddress/sessionID combo
...
========================
*/
void idLobby::SetPeerConnectionState(int p, connectionState_t newState,
                                     bool skipGoodbye) {
  ...
  if (peer.GetConnectionState() == newState) {
    idLib::Printf("NET: SetPeerConnectionState: Peer already in state %i\n",
                  newState);
    idassert(0);  // This case means something is most likely bad, and it's the
                  // programmers fault
    return;
  }

  if (newState == connectionState_t::CONNECTION_CONNECTING) {
    // We better be coming from a free connection state if we are trying to
    // connect
    ...
    // If we are marking this peer as connected for the first time, make sure
    // this peer was actually trying to connect.
    idassert(peer.GetConnectionState() ==
             connectionState_t::CONNECTION_CONNECTING);
  } else if (newState == connectionState_t::CONNECTION_FREE) {
    // If we are freeing this connection and we had an established connection
    // before, make sure to send a goodbye
    if (peer.GetConnectionState() ==
            connectionState_t::CONNECTION_ESTABLISHED &&
        !skipGoodbye) {
      idLib::Printf(
          "SetPeerConnectionState: Sending goodbye to peer %s from session "
          "%s\n",
...
========================
*/
void idLobby::QueueReliableMessage(int p, reliableType_t type,
                                   const std::byte* data, int dataLen) {
  if (!idverify(p >= 0 && p < peers.size())) {
    return;
  }
  ...
  if (!peer.IsConnected()) {
    // Don't send to this peer if we don't have an established connection of
    // this session type
    NET_VERBOSE_PRINT(
        "NET: Not sending reliable type %i to peer %i because connectionState "
        "is %i\n",
  ...
  if (peer.packetProc->NumQueuedReliables() > 2) {
    idLib::Printf(
        "NET: peer.packetProc->NumQueuedReliables() > 2: %i (%i / %s)\n",
        peer.packetProc->NumQueuedReliables(), p,
        peer.address.ToString().c_str());
  }

  if (!peer.packetProc->QueueReliableMessage(static_cast<std::byte>(type), data,
                                             dataLen)) {
    // For now, when this happens, disconnect from all session types
    NET_VERBOSE_PRINT(
        "NET: Dropping peer because we overflowed his reliable message "
        "queue\n");
    if (IsHost()) {
...
========================
*/
// void idLobby::SendConnectionLess(const lobbyAddress_t& remoteAddress, char
// type, const std::byte* data, int dataLen) {
//
...
========================
*/
void idLobby::SendConnectionLess(const lobbyAddress_t& remoteAddress,
                                 std::byte type, const std::byte* data,
                                 int dataLen) {
  ...
  idPacketProcessor::ProcessConnectionlessOutgoing(
      msg, processedMsg, static_cast<int>(lobbyType), static_cast<int>(type));

  const bool useDirectPort = (lobbyType == lobbyType_t::TYPE_GAME_STATE);

  // Send it
  sessionCB->SendRawPacket(remoteAddress, processedMsg.GetReadData(),
                           processedMsg.GetSize(), useDirectPort);
}
...
void idLobby::SendConnectionRequest() {
  ...
  idassert(peers[host].GetConnectionState() ==
           connectionState_t::CONNECTION_CONNECTING);
  idassert(GetNumLobbyUsers() == 0);
  ...
  // We use InitSessionUsersFromLocalUsers here to copy the current local users
  // over to session users simply to have a list to send on the initial
  // connection attempt.  We immediately clear our session user list once sent.
  ...
  // We just used these users to fill up the msg above, we will get the real
  // list from the server if we connect.
  FreeAllUsers();

  NET_VERBOSE_PRINT(
      "NET: Sending hello to: %s (lobbyType: %s, session ID %i, attempt: %i)\n",
      hostAddress.ToString().c_str(), GetLobbyName().c_str(),
      peers[host].sessionID, connectionAttempts);

  SendConnectionLess(hostAddress, static_cast<std::byte>(OOB_HELLO),
...
Fires off a request to get the address of a lobbyBackend owner, and then
attempts to connect (eventually handled in HandleObtainingLobbyOwnerAddress)
========================
*/
void idLobby::ConnectTo(const lobbyConnectInfo_t& connectInfo,
                        bool fromInvite) {
  ...
  lobbyBackend = sessionCB->JoinFromConnectInfo(
      connectInfo, (idLobbyBackend::lobbyBackendType_t)lobbyType);
...
========================
*/
void idLobby::HandleGoodbyeFromPeer(int peerNum, lobbyAddress_t& remoteAddress,
                                    int msgType) {
  if (peerNum < 0) {
  ...
  if (peers[peerNum].GetConnectionState() ==
      connectionState_t::CONNECTION_FREE) {
    NET_VERBOSE_PRINT(
        "NET: Goodbye from peer %s on session %s that is not connected\n",
        remoteAddress.ToString().c_str(), GetLobbyName().c_str());
    return;
  }
...
bool idLobby::ConnectToNextSearchResult() {
  ...
  // If we are connecting to a game lobby, tell our party to connect to this
  // lobby as well
  if (lobbyType == lobbyType_t::TYPE_GAME &&
      sessionCB->GetPartyLobby().IsLobbyActive()) {
    sessionCB->GetPartyLobby().SendMembersToLobby(lobbyType, connectInfo, true);
  }

  // Attempt to connect the lobby
  ConnectTo(connectInfo, true);  // Pass in true for invite, since searches are
                                 // for matchmaking, and we should always be
                                 // able to connect to those types of matches
...
========================
*/
int idLobby::HandleInitialPeerConnection(idBitMsg& msg,
                                         const lobbyAddress_t& peerAddress,
                                         int peerNum) {
  if (!IsHost()) {
    NET_VERBOSE_PRINT(
        "NET: Got connectionless hello from peer %s on session, and we are not "
        "a host\n",
  ...
  // if (!MatchTypeIsJoinInProgress(parms.matchFlags) && lobbyType ==
  // lobbyType_t::TYPE_GAME) {
  //	// No matter what, don't let people join migrated game sessions that are
  // going to continue on to the same game
  //	// Not on invite list in a migrated game session - bounce him
  //	NET_VERBOSE_PRINT("NET: Denying game connection from %s since not on
  // migration invite list\n", peerAddress.ToString().c_str());
  //	SendGoodbye(peerAddress);
  //	return -1;
  // }
  ...
  if (true) {
    ...
    if ((lobbyType == lobbyType_t::TYPE_GAME ||
         lobbyType == lobbyType_t::TYPE_GAME_STATE) &&
        sessionCB->GetState() < idSession::sessionState_t::GAME_LOBBY) {
      NET_VERBOSE_PRINT(
          "NET: Denying game connection from %s because we don't have a game "
          "lobby\n",
          peerAddress.ToString().c_str());
      SendGoodbye(peerAddress);
      return -1;
  ...
  peerAddress.ToString()); SendGoodbye(peerAddress); return -1;
  }*/
  ...
  // If the peer got abruptly disconnected, the peer could be trying to
  // reconnect from a non clean disconnect
  if (peerNum >= 0) {
    peer_t& existingPeer = peers[peerNum];

    idassert(existingPeer.GetConnectionState() !=
             connectionState_t::CONNECTION_FREE);

    if (existingPeer.sessionID == sessionID) {
      return peerNum;  // If this is the same sessionID, then assume redundant
                       // connection attempt
    }
    ...
    // We want to set the connection back to FREE manually, so we don't send a
    // goodbye
    ...
    if (existingPeer.snapProc != NULL) {
      idassert(lobbyType ==
               lobbyType_t::TYPE_GAME);  // Only games sessions should be
                                         // creating snap processors
  ...
  // Calling AddPeer will set our connectionState to this peer as
  // CONNECTION_CONNECTING (which will get set to CONNECTION_ESTABLISHED below)
  peerNum = AddPeer(peerAddress, sessionID);

  peer_t& newPeer = peers[peerNum];

  idassert(newPeer.GetConnectionState() ==
           connectionState_t::CONNECTION_CONNECTING);
  ...
  *proto_out_msg.mutable_matchparams() = proto_match_params;
  ...
  // (the users from the new peer will also be in this list, since we already
  // called AddUsersFromMsg)
  ...
  QueueReliableMessage(peerNum, reliableType_t::RELIABLE_HELLO,
                       outmsg.GetReadData(), outmsg.GetSize());

  if (true /*MatchTypeIsJoinInProgress( parms.matchFlags )*/) {
    // We are are ingame, then start the client loading immediately
    if ((lobbyType == lobbyType_t::TYPE_GAME ||
         lobbyType == lobbyType_t::TYPE_GAME_STATE) &&
        sessionCB->GetState() >= idSession::sessionState_t::LOADING) {
      idLib::Printf("******* JOIN IN PROGRESS ********\n");
      if (sessionCB->GetState() == idSession::sessionState_t::INGAME) {
        // newPeer.pauseSnapshots = true;		// Since this player
        // joined in progress, let game dictate when to start sending snaps
      }
      QueueReliableMessage(peerNum,
                           idLobby::reliableType_t::RELIABLE_START_LOADING);
    }
  }
...
void idLobby::InitStateLobbyHost() {
  ...
  /*const int myUserIndex =
  GetLobbyUserIndexByLocalUserHandle(sessionCB->GetSignInManager().GetMasterLocalUserHandle());
  if (myUserIndex != -1) {
          hostAddress = GetLobbyUser(myUserIndex)->address;
  }*/

  // Since we are the host, we have to register our initial session users with
  // the lobby All additional users will join through AddUsersFromMsg, and
  // RegisterUser is handled in there from here on out. Peers will add users
  // exclusively through AddUsersFromMsg.
  /*for (int i = 0; i < GetNumLobbyUsers(); i++) {
          lobbyUser_t* user = GetLobbyUser(i);
          RegisterUser(user);
...
========================
*/
void idLobby::SendMembersToLobby(lobbyType_t destLobbyType,
                                 const lobbyConnectInfo_t& connectInfo,
                                 bool waitForOtherMembers) {
  // It's not our job to send party members to a game if we aren't the party
  // host
  if (!IsHost()) {
    return;
  }
  ...
  for (int i = 0; i < peers.size(); i++) {
    if (peers[i].IsConnected()) {
      SendPeerMembersToLobby(i, destLobbyType, connectInfo,
                             waitForOtherMembers);
    }
  }
}
...
Give the address of a game lobby to a particular peer, notifying that peer to
send a hello to the same server.
========================
*/
void idLobby::SendPeerMembersToLobby(int peerIndex, lobbyType_t destLobbyType,
                                     const lobbyConnectInfo_t& connectInfo,
                                     bool waitForOtherMembers) {
  // It's not our job to send party members to a game if we aren't the party
  // host
  ...
  if (!peer.IsConnected()) {
    idLib::Warning("NET: Can't send peer %i to game lobby: peer isn't in party",
                   peerIndex);
    return;
  }
}
...
idPacketProcessor::sessionId_t idLobby::EncodeSessionID(uint32_t key) const {
  ...
  const int numBits = sizeof(idPacketProcessor::sessionId_t) * 8 -
                      idPacketProcessor::NUM_LOBBY_TYPE_BITS;
  const uint32_t mask = (1 << numBits) - 1;
  idPacketProcessor::sessionId_t sessionID =
      (key & mask) << idPacketProcessor::NUM_LOBBY_TYPE_BITS;
...
========================
*/
void idLobby::DecodeSessionID(idPacketProcessor::sessionId_t sessionID,
                              uint32_t& key) const {
  idassert(sizeof(uint32_t) >= sizeof(idPacketProcessor::sessionId_t));
...
idPacketProcessor::sessionId_t idLobby::GenerateSessionID() const {
  idPacketProcessor::sessionId_t sessionID =
      EncodeSessionID(Sys_Milliseconds());
...
========================
*/
bool idLobby::SessionIDCanBeUsedForInBand(
    idPacketProcessor::sessionId_t sessionID) const {
...
#define VERIFY_CONNECTED_PEER(p, sessionType_, msgType)                  \
  if (!idverify(lobbyType == sessionType_)) {                            \
    idLib::Printf("NET: " #msgType                                       \
                  ", peer:%s invalid session type for " #sessionType_    \
                  " %i.\n",                                              \
                  peer.address.ToString(), sessionType_);                \
    return;                                                              \
  }                                                                      \
  if (peers[p].GetConnectionState() !=                                   \
      connectionState_t::CONNECTION_ESTABLISHED) {                       \
    idLib::Printf("NET: " #msgType                                       \
                  ", peer:%s not connected for " #sessionType_ " %i.\n", \
                  peer.address.ToString(), sessionType_);                \
    return;                                                              \
  }
...
#define VERIFY_CONNECTING_PEER(p, sessionType_, msgType)                  \
  ...
  if (peers[p].GetConnectionState() !=                                    \
      connectionState_t::CONNECTION_CONNECTING) {                         \
    idLib::Printf("NET: " #msgType                                        \
                  ", peer:%s not connecting for " #sessionType_ " %i.\n", \
                  peer.address.ToString(), sessionType_);                 \
    return;                                                               \
  }
...
void idLobby::HandleHelloAck(int p, idBitMsg& msg) {
  ...
  if (state != lobbyState_t::STATE_CONNECT_HELLO_WAIT) {
    idLib::Printf(
        "NET: Hello ack for session type %s while not waiting for hello.\n",
        GetLobbyName().c_str());
    SendGoodbye(peer.address);  // We send a customary goodbye to make sure we
                                // are not in their list anymore
    return;
  }
  if (p != host) {
    // This shouldn't be possible
    idLib::Printf(
        "NET: Hello ack for session type %s, not from correct host.\n",
        GetLobbyName());
    SendGoodbye(peer.address);  // We send a customary goodbye to make sure we
                                // are not in their list anymore
    return;
  ...
  // Populate the user list with the one from the host (which will also include
  // our local users) This ensures the user lists are kept in sync
  FreeAllUsers();
  ...
  //
  // Success - We've received an ack from the server, letting us know we've been
  // registered with the lobbies
  //
}
...
void idLobby::HandleReliableMsg(int p, idBitMsg& msg) {
  ...
  //	// This message is sent from a peer requesting for a new user to join
  // the game lobby
  //	// This will be sent while we are in a game lobby as a host.  otherwise,
  // denied. 	NET_VERBOSE_PRINT("NET: RELIABLE_USER_CONNECT_REQUEST (%s) from
  //%s\n", GetLobbyName().c_str(), peer.address.ToString().c_str());

  //	idSession::sessionState_t expectedState = (lobbyType ==
  // lobbyType_t::TYPE_PARTY) ? idSession::sessionState_t::PARTY_LOBBY :
  // idSession::sessionState_t::GAME_LOBBY;

  //	if (sessionCB->GetState() == expectedState && IsHost() && NumFreeSlots()
  //> 0) {	// This assumes only one user in the msg
  //		// Add user to session, which will also forward the operation to
  // all other peers 		AddUsersFromMsg(msg, p);
  //	}
  //	else {
  ...
  //	// This message is sent back from the host when a
  // RELIABLE_PARTY_USER_CONNECT_REQUEST failed 	VERIFY_FROM_HOST(p,
  // lobbyType, RELIABLE_PARTY_USER_CONNECT_DENIED);

  //	// Remove this user from the sign-in manager, so we don't keep trying to
  // add them 	if
  //(!sessionCB->GetSignInManager().RemoveLocalUserByHandle(localUserHandle_t(msg.ReadLong())))
  //{ 		NET_VERBOSE_PRINT("NET: RELIABLE_PARTY_USER_CONNECT_DENIED,
  // local user not found\n"); 		return;
  //	}
  //}
  // else if (reliableType == RELIABLE_KICK_PLAYER) {
  //	VERIFY_FROM_HOST(p, lobbyType, RELIABLE_KICK_PLAYER);
  //	common->Dialog().AddDialog(GDM_KICKED, DIALOG_ACCEPT, NULL, NULL,
  // false); 	if (sessionCB->GetPartyLobby().IsHost()) {
  ...
  //	// This message is sent back from the host when users have connected,
  // and we need to update our lists to reflect that 	VERIFY_FROM_HOST(p,
  // lobbyType, RELIABLE_USER_CONNECTED);

  //	NET_VERBOSE_PRINT("NET: RELIABLE_USER_CONNECTED (%s) from %s\n",
  // GetLobbyName(), peer.address.ToString()); 	AddUsersFromMsg(msg, p);
  //}
  // else if (reliableType == RELIABLE_USER_DISCONNECTED) {
  //	// This message is sent back from the host when users have diconnected,
  // and we need to update our lists to reflect that 	VERIFY_FROM_HOST(p,
  // lobbyType, RELIABLE_USER_DISCONNECTED);
  ...
  //}
    ...
    // This message is sent from the peers to state they are done loading the
    // map
  ...
  //	VERIFY_CONNECTED_PEER(p, actingGameStateLobbyType,
  // RELIABLE_SNAPSHOT_ACK);

  //	// update our base state for his last received snapshot
  //	int snapNum = msg.ReadLong();
  //	float receivedBps = msg.ReadQuantizedUFloat< BANDWIDTH_REPORTING_MAX,
  // BANDWIDTH_REPORTING_BITS >();

  //	// Update reported received bps
  //	if (peer.receivedBpsIndex != snapNum) {
  //		// Only do this the first time we get reported bps per snapshot.
  // Subsequent ACKs of the same shot will usually have lower reported bps
  ...
  //	NET_VERBOSESNAPSHOT_PRINT_LEVEL(2, va("NET: Peer %d Ack'd snapshot
  //%d\n", p, snapNum));
  ...
  //	VERIFY_FROM_HOST(p, actingGameStateLobbyType,
  // RELIABLE_ENDMATCH_PREMATURE);

  //	sessionCB->EndMatchInternal(true);

  //}
  // else if (reliableType == RELIABLE_START_MATCH_GAME_LOBBY_HOST) {
  //	// This message should be from the host of the game lobby, telling us
  //(as the host of the GameStateLobby) to start loading
  //	VERIFY_CONNECTED_PEER(p, TYPE_GAME_STATE,
  // RELIABLE_START_MATCH_GAME_LOBBY_HOST);

  //	if (session->GetState() >= idSession::LOADING) {
  //		NET_VERBOSE_PRINT("NET: RELIABLE_START_MATCH_GAME_LOBBY_HOST
  // already loading\n"); 		return;
  //	}
  ...
  //	// Set a flag to do this later, since the lobby may not be in a state
  // where it can fulfil the request at the moment 	respondToArbitrate =
  // true;
  //}
  ...
  //		NET_VERBOSE_PRINT("NET: Everyone says they registered for
  // arbitration, verifying\n"); 		lobbyBackend->Arbitrate();
  ...
  //	if (IsHost()) {
  //		idLib::Printf("RELIABLE_CONNECT_AND_MOVE_TO_LOBBY: We are the
  // host.\n"); 		return;
  //	}
  ...
  //	idassert(destLobbyType > lobbyType);		// Make sure this is a
  // proper transition (i.e. TYPE_PARTY moves to TYPE_GAME, TYPE_GAME moves to
  // TYPE_GAME_STATE)

  //	sessionCB->ConnectAndMoveToLobby(destLobbyType, connectInfo,
  // waitForMembers);
  //}
  ...
  //		idLib::Printf("RELIABLE_PARTY_CONNECT_OK: Wasn't waiting for
  // ok.\n");
  //	}
  ...
  //		idLib::Printf("RELIABLE_PARTY_LEAVE_GAME_LOBBY: Not in a game
  // lobby, ignoring.\n"); 		return;
  //	}

  //	if (IsHost()) {
  //		idLib::Printf("RELIABLE_PARTY_LEAVE_GAME_LOBBY: Host of party,
  // ignoring.\n"); 		return;
  //	}
...
========================
idLobby::PumpPings
Host: Periodically determine the round-trip time for a packet to all peers, and
tell everyone what everyone else's ping to the host is so they can display it in
the UI. Client: Indicate to the player when the server hasn't updated the ping
values in too long. This is usually going to preceed a connection timeout.
========================
*/
void idLobby::PumpPings() {
  ...
  //	PingPeers();
  //	// Send the hosts calculated ping values to each peer to everyone has
  // updated ping times 	SendPingValues();
  ...
  //	if (lastPingValuesRecvTime + PING_INTERVAL_MS + 1000 <
  // Sys_Milliseconds() && migrationInfo.state == MIGRATE_NONE) {
  // for (int userIndex = 0; userIndex < GetNumLobbyUsers(); ++userIndex) {
  // lobbyUser_t* user = GetLobbyUser(userIndex); 			if
  // (!verify(user != NULL)) { 				continue;
  //			}
  //			user->pingMs = 999999;
  //		}
  //	}
...
Other than connectionless sends, this should be the chokepoint for sending
packets to peers.
========================
*/
bool idLobby::SendAnotherFragment(int p) {
  ...
  if (!peer.IsConnected()) {  // Not connected to any mode (party or game), so
                              // no need to send
    return false;
  }
  ...
  //	return false;		// We need to throttle the sends so we don't
  ...
  //- peer.lastFragmentSendTime)); 	return false;		// Too soon to
  ...
  while (true) {
    idBitMsg msg;
    // We use the final packet size here because it has been processed, and no
    // more headers will be added
    ...
    msg.BeginReading();
    sessionCB->SendRawPacket(peers[p].address, msg.GetReadData(), msg.GetSize(),
                             useDirectPort);
    sentFragment = true;
...
========================
*/
void idLobby::ProcessOutgoingMsg(int p, const void* data, int size, bool isOOB,
                                 int userData) {
  peer_t& peer = peers[p];
  ...
  if (peer.packetProc->HasMoreFragments()) {
    idLib::Error(
        "FATAL: Attempt to process a packet while fragments still need to be "
  ...
  //	// (used for things like knowing when reliables could have been last
  // sent) 	peer.lastInBandProcTime = peer.lastProcTime;
  // }
...
void idLobby::ResendReliables(int p) {
  ...
  const int DEFAULT_MIN_RESEND = 20;  // Quicker resend while not in game to
                                      // speed up resource transmission acks
  ...
  //	if (lobbyType == TYPE_PARTY) {
  //		resendWait = Max(500, resendWait); // party session does not
  // need fast frequency at all once in game
  //	}
  // }
  // else {
  ...
  //	// no need to resend reliables if they went out on an in-band packet
  // recently 	return;
  // }

  if (peer.packetProc->NumQueuedReliables() > 0 ||
      peer.packetProc->NeedToSendReliableAck()) {
    // NET_VERBOSE_PRINT( "NET: ResendReliables %s\n", GetLobbyName() );
    ProcessOutgoingMsg(p, nullptr, 0, false,
                       0);  // Force an empty unreliable msg so any reliables
...
void idLobby::PumpPackets() {
  ...
  // If we haven't sent anything to our peers in a long time, make sure to send
  // an empty packet (so our heartbeat gets updated) so we don't get
  // disconnected NOTE - We used to only send these to the host, but the host
  // needs to also send these to clients
  // for (int p = 0; p < peers.size(); p++) {
  ...
  // GetLobbyName() ); 		ProcessOutgoingMsg(p, NULL, 0, false, 0);
  //	}
  //}
...
idLobby::lobbyType_t idLobby::GetActingGameStateLobbyType() const {
  ...
  return (net_useGameStateLobby.GetBool()) ? lobbyType_t::TYPE_GAME_STATE
                                           : lobbyType_t::TYPE_GAME;
}
