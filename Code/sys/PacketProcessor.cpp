...
void idPacketProcessor::QueueReliableAck(int lastReliable) {
  // NOTE - Even if it was the last known sequence, go ahead and ack it, in case
  // our last ack for this sequence got dropped
...
================================================
*/
int idPacketProcessor::FinalizeRead(idBitMsg& inMsg, idBitMsg& outMsg,
                                    int& userValue) {
  userValue = 0;
  ...
  if (!idverify(header.Type() !=
                PACKET_TYPE_FRAGMENTED)) {  // We shouldn't be fragmented at
                                            // this point
  ...
  if (header.Type() == PACKET_TYPE_RELIABLE_ACK) {
    ...
    header.ReadFromMsg(inMsg);  // Read the new header, since the reliable ack
                                // sits on top the actual header of the message
  }
  ...
  if (header.Type() == PACKET_TYPE_OOB) {
    ...
    // Reset number of reliables received (NOTE - This means you MUST unload all
    // reliables as they are received)
    numReliable = 0;
    ...
    if (numReliableRecv > 0) {
      ...
      idLib::Printf(
          "NET: got %d msgs reliableSequence %d / reliableSequenceRecv %d\n",
      ...
      for (int r = 0; r < numReliableRecv; r++) {
        ...
        if (reliableSequence + r >
            reliableSequenceRecv) {  // Only accept newer reliable msg's than
                                     // we've currently already received
          if (!idverify(bufferPos + reliableDataLength <=
                        reliableBuffer.size())) {
          ...
          for (size_t i = 0; i < reliableDataLength; ++i)
            *(reliableBuffer.data() + bufferPos + i) =
                static_cast<std::byte>(inMsg.ReadByte());
  ...
  // Load actual msg
  outMsg.BeginWriting();
  outMsg.WriteData(inMsg.GetReadData() + inMsg.GetReadCount(),
                   inMsg.GetRemainingData());
  outMsg.SetSize(inMsg.GetRemainingData());

  return (header.Type() == PACKET_TYPE_OOB) ? RETURN_TYPE_OOB
                                            : RETURN_TYPE_INBAND;
}
...
================================================
*/
bool idPacketProcessor::QueueReliableMessage(std::byte type,
                                             const std::byte* data,
                                             int dataLen) {
...
NOTE - We only compress reliables because we assume everything else has already
been compressed.
================================================
*/
bool idPacketProcessor::ProcessOutgoing(const int time, const idBitMsg& msg,
                                        bool isOOB, int userData) {
  // We can only do ONE ProcessOutgoing call, then we need to do GetSendFragment
  // to COMPLETELY empty unsentMsg before calling ProcessOutgoing again.
  ...
  // Ack reliables if we need to (NOTE - We will send this ack on both the
  ...
  if (isOOB) {
    if (msg.GetSize() + unsentMsg.GetSize() >
        MAX_OOB_MSG_SIZE) {  // Fragmentation not allowed for out-of-band msg's
...
================================================
*/
bool idPacketProcessor::GetSendFragment(const int time, sessionId_t sessionID,
                                        idBitMsg& outMsg) {
  // lastSendTime = time;
  ...
  if (!fragmentedSend) {
    ...
    int currentSize =
        idMath::ClampInt(0, MAX_PACKET_SIZE, unsentMsg.GetRemainingData());
    idassert(currentSize > 0);
    idassert(unsentMsg.GetRemainingData() - currentSize >= 0);

    // See if we'll have more fragments once we subtract off how much we're
    // about to write
    bool moreFragments =
        (unsentMsg.GetRemainingData() - currentSize > 0) ? true : false;

    if (!unsentMsg.GetReadCount()) {  // If this is the first read, then we know
                                      // it's the first fragment
      idassert(moreFragments);  // If we have a first, we must have more or
                                // something went wrong
      idInnerPacketHeader header(PACKET_TYPE_FRAGMENTED, FRAGMENT_START);
      header.WriteToMsg(outMsg);
    } else {
      idInnerPacketHeader header(PACKET_TYPE_FRAGMENTED, moreFragments
                                                             ? FRAGMENT_MIDDLE
                                                             : FRAGMENT_END);
      header.WriteToMsg(outMsg);
    }
    ...
    outMsg.WriteData(unsentMsg.GetReadData() + unsentMsg.GetReadCount(),
                     currentSize);
...
================================================
*/
int idPacketProcessor::ProcessIncoming(int time, sessionId_t expectedSessionID,
                                       idBitMsg& msg, idBitMsg& out,
                                       int& userData, const int peerNum) {
  ...
  if (!idverify(sessionID != SESSION_ID_CONNECTIONLESS_PARTY &&
                sessionID != SESSION_ID_CONNECTIONLESS_GAME &&
                sessionID != SESSION_ID_CONNECTIONLESS_GAME_STATE)) {
    idLib::Printf(
        "Expected non connectionless ID, but got a connectionless one\n");
    return RETURN_TYPE_NONE;
  }
  ...
  if (header.Value() == FRAGMENT_START) {
    ...
    droppedFrags++;
    idLib::Printf(
        "Dropped Fragments - PeerNum: %i FragmentSeq: %i, ReadSeq: %i, Total: "
        "%i\n",
        peerNum, fragmentSequence, readSequence, droppedFrags);

    // If this is the middle or end, make sure we are reading in
    // fragmentSequence
    fragmentSequence = -1;
  ...
  memcpy(msgBuffer.data() + msgWritePos, msg.GetReadData() + msg.GetReadCount(),
  ...
  if (!idverify(header.Value() == FRAGMENT_START ||
                header.Value() == FRAGMENT_MIDDLE)) {
    idLib::Printf("ProcessIncoming: Invalid packet.\n");
  }

  // If we get here, this is part (either beginning or end) of a fragmented
  // packet. We return RETURN_TYPE_NONE to let the caller know they don't need
  // to do anything yet.
  return RETURN_TYPE_NONE;
}
...
================================================
*/
bool idPacketProcessor::ProcessConnectionlessOutgoing(idBitMsg& msg,
                                                      idBitMsg& out,
                                                      int lobbyType,
                                                      int userData) {
...
================================================
*/
bool idPacketProcessor::ProcessConnectionlessIncoming(idBitMsg& msg,
                                                      idBitMsg& out,
                                                      int& userData) {
  idOuterPacketHeader outerHeader;
  outerHeader.ReadFromMsg(msg);
  ...
  if (sessionID != SESSION_ID_CONNECTIONLESS_PARTY &&
      sessionID != SESSION_ID_CONNECTIONLESS_GAME &&
      sessionID != SESSION_ID_CONNECTIONLESS_GAME_STATE) {
    // Not a connectionless msg (this can happen if a previously connected peer
    // keeps sending data for whatever reason)
  ...
  if (header.Type() != PACKET_TYPE_OOB) {
    idLib::Printf(
        "ProcessConnectionlessIncoming: header.Type() != PACKET_TYPE_OOB\n");
