...
#define NET_VERBOSESNAPSHOT_PRINT_LEVEL(X, Y) \
  if (net_verboseSnapshot.GetInteger() >= (X)) idLib::Printf(Y)
...
class idSnapShot {
 public:
  ...
  // Reads a new object state packet, which is assumed to be delta compressed
  // against this snapshot
  bool ReadDeltaForJob(const std::byte* deltaMem, int deltaSize, int& outSeq,
                       int& outBaseSeq);

  // Writes an object state packet which is delta compressed against the old
  // snapshot
  ...
  // Adds an object to the state, overwrites any existing object with the same
  // number
  objectState_t* S_AddObject(int objectNum, const idBitMsg& msg) {
    return S_AddObject(objectNum, msg.GetReadData(), msg.GetSize());
  }
  ...
  int GetObjectMsgByIndex(int i, idBitMsg& msg,
                          bool ignoreIfStale = false) const;
