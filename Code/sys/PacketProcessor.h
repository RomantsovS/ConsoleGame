...
private:
	...
	int numReliable;
	std::array<std::byte, MAX_MSG_SIZE> reliableBuffer; // We shouldn't have to hold more than this
	...
	idBitMsg unsentMsg;
	std::array<std::byte, MAX_MSG_SIZE> unsentBuffer; // Buffer used hold the current msg until it's all sent
};
