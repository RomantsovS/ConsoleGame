...
class idLexer {
 ...
 public:
  ...
  int LoadMemory(const char* ptr, int length, const std::string& name,
                 int startLine = 1);
  // free the script
 ...
 private:
  ...
  const std::vector<punctuation_t>*
      punctuations;                    // the punctuations used in the script
...
inline const int idLexer::GetLineNum() noexcept { return idLexer::line; }
