...
================
*/
void idLexer::CreatePunctuationTable(
    const std::vector<punctuation_t>& punctuations) {
  int i, n, lastp;
  ...
  std::fill((*idLexer::punctuationtable).begin(),
            (*idLexer::punctuationtable).end(), -1);
  // memset(idLexer::nextpunctuation, 0xFF, i * sizeof(int));
  std::fill((*idLexer::nextpunctuation).begin(),
            (*idLexer::nextpunctuation).end(), -1);
...
void idLexer::Error(const char* str, ...) {
  ...
  if (false /*idLexer::flags & LEXFL_NOFATALERRORS*/) {
    common->Warning("file %s, line %d: %s", idLexer::filename.c_str(),
                    idLexer::line, text);
  } else {
    common->Error("file %s, line %d: %s", idLexer::filename.c_str(),
                  idLexer::line, text);
  }
}
...
int idLexer::ReadString(gsl::not_null<idToken*> token, int quote) {
  ...
  while (1) {
    ...
    /*if (*idLexer::script_p == '\\' && !(idLexer::flags &
    LEXFL_NOSTRINGESCAPECHARS)) { if (!idLexer::ReadEscapeCharacter(&ch)) {
                    return 0;
            }
...
int idLexer::ReadName(gsl::not_null<idToken*> token) {
  ...
  do {
    ...
    c = *idLexer::script_p;
      ...
      (/*(idLexer::flags & LEXFL_ALLOWPATHNAMES) &&*/ (c == '/' || c == '\\' ||
                                                       c == ':' || c == '.')));
  // token->data[token->len] = '\0';
...
int idLexer::ReadToken(gsl::not_null<idToken*> token) {
  ...
  if ((c >= '0' && c <= '9') ||
      (c == '.' &&
       (*(idLexer::script_p + 1) >= '0' && *(idLexer::script_p + 1) <= '9'))) {
...
int idLexer::ExpectTokenString(const std::string& string) {
  ...
  if (token != string) {
    idLexer::Error("expected '%s' but found '%s'", string.c_str(),
                   token.c_str());
    return 0;
  }
  return 1;
...
================
*/
int idLexer::ExpectTokenType(int type, int subtype,
                             gsl::not_null<idToken*> token) {
  std::string str;
  ...
  GetPunctuationFromId(subtype), token->c_str()); return 0;
          }
  }*/
  return 1;
}
...
================
*/
int idLexer::CheckTokenType(int type, int subtype,
                            gsl::not_null<idToken*> token) {
  idToken tok;
...
================
*/
int idLexer::LoadMemory(const char* ptr, int length, const std::string& name,
                        int startLine) {
  if (idLexer::loaded) {
...
================
*/
idLexer::~idLexer() { idLexer::FreeSource(); }
