...
=================
*/
std::shared_ptr<idMapBrush> idMapBrush::Parse(idLexer& src,
                                              const Vector2& origin,
                                              bool newFormat, float version) {
  // idVec3 planepts[3];
  idToken token;
  ...
  do {
    ...
    if (newFormat) {
      if (!src.Parse1DMatrix(
              4, gsl::span<float>{side->points.at(0).ToFloatPtr(), 4})) {
...
================
*/
std::shared_ptr<idMapEntity> idMapEntity::Parse(idLexer& src, bool worldSpawn,
                                                float version) {
  idToken token;
  bool worldent;
  Vector2 origin;
  ...
  do {
    ...
    if (token == "{") {
      ...
      if (token.compare(0, 5, "brush") == 0) {
        std::shared_ptr<idMapBrush> mapBrush =
            idMapBrush::Parse(src, origin, true, version);
        if (!mapBrush) {
          return nullptr;
        }
      ...
      /*else if (token.Icmpn("patch", 5) == 0) {
              mapPatch = idMapPatch::Parse(src, origin,
      !token.Icmp("patchDef3"), version); if (!mapPatch) { return NULL;
              }
...
===============
*/
void idMapEntity::RemovePrimitiveData() noexcept { primitives.clear(); }
...
===============
*/
bool idMapFile::Parse(const std::string& filename, bool ignoreRegion,
                      bool osPath) {
  ...
  while (1) {
    std::shared_ptr<idMapEntity> mapEnt =
        idMapEntity::Parse(src, (entities.size() == 0), version);
    if (!mapEnt) {
      break;
    }
