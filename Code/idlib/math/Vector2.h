...
struct Vector2 {
  ...
  Vector2(int height, int width) noexcept
      : x(static_cast<float>(height)), y(static_cast<float>(width)) {}
...
inline float Vector2::operator[](const int index) const {
  if (index < GetDimension()) return (&x)[index];

  throw std::out_of_range("trying access vector data by index " +
                          std::to_string(index));
}
...
inline float Vector2::LengthSqr() const noexcept { return (x * x + y * y); }
...
inline uint8_t Vector2::GetDimension() const noexcept { return 2; }
...
inline Vector2 Vector2::operator-() const noexcept { return Vector2(-x, -y); }
...
inline void Vector2::Vector2::Zero() noexcept { x = y = 0; }
