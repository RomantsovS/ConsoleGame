...
========================
*/
extern inline bool IEEE_FLT_IS_NAN(float x) { return x != x; }
...
========================
*/
extern inline bool IEEE_FLT_IS_INF(float x) { return x == x && x * 0 != x * 0; }
...
========================
*/
extern inline bool IEEE_FLT_IS_INF_NAN(float x) { return x * 0 != x * 0; }
...
extern inline bool IEEE_FLT_IS_DENORMAL(float x) {
  return (
      (reinterpret_cast<const unsigned int&>(x) & 0x7f800000) == 0x00000000 &&
...
template <>
inline bool IsValid(
    const float& f) {  // these parameter must be a reference for the function
                       // to be considered a specialization
  return !(IEEE_FLT_IS_NAN(f) || IEEE_FLT_IS_INF(f) || IEEE_FLT_IS_IND(f) ||
           IEEE_FLT_IS_DENORMAL(f));
}
...
================================================================================================
        ...
        floating point bit layouts according to the IEEE 754-1985 and 754-2008
standard
...
class idMath {
 public:
  static float InvSqrt(
      float x) noexcept;  // inverse square root with 32 bits precision, returns
                          // huge number when x == 0.0

  static float Sqrt(float x) noexcept;  // square root with 32 bits precision

  static float Fabs(float f) noexcept;  // returns the absolute value of the
  ...
  static const float INFINITY;  // huge number which should be larger than any
                                // valid number used
  static const float FLT_SMALLEST_NON_DENORMAL;  // smallest non-denormal 32-bit
                                                 // floating point value
};
...
========================
*/
inline float idMath::Fabs(float f) noexcept { return fabsf(f); }
...
inline int Lerp(const int from, const int to, float f) {
  return static_cast<int>(
      ...
      ((static_cast<float>(to) - static_cast<float>(from)) * f));
}
