...
class LogsSum {
 public:
  ~LogsSum() {
    try {
      std::vector<std::pair<std::string, CountTime>> res(logs.begin(),
                                                         logs.end());

      std::sort(res.begin(), res.end(),
                [](std::pair<std::string, CountTime> lhs,
                   std::pair<std::string, CountTime> rhs) {
                  return lhs.second.time < rhs.second.time;
                });

      std::ostringstream os;
      os << std::setw(60) << "name" << std::setw(20) << "time" << std::setw(20)
         << "cnt" << std::setw(20) << "cnt_gr_zero_time" << std::endl;
...
#define LOG_DURATION(message)                                                \
  LogDuration<chrono::microseconds> UNIQ_ID(__LINE__){std::string(message) + \
                                                      " microseconds"};

#define LOG_SUM_DURATION(message, duration)             \
  LogDuration<std::chrono::duration> UNIQ_ID(__LINE__){ \
      std::string(message) + ' ' + #duration, &logs};
