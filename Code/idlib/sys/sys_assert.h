...
Getting idassert() to work as we want on all platforms and code analysis tools
can be tricky.

================================================================================================
*/

bool idassertFailed(const std::string& file, int line,
...
#define ididassert(x) \
  (void)((!!(x)) || (idassertFailed(__FILE__, __LINE__, #x)))
...
#define idassert(x)     \
  ...
  ididassert(x)
#else
#define idassert(x) ididassert(x)
#endif

#define idverify(x) \
  ((x) ? true : (idassertFailed(__FILE__, __LINE__, #x), false))
