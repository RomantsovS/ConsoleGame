...
public:
	...
	bool Append(int sequence, const std::byte* b1, int b1Len, const std::byte* b2 = nullptr, int b2Len = 0);
...
*/
template< int maxItems, int maxBuffer >
bool idDataQueue< maxItems, maxBuffer >::Append(int sequence, const std::byte* b1, int b1Len, const std::byte* b2, int b2Len) {
	...
	memcpy(data.data() + dataLength, b1, b1Len);	dataLength += b1Len;
	memcpy(data.data() + dataLength, b2, b2Len);	dataLength += b2Len;
	return true;
}
