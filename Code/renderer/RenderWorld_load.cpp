...
bool idRenderWorldLocal::InitFromMap(const std::string& name) {
  ...
  if (name == mapName) {
    // if (fileSystem->InProductionMode() || (currentTimeStamp !=
    // FILE_NOT_FOUND_TIMESTAMP && currentTimeStamp == mapTimeStamp)) {
    // common->Printf("idRenderWorldLocal::InitFromMap: retaining existing
    // map\n");
    FreeDefs();
    // TouchWorldModels();
