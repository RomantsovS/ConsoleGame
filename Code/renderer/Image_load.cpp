...
void idImage::ActuallyLoadImage(bool fromBackEnd) {
  ...
  auto deleter = [](unsigned char* ptr) { stbi_image_free(ptr); };

  int img_width{}, img_height{}, img_nrChannels{};
  std::unique_ptr<unsigned char, decltype(deleter)> data_ptr(
      stbi_load(full_path.c_str(), &img_width, &img_height, &img_nrChannels, 0),
      deleter);
  if (!data_ptr) {
  ...
  ConvertImageDataToModelPixels(data_ptr.get(), img_height, img_width,
                                img_nrChannels, pixels);
  ...
  for (int j = 0; j < img_height; ++j) {
    for (int i = 0; i < img_width; ++i) {
      ...
      if (data[cur_pixel + 0] > 200 && data[cur_pixel + 1] > 200 &&
        ...
        col = colorWhite;
      } else if (data[cur_pixel + 1] > 100 && data[cur_pixel + 2] > 100 &&
                 data[cur_pixel + 0] < 100) {
      ...
      } else if (data[cur_pixel + 0] > 50 && data[cur_pixel + 0] < 200 &&
                 data[cur_pixel + 1] > 50 && data[cur_pixel + 1] < 200 &&
                 data[cur_pixel + 2] < 50) {
        col = colorBrown;
      } else if (data[cur_pixel + 0] > 100 && data[cur_pixel + 1] > 100 &&
                 data[cur_pixel + 2] < 100) {
        col = colorYellow;
      } else if (data[cur_pixel + 2] >
                 data[cur_pixel + 0] + data[cur_pixel + 1]) {
        col = (data[cur_pixel + 0] + data[cur_pixel + 1] +
               data[cur_pixel + 2]) > 300
                  ? colorLightBlue
                  : colorBlue;
      } else if (data[cur_pixel + 1] >
                 data[cur_pixel + 0] + data[cur_pixel + 2]) {
        col = (data[cur_pixel + 0] + data[cur_pixel + 1] +
               data[cur_pixel + 2]) > 200
                  ? colorLightGreen
                  : colorGreen;
      } else if (data[cur_pixel + 0] >
                 data[cur_pixel + 1] + data[cur_pixel + 2]) {
        col = (data[cur_pixel + 0] + data[cur_pixel + 1] +
               data[cur_pixel + 2]) > 300
                  ? colorLightRed
                  : colorRed;
                 ...
                 data[cur_pixel + 2] < 50) {
        col = colorBlack;
      } else if (data[cur_pixel + 0] + data[cur_pixel + 1] +
                     data[cur_pixel + 2] >
                 300) {
        col = colorLightGray;
      } else
