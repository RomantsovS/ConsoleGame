...
struct areaReference_t {
  ...
  std::shared_ptr<areaReference_t>
...
class idRenderEntity {
 public:
  ...
  virtual void UpdateRenderEntity(const renderEntity_s *re, bool forceUpdate =
  false) = 0; virtual void GetRenderEntity(renderEntity_s *re) = 0; virtual void
  ForceUpdate() = 0; virtual int	GetIndex() = 0;*/
};
...
class idRenderEntityLocal : public idRenderEntity {
 public:
  ...
  virtual void UpdateRenderEntity(const renderEntity_t *re, bool forceUpdate =
  false); virtual void GetRenderEntity(renderEntity_t *re); virtual void
  ForceUpdate(); virtual int	GetIndex();*/
  ...
  idRenderModel* dynamicModel =
      nullptr;  // if parms.model->IsDynamicModel(), this is the generated data
...
// idRenderEntityLocal, which the front end may be modifying simultaneously if
...
struct viewEntity_t {
  ...
  // be linked to the lights or added to the drawsurf list in a serial code
  // section drawSurf_t *			drawSurfs;
};
...
struct viewDef_t {
  ...
  // idScreenRect		viewport;				// in
  // real pixels and proper Y flip
  ...
  // viewDef_t *			superView;
  // // never go into an infinite subview loop const std::shared_ptr<drawSurf_t>
  // subviewSurface;

  // drawSurfs are the visible surfaces of the viewEntities, sorted
  // by the material sort parameter
  // drawSurf_t **		drawSurfs;				// we
  // don't use an idList for this, because

  std::shared_ptr<viewEntity_t>
...
class idRenderSystemLocal : public idRenderSystem {
 public:
  ...
  void DrawStretchPic(int x, int y, int w, int h, int s1, int t1,
                      const idMaterial* material) override;
  void DrawBigChar(int x, int y, int ch) override;
  void DrawBigStringExt(int x, int y, std::string_view string,
...
std::shared_ptr<viewEntity_t> R_SetEntityDefViewEntity(
...
void RB_AddDebugText(std::string text, const Vector2& origin,
                     const Screen::color_type color, const int lifetime = 0);
void RB_ClearDebugText(int time);
void RB_AddDebugLine(const Screen::color_type color, const Vector2& start,
                     const Vector2& end, const int lifeTime,
