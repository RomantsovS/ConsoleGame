...
struct portalArea_t {
  ...
  std::shared_ptr<areaReference_t>
      entityRefs;  // head/tail of doubly linked list, may change
};
...
class idRenderWorldLocal
    : public idRenderWorld,
      public std::enable_shared_from_this<idRenderWorldLocal> {
 public:
  idRenderWorldLocal();
  ...
  void UpdateEntityDef(int entityHandle,
                       gsl::not_null<const renderEntity_t*> re) override;
  ...
  void DrawTextToScreen(std::string text, const Vector2& origin,
                        const Screen::color_type color,
                        const int lifetime) override;
  void DebugLine(const Screen::color_type color, const Vector2& start,
                 const Vector2& end, const int lifetime = 0,
                 const bool depthTest = false) noexcept override;
  void DebugBounds(const Screen::color_type color, const idBounds& bounds,
                   const Vector2& org = vec2_origin,
                   const int lifetime = 0) override;
