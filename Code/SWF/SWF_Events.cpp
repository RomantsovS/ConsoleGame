...
bool idSWF::HandleEvent(const sysEvent_t* event) {
  ...
  if (event->evType == SE_KEY) {
    const std::string& keyName =
        idKeyInput::KeyNumToString((keyNum_t)event->evValue);

    idSWFScriptVar var = shortcutKeys->Get(keyName);
    // anything more than 32 levels of indirection we can be pretty sure is an
    // infinite loop
    for (int runaway = 0; runaway < 32; runaway++) {
      ...
      if (var.IsString()) {
        ...
        // make sure we don't send an onRelease event unless we have already
        // sent that object an onPress
      ...
      if (focusWindow.IsObject()) {
              ...
              if (onKey.IsFunction()) {
                      ...
                      // make sure we don't send an onRelease event unless we
      have already sent that object an onPress idSWFScriptObject* object =
      focusWindow.GetObject(); bool wasPressed =
      object->Get("_kpressed").ToBool(); object->Set("_kpressed",
      event->evValue2); if (event->evValue2 || wasPressed) { idSWFParmList
      parms; parms.Append(event->evValue); parms.Append(event->evValue2);
                              onKey.GetFunction()->Call(focusWindow.GetObject(),
      parms).ToBool(); return true;
                      }
                      else if (event->evValue == K_LSHIFT || event->evValue ==
      K_RSHIFT) { idSWFParmList parms; parms.Append(event->evValue);
    ...
    common->Warning("Circular reference in %s shortcutKeys.%s",
                    filename.c_str(), keyName);
