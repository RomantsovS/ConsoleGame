...
idSWFSpriteInstance::idSWFSpriteInstance(std::shared_ptr<idSWF> _swf) noexcept
    : swf(_swf), isVisible(false), scriptObject(nullptr) {
...
void idSWFSpriteInstance::FreeDisplayList() {
  for (size_t i = 0; i < displayList.size(); i++) {
    if (displayList[i].spriteInstance) displayList[i].spriteInstance->Clear();
    if (displayList[i].textInstance) displayList[i].textInstance->Clear();
  }
  displayList.clear();  // not calling Clear() so we don't continuously
                        // re-allocate memory
}
...
========================
*/
swfDisplayEntry_t* idSWFSpriteInstance::AddDisplayEntry(int depth,
                                                        bool isSprite) {
  size_t i = 0;
...
===================
*/
void idSWFSpriteInstance::RunTo(int targetFrame) { FreeDisplayList(); }
...
void idSWFSpriteInstance::SetXPos(float xPos) noexcept {
  ...
  if (thisDisplayEntry == NULL || thisDisplayEntry->spriteInstance != this) {
          idLib::Warning("_y: Couldn't find our display entry in our parents
  display list"); return;
  }
