...
struct cm_vertex_t {
  Vector2 p;  // vertex point
              // int						checkcount;
  // // for multi-check avoidance unsigned long			side;
  // // each bit tells at which side this vertex passes one of the trace model
  // edges unsigned long			sideSet;
  // // each bit tells if sidedness for the trace model edge has been calculated
  // yet
};
...
struct cm_brush_t {
  ...
  // int						primitiveNum;
  // // number of brush primitive int
  // numPlanes;			// number of bounding planes idPlane
  // planes[1];			// variable sized
};
...
struct cm_node_t {
  ...
  // float					planeDist;
  // // node plane distance std::shared_ptr<cm_polygonRef_t> polygons;
  // // polygons in node
...
struct cm_model_t {
  ...
  std::vector<cm_vertex_t>
      vertices{};  // array with all vertices used by the model
  // int						maxEdges;
  // // size of edge array int
  // numEdges;			// number of edges cm_edge_t* edges;
  // // array with all edges used by the model
  ...
  // cm_polygonRefBlock_t* polygonRefBlocks;	// list with blocks of polygon
  // references
  std::shared_ptr<cm_brushRefBlock_t>
      brushRefBlocks{};  // list with blocks of brush references
  // cm_polygonBlock_t* polygonBlock;		// memory block with all
  // polygons std::shared_ptr<cm_brushBlock_t> brushBlock;
  // // memory block with all brushes
  //  statistics
  /*int						numPolygons;
  int						polygonMemory;*/
  int numBrushes{};
...
struct cm_trmVertex_t {
  ...
  // idPluecker pl;
  // // pluecker coordinate for vertex movement idVec3 rotationOrigin;
  // // rotation origin for this vertex
...
struct cm_traceWork_t {
  ...
  ~cm_traceWork_t() {
#ifdef DEBUG_PRINT_Ctor_Dtor
    if (isCommonExists) common->DPrintf("%s ctor\n", "cm_traceWork_t");
#endif  // DEBUG_PRINT_Ctor_Dtor
  }
  ...
  // bool rotation;
  // // true if calculating rotational collision
  ...
  // idPlane heartPlane1;
  // // polygons should be near anough the trace heart planes float
  // maxDistFromHeartPlane1; idPlane heartPlane2; float maxDistFromHeartPlane2;
...
class idCollisionModelManagerLocal : public idCollisionModelManager {
 public:
  ...
  int SetupTrmModel(const idTraceModel&
                        trm /*, const idMaterial* material*/) noexcept override;
  ...
  void Translation(trace_t* results, const Vector2& start, const Vector2& end,
                   const idTraceModel* trm, int contentMask, int model,
                   const Vector2& modelOrigin) override;
  // stores all contact points of the trm with the model, returns the number of
  // contacts
  int Contacts(contactInfo_t* contacts, const int maxContacts,
               const Vector2& start, const Vector2& dir, const float depth,
               const idTraceModel* trm, int contentMask, int model,
               const Vector2& modelOrigin) override;

 private:  // CollisionMap_translate.cpp
  void SetupTrm(cm_traceWork_t* tw, const idTraceModel* trm) noexcept;
  void TranslationIter(trace_t* results, const Vector2& start,
                       const Vector2& end, const idTraceModel* trm,
 ...
 private:  // CollisionMap_trace.cpp
  ...
  void TraceThroughAxialBSPTree_r(cm_traceWork_t* tw, cm_node_t* node,
                                  float p1f, float p2f, Vector2& p1,
                                  Vector2& p2);
 ...
 private:  // CollisionMap_load.cpp
  ...
  void FreeTree_r(std::shared_ptr<cm_model_t> model,
                  std::shared_ptr<cm_node_t> headNode,
  ...
  std::shared_ptr<cm_brushRef_t> AllocBrushReference(cm_model_t* model,
                                                     int blockSize);
  std::shared_ptr<cm_brush_t> AllocBrush(cm_model_t* model);
  void AddBrushToNode(cm_model_t* model, cm_node_t* node,
                      std::shared_ptr<cm_brush_t> b);
  void SetupTrmModelStructure();

  // creation of raw polygons
  void ConvertBrushSides(std::shared_ptr<cm_model_t> model,
                         const std::shared_ptr<idMapBrush> mapBrush,
                         int primitiveNum) noexcept;
  void ConvertBrush(cm_model_t* model,
                    gsl::not_null<const idMapBrush*> mapBrush,
                    int primitiveNum);
  ...
  std::shared_ptr<cm_model_t> CollisionModelForMapEntity(
      const std::shared_ptr<idMapEntity>
          mapEnt);  // brush/patch model from .map
