...
void idCollisionModelManagerLocal::TraceThroughAxialBSPTree_r(
    cm_traceWork_t* tw, cm_node_t* node, float p1f, float p2f, Vector2& p1,
    Vector2& p2) {
  if (!node) {
    return;
  }
  ...
  idCollisionModelManagerLocal::TraceThroughAxialBSPTree_r(
      tw, node->children[0].get(), p1f, p2f, p1, p2);
  idCollisionModelManagerLocal::TraceThroughAxialBSPTree_r(
      tw, node->children[1].get(), p1f, p2f, p1, p2);
...
void idCollisionModelManagerLocal::TraceThroughModel(cm_traceWork_t* tw) {
  ...
  idCollisionModelManagerLocal::TraceThroughAxialBSPTree_r(
      tw, tw->model.lock()->node.get(), 0, 1, tw->start, tw->end);
}