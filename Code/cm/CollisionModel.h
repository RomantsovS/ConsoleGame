...
struct contactInfo_t {
  ...
  // Vector2					point;			// point
  // of contact Vector2					normal;
  // // contact plane normal float					dist;
  // // contact plane distance
  int contents;  // contents at other side of surface
  // const idMaterial* material;		// surface material
  // int						modelFeature;	//
  // contact feature on model int
  // trmFeature;		// contact feature on trace model
...
class idCollisionModelManager {
 public:
  ...
  virtual int SetupTrmModel(
      const idTraceModel& trm /*, const idMaterial* material*/) = 0;
  // Creates a trace model from a collision model, returns true if succesfull.
  virtual bool TrmFromModel(const std::string& modelName,
                            idTraceModel& trm) = 0;
  ...
  virtual void Translation(trace_t* results, const Vector2& start,
                           const Vector2& end, const idTraceModel* trm,
                           int contentMask, int model,
                           const Vector2& modelOrigin) = 0;
  // stores all contact points of the trm with the model, returns the number of
  // contacts
  virtual int Contacts(contactInfo_t* contacts, const int maxContacts,
                       const Vector2& start, const Vector2& dir,
                       const float depth, const idTraceModel* trm,
                       int contentMask, int model,
                       const Vector2& modelOrigin) = 0;
};
