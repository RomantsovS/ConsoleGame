...
void idCollisionModelManagerLocal::Translation(trace_t* results,
  ...
  int num_steps = static_cast<int>(
      std::ceil(idMath::Fabs(dir[max_dimension]) / CM_POINT_SIZE));
  ...
  if (num_steps > 1) {
    ...
    for (last_step_dist = vec2_origin, cur_step_dist = step_dist, cur_step = 0;
         cur_step < num_steps;
         last_step_dist = cur_step_dist, cur_step_dist += step_dist) {
      TranslationIter(results, start + last_step_dist, start + cur_step_dist,
                      trm, contentMask, model, modelOrigin);
      // if there is a collision
      if (results->fraction < 1.0f) {
        // fraction of total translation
        results->fraction = (last_step_dist[max_dimension] +
                             step_dist[max_dimension] * results->fraction) /
                            dir[max_dimension];
        return;
      }
      ++cur_step;
  ...
  TranslationIter(results, start, start + dir, trm, contentMask, model,
                  modelOrigin);
}

void idCollisionModelManagerLocal::TranslationIter(trace_t* results,
                                                   const Vector2& start,
                                                   const Vector2& end,
                                                   const idTraceModel* trm,
                                                   int contentMask, int model,
  ...
  if (start[0] == end[0] && start[1] == end[1]) {
    idCollisionModelManagerLocal::ContentsTrm(results, start, trm, contentMask,
                                              model, modelOrigin);
    return;
  }
