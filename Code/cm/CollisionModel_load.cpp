...
================
*/
void idCollisionModelManagerLocal::RemoveBrushReferences_r(
    cm_node_t* node, const cm_brush_t* b) noexcept {
  while (node) {
    for (std::shared_ptr<cm_brushRef_t> bref = node->brushes; bref;
         bref = bref->next) {
      if (bref->b.get() == b) {
        bref->b = nullptr;
        return;
...
================
*/
void idCollisionModelManagerLocal::FreeNode(
    std::shared_ptr<cm_node_t> node) noexcept {
...
================
*/
void idCollisionModelManagerLocal::FreeBrushReference(
    std::shared_ptr<cm_brushRef_t> bref) noexcept {
...
================
*/
void idCollisionModelManagerLocal::FreeTree_r(
    std::shared_ptr<cm_model_t> model, std::shared_ptr<cm_node_t> headNode,
...
void idCollisionModelManagerLocal::FreeModel(
  ...
  polygonRefBlock = nextPolygonRefBlock) { nextPolygonRefBlock =
  polygonRefBlock->next; Mem_Free(polygonRefBlock);
  }
  // free blocks with brush references
  for (brushRefBlock = model->brushRefBlocks; brushRefBlock; brushRefBlock =
  nextBrushRefBlock) { nextBrushRefBlock = brushRefBlock->next;
...
int idCollisionModelManagerLocal::LoadModel(const std::string& modelName) {
  ...
  if (models[numModels]) {
    ...
    /*if (cvarSystem->GetCVarBool("fs_buildresources")) {
            // for resource gathering write this model to the preload file for
    this map fileSystem->AddCollisionPreload(modelName);
    }*/
    return (numModels - 1);
  }
...
================
*/
int idCollisionModelManagerLocal::SetupTrmModel(
    const idTraceModel& trm) noexcept {
  int i;
...
bool idCollisionModelManagerLocal::TrmFromModel(const std::string& modelName,
                                                idTraceModel& trm) {
  int handle;
...
bool idCollisionModelManagerLocal::GetModelBounds(int model,
...
===================
*/
bool idCollisionModelManagerLocal::GetModelContents(int model,
                                                    int& contents) const {
  if (model < 0 || model > MAX_SUBMODELS || model >= numModels ||
      !models[model]) {
    common->Printf(
...
std::shared_ptr<cm_model_t> idCollisionModelManagerLocal::AllocModel() {
  ...
  model->numBrushes = model->brushMemory = model->numNodes =
      model->numBrushRefs = model->usedMemory = 0;
...
================
*/
std::shared_ptr<cm_node_t> idCollisionModelManagerLocal::AllocNode(
    cm_model_t* model, int blockSize) {
  int i;
  std::shared_ptr<cm_node_t> node;
...
std::shared_ptr<cm_brushRef_t>
idCollisionModelManagerLocal::AllocBrushReference(cm_model_t* model,
                                                  int blockSize) {
  int i;
...
std::shared_ptr<cm_brush_t> idCollisionModelManagerLocal::AllocBrush(
    cm_model_t* model) {
  int size;
...
void idCollisionModelManagerLocal::AddBrushToNode(
    cm_model_t* model, cm_node_t* node, std::shared_ptr<cm_brush_t> b) {
...
================
*/
void idCollisionModelManagerLocal::ConvertBrushSides(
    std::shared_ptr<cm_model_t> model,
    const std::shared_ptr<idMapBrush> mapBrush, int primitiveNum) noexcept {
  /*int i, j;
  ...
  for (i = 0; i < mapBrush->GetNumSides(); i++) {
          ...
          if (w.GetNumPoints()) {
                  PolygonFromWinding(model, &w, planes[i], material,
  primitiveNum);
          }
  }*/
}

void idCollisionModelManagerLocal::ConvertBrush(
    cm_model_t* model, gsl::not_null<const idMapBrush*> mapBrush,
    int primitiveNum) {
  int i, contents;
  idBounds bounds;
...
================
*/
static void CM_R_GetNodeBounds(gsl::not_null<idBounds*> bounds,
                               std::shared_ptr<cm_node_t> node) {
  while (1) {
    for (auto bref = node->brushes; bref; bref = bref->next) {
...
================
*/
void CM_GetNodeBounds(gsl::not_null<idBounds*> bounds,
                      std::shared_ptr<cm_node_t> node) {
  bounds->Clear();
...
void idCollisionModelManagerLocal::BuildModels(
    gsl::not_null<const idMapFile*> mapFile) {
  int i;
...
void idCollisionModelManagerLocal::PrintModelInfo(const cm_model_t* model) {
  ...
  // common->Printf("%6i edges (%i KB)\n", model->numEdges, (model->numEdges *
  // sizeof(cm_edge_t)) >> 10); common->Printf("%6i polygons (%i KB)\n",
...
================
*/
void idCollisionModelManagerLocal::AccumulateModelInfo(
    cm_model_t* model) noexcept {
  int i;
...
=================
*/
static void CM_EstimateVertsAndEdges(
    gsl::not_null<const idMapEntity*> mapEnt,
    gsl::not_null<int*> numVerts /*, int* numEdges*/) {
  int j;
  ...
  *numVerts = /**numEdges =*/0;
  for (j = 0; j < mapEnt->GetNumPrimitives(); j++) {
    ...
    /*if (mapPrim->GetType() == idMapPrimitive::TYPE_PATCH) {
            ...
            *numEdges += (width - 1) * height + width * (height - 1) + (width -
    1) * (height - 1); continue;
    }*/
    if (mapPrim->GetType() == idMapPrimitive::TYPE_BRUSH) {
      ...
      // the bottom
      *numVerts +=
          (std::dynamic_pointer_cast<idMapBrush>(mapPrim)->GetNumSides() - 2) *
          2;
      //*numEdges += (static_cast<const idMapBrush*>(mapPrim)->GetNumSides() -
      // 2) * 3;
      continue;
    }
  }
...
std::shared_ptr<cm_model_t>
idCollisionModelManagerLocal::CollisionModelForMapEntity(
    const std::shared_ptr<idMapEntity> mapEnt) {
  std::shared_ptr<cm_model_t> model;
  idBounds bounds;
  std::string name;
  int i{}, brushCount{};
  ...
  CM_EstimateVertsAndEdges(mapEnt.get(),
                           &model->maxVertices /*, &model->maxEdges*/);
  model->numVertices = 0;
  // model->numEdges = 0;
  model->vertices.resize(model->maxVertices);
  // model->edges = (cm_edge_t*)Mem_ClearedAlloc(model->maxEdges *
  // sizeof(cm_edge_t), TAG_COLLISION);
  ...
  for (i = 0; i < mapEnt->GetNumPrimitives(); i++) {
    ...
    if (mapPrim->GetType() == idMapPrimitive::TYPE_BRUSH) {
      ConvertBrushSides(model, std::dynamic_pointer_cast<idMapBrush>(mapPrim),
                        i);
      continue;
    }
  }
...
std::shared_ptr<cm_model_t> idCollisionModelManagerLocal::LoadBinaryModel(
...
std::shared_ptr<cm_model_t>
idCollisionModelManagerLocal::LoadBinaryModelFromFile() {
  ...
  struct local {
    static void ReadNodeTree(
        cm_model_t* model, cm_node_t* node,
        std::vector<std::shared_ptr<cm_brush_t>>& brushes) {
      node->planeType = -1;
      int i = 0;

      std::shared_ptr<cm_brushRef_t> bref =
          collisionModelManagerLocal.AllocBrushReference(model,
                                                         model->numBrushRefs);
      bref->b = brushes[i];
      bref->next = node->brushes;
      node->brushes = bref;
      ...
      /*if (node->planeType != -1) {
              node->children[0] = collisionModelManagerLocal.AllocNode(model,
      model->numNodes); node->children[1] =
      collisionModelManagerLocal.AllocNode(model, model->numNodes);
  ...
  model->usedMemory = model->numVertices * sizeof(Vector2) +
                      ...
                      // model->polygonMemory +
                      model->brushMemory + model->numNodes * sizeof(cm_node_t) +
...
bool idCollisionModelManagerLocal::TrmFromModel(const cm_model_t* model,
  ...
  /*if (model->numEdges > MAX_TRACEMODEL_EDGES + 1) {
          common->Printf("idCollisionModelManagerLocal::TrmFromModel: model %s
  has too many edges.\n", model->name.c_str());
          //PrintModelInfo(model);
          return false;
  }*/
